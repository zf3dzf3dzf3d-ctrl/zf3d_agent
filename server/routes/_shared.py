...
# 聊天附件暂存（待发图片/粘贴卡片，跨重启恢复）
_CHAT_ATTACHMENTS_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'private', '用户设置', 'chat_attachments.json')
_CHAT_ATTACHMENTS_LOCK = threading.Lock()
...
# ===== 上游请求重试：吸收网络抖动（SSL 中断/连接重置） =====
def _urlopen_retry(req, timeout, attempts=6, backoff=1.0, tag='Proxy'):
    """带重试的 urlopen。仅对"连接阶段"瞬时故障重试（此时请求未到达上游，重放安全）：
    - URLError(非 HTTPError)：TLS 握手被中断(UNEXPECTED_EOF)/连接被重置/DNS 抖动等
    - ConnectionError / ssl.SSLError：连接被对端断开
    - HTTP 502/503/504 Bad Gateway：网关瞬时故障，连接阶段即返回，重放安全
    超时不重试（重试会让等待时间翻倍）。重试全部失败后把最后一个异常抛给调用方。"""
    last = None
    for i in range(max(1, attempts)):
        try:
            ...
            if e.code in (502, 503, 504) and i < attempts - 1:
                last = e
                print('[%s] 上游网关 HTTP %d，%.1fs 后重试(%d/%d)' % (
                    tag, e.code, backoff, i + 1, attempts - 1))
                time.sleep(backoff)
                backoff *= 2
                continue
            raise
            ...
            if isinstance(reason, socket.timeout):
                raise
            if i < attempts - 1:
                print('[%s] 上游连接失败，%.1fs 后重试(%d/%d): %s' % (
                    tag, backoff, i + 1, attempts - 1, str(reason)[:160]))
                time.sleep(backoff)
                backoff *= 2
    # 兜底：所有重试耗尽后，绕过系统/环境代理直连再试一次。
    # 根因场景：本机走 Clash 等本地代理（127.0.0.1:7897），代理切换节点/重启瞬间
    # 会出现 SSL EOF / 10054 / 10061 / getaddrinfo failed，表现为"对话断网"。
    # 代理挂掉但直连可用时，这一次兜底可以救回请求，根治断网。
    try:
        print('[%s] 重试耗尽，尝试绕过代理直连兜底...' % tag)
        ...
        print('[%s] 直连兜底也失败: %s' % (tag, str(e2)[:160]))
        raise last
...
__all__ = [
    ...
    '_CHAT_ATTACHMENTS_PATH', '_CHAT_ATTACHMENTS_LOCK',
    ...
    '_SSL_CTX', '_ssrf_check_url', '_urlopen_retry', 'chat_mode_rules',
]
