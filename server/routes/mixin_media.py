...
class MixinMedia(MixinBase):
    ...
    def _handle_screenrecord_log(self):
        """前端发来的日志（桌面版同名接口移植）"""
        try:
            body = self._read_json()
        except Exception:
        ...
        try:
            ...
            消息 = body.get('消息', '')
            _写日志('前端日志', '信息', 消息)
            self._send_json({'成功': True})
        except Exception as e:
            self._send_json({'成功': False, '错误': str(e)})

    def _handle_screenrecord_test_volume(self):
        """试听音量：用系统声音文件按当前音量倍数直接播放（桌面版移植）"""
        try:
            body = self._read_json()
        except Exception:
            body = {}
        try:
            ...
            音量倍数 = body.get('音量倍数', 1.0)
            # 跨平台：优先 PATH 中的 ffmpeg，其次常见安装位置
            _ffmpeg_candidates = [
            ...
            _ffmpeg_path = next((p for p in _ffmpeg_candidates if p and os.path.exists(p)), None)
            if not _ffmpeg_path:
                return {'ok': False, 'error': '未找到 ffmpeg，请安装后加入 PATH（macOS: brew install ffmpeg / Linux: apt install ffmpeg）'}
            # 跨平台测试音源：优先系统自带提示音，找不到则用 ffmpeg 生成正弦波
            if sys.platform == 'win32':
                源音频 = r'C:\Windows\Media\chord.wav'
                if not os.path.exists(源音频):
                    源音频 = r'C:\Windows\Media\ding.wav'
            elif sys.platform == 'darwin':
                源音频 = '/System/Library/Sounds/Glass.aiff'
            else:
                源音频 = '/usr/share/sounds/alsa/Front_Center.wav'
            生成正弦波 = not os.path.exists(源音频)
            临时wav = os.path.join(tempfile.gettempdir(), f'_sr_test_{int(time.time()*1000)}.wav')
            vol = max(0, 音量倍数)
            if 生成正弦波:
                # 无系统提示音时用 ffmpeg 直接生成 1 秒 440Hz 正弦波
                cmd = [_ffmpeg_path, '-y', '-f', 'lavfi', '-i', 'sine=frequency=440:duration=1',
                       '-af', f'volume={vol}', '-ar', '44100', '-ac', '2', 临时wav]
            else:
                cmd = [_ffmpeg_path, '-y', '-i', 源音频,
                       '-af', f'volume={vol}',
                       '-ar', '44100', '-ac', '2', 临时wav]
            _sub.run(cmd, capture_output=True, timeout=10,
                     creationflags=_sub.CREATE_NO_WINDOW if os.name == 'nt' else 0)
            if os.path.exists(临时wav) and os.path.getsize(临时wav) > 100:
                def _播放():
                    try:
                        if sys.platform == 'win32':
                            ...
                            _sub.Popen(['afplay', 临时wav])
                        else:
                            # Linux：优先 paplay，其次 aplay（不支持 mp3，但此处为 wav）
                            _player = _shutil.which('paplay') or _shutil.which('aplay')
                            if _player:
                                _sub.Popen([_player, 临时wav])
                        os.remove(临时wav)
                    except Exception:
                        pass
                ...
                self._send_json({'成功': False, '错误': '音频生成失败'})
        except Exception as e:
            self._send_json({'成功': False, '错误': str(e)})
