...
class MixinBackup(MixinBase):
    ...
    def _handle_app_restart(self):
        """POST /api/app/restart - 已废弃：统一转发到 restart_server.py（与 /api/restart_server 同一套带锁逻辑），
        避免两套重启逻辑（旧：直接拉起新进程再退出 / 新：restart_server.py）互相杀掉对方拉起的服务器。"""
        try:
            import subprocess, sys
            self._send_json({'ok': True, 'msg': '重启中，页面将在几秒后自动恢复'})
            print('[POST /api/app/restart] 转发到 restart_server.py 执行统一重启...')
            script = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'restart_server.py')
            if not os.path.isfile(script):
                print('[restart] 警告: 未找到 %s' % script)
            try:
                subprocess.Popen(
                    [sys.executable, script, '--manual'],
                    cwd=os.path.dirname(script),
                    creationflags=subprocess.CREATE_NO_WINDOW if sys.platform == 'win32' else 0,
                )
            except Exception as e:
                print('[restart] 拉起重启脚本失败: %s' % e)
    ...
    def _handle_backup_delete(self, filename):
        ...
        try:
            ...
            try:
                _m2t(filepath, source='backup_delete')
            except Exception:
                os.remove(filepath)
            self._send_json({'ok': True})
        except Exception as e:
