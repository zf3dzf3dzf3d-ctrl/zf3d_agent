...
class MixinProxy(MixinProxyStream, MixinBase):
    ...
    def _unmask_auth_headers(headers):
        ...
        try:
            ...
            if not real:
                # 兜底：末4位大小写/空白不一致时忽略大小写再匹配
                from model_config import _load_keys_map, _load_model_file_keys
                tl = tail.lower()
                for _src in (_load_keys_map(), _load_model_file_keys()):
                    for _rk in (_src or {}).values():
                        _rk = str(_rk or '').strip()
                        if _rk and _rk.lower().endswith(tl):
                            real = _rk
                            break
                    if real:
                        break
    ...
    # =================== [ChatGate] 对话串行管理器接入 ===================
    # 所有发往大模型的请求必须先过闸门（chat_gate.py）：
    #   1) 全局同一时刻至多 1 个请求在与大模型通信（严格串行，不打架）；
    #   2) 相邻两个请求的"起跑时刻"之间至少隔 gap 秒（面板可调，默认 2s）；
    #   3) 每个对话框(_box_id)可在独立面板单独放行/禁用，全程可监控。
    # 面板地址：/gate-panel.html  控制 API：POST /api/gate/control
    def _gate_acquire(self, body, entry):
        """过闸门，返回 ticket；被拒时抛 chat_gate.GateRejected。
        v4：从 _target_url 提取接口域名作为模型分组键（provider），
        闸门据此把不同大模型瓜分到互不相交的车道组。"""
        import chat_gate
        box_id = ''
        try:
            box_id = str(body.get('_box_id') or body.get('box_id') or '')
        except Exception:
            box_id = ''
        engine = ''
        model = ''
        ...
        try:
            ...
            model = str((body.get('_body') or {}).get('model') or '')
            from urllib.parse import urlparse
            provider = urlparse(str(body.get('_target_url') or '')).hostname or ''
        except Exception:
            pass
        return chat_gate.acquire(box_id, engine=engine, model=model,
                                  entry=entry, provider=provider)

    def _gate_reject_response(self, exc):
        """闸门拒绝：统一 JSON 错误响应（流式未握手前也是 JSON，前端可读）。"""
        try:
            code = int(getattr(exc, 'http_code', 503))
        except Exception:
            code = 503
        self._send_json({'ok': False, 'gate': True, 'error': str(exc)}, code)
    ...
    def _handle_proxy(self):
        ...
        _ticket = None
        try:
            _ticket = self._gate_acquire(body or {}, 'proxy')
        except chat_gate.GateRejected as _gr:
            self._gate_reject_response(_gr)
            return
        except Exception as _ge:
            # 闸门自身异常不阻断业务（放行直通），避免管理器故障导致整个系统不可用
            print('[ChatGate] 闸门异常(直通放行): %s' % _ge)
        _ok, _err = True, ''
        self._gate_net_err = ''   # 【v4.2让道】每请求清零（keep-alive 连接复用安全）
        try:
            return self._handle_proxy_impl()
        except Exception as _e:
            _ok, _err = False, str(_e)
            raise
        finally:
            if _ticket:
                # 【v4.2让道】impl 内网络级失败被消化为正常响应时，靠打标传递给闸门
                _net_err = getattr(self, '_gate_net_err', '')
                try:
                    chat_gate.release(_ticket, ok=_ok and not _net_err,
                                      error=_err or _net_err)
                except Exception:
                    pass
    ...
    def _handle_proxy_stream(self):
        ...
        _ticket = None
        try:
            _ticket = self._gate_acquire(body or {}, 'proxy_stream')
        except chat_gate.GateRejected as _gr:
            self._gate_reject_response(_gr)
            return
        except Exception as _ge:
            print('[ChatGate] 闸门异常(直通放行): %s' % _ge)
        _ok, _err = True, ''
        self._gate_net_err = ''   # 【v4.2让道】每请求清零
        try:
            return self._handle_proxy_stream_impl()
        except Exception as _e:
            _ok, _err = False, str(_e)
            raise
        finally:
    ...
    def _handle_proxy_impl(self):
        try:
            body = self._read_body()
            # [2026-09-11 fix] 兼容 {endpoint, headers, payload} 简化格式（此前 Missing _target_url -> 401）
            if not body.get('_target_url') and body.get('endpoint'):
                body = {'_target_url': body.get('endpoint'),
                        '_method': body.get('method', 'POST'),
                        '_headers': body.get('headers') or {},
                        '_body': body.get('payload') or {}}
            target_url = body.get('_target_url', '')
            method = body.get('_method', 'POST')
            headers = body.get('_headers', {})
            payload = body.get('_body', {})

            # [2026-09-11 fix3] 思考型模型兜底：max_tokens 过小会被 reasoning 耗尽，
            # 导致 finish_reason=length & content_len=0 空回复（日志出现102次）。
            # 非流式 chat 请求且 max_tokens < 8192 时自动放大到 8192。
            try:
                if (isinstance(payload, dict) and payload.get('model')
                        and isinstance(payload.get('messages'), list)
                        and not payload.get('stream')):
                    _mt = payload.get('max_tokens')
                    if _mt and int(_mt) < 8192:
                        payload['max_tokens'] = 8192
                        print('[Proxy][fix3] max_tokens %s -> 8192 (model=%s)' % (_mt, payload.get('model')))
            except Exception as _e:
                print('[Proxy][fix3] skip: %s' % _e)
            ...
            # 【v5.1.3】Anthropic 协议线路：转换请求体 + 换协议头（x-api-key / anthropic-version）
            _anth_hit = False
            try:
                ...
                if _anth_hit:
                    print('[Proxy] anthropic adapter -> %s' % target_url)
            except ImportError:
                pass
            except Exception as _anth_e:
                print('[Proxy] anthropic adapter error: %r' % _anth_e)
            ...
            # 鍙戦€佽姹傦紙甯﹁秴鏃讹級
            try:
                resp = _urlopen_retry(req, timeout=(chat_mode_rules.get_request_timeout(loop_mode) if chat_mode_rules is not None else 300), tag='Proxy')
                resp_body = resp.read().decode('utf-8', errors='replace')
                resp_status = resp.getcode()
                if resp_status == 200:
                    try:
                        import network_guard
                        network_guard.report(True)  # 成功 → 断网计时清零
                    except Exception:
                        pass
                ...
                # 【v5.1.3】Anthropic 响应转回 OpenAI 格式再回给前端
                if _anth_hit and resp_status == 200 and isinstance(resp_data, dict) \
                        and isinstance(resp_data.get('content'), list) and not resp_data.get('_sse_aggregated'):
                    try:
                        from common.anthropic_adapter import from_anthropic_response
                        resp_data = from_anthropic_response(resp_data)
                    except Exception as _cve:
                        print('[Proxy] anthropic response convert error: %r' % _cve)
                ...
                try:
                    import network_guard
                    network_guard.report(False, str(e.reason))  # 连接失败 → 断网计时
                except Exception:
                    pass
                self._send_json({
                    'ok': False,
                ...
                self._gate_net_err = str(e)   # 【v4.2让道】打标
                try:
                    import network_guard
                    network_guard.report(False, 'timeout')  # 超时 → 断网计时
                except Exception:
                    pass
                self._send_json({
                    'ok': False,
