...
class HotReloader:
    ...
    PY_MODULES = {
        ...
        # tool 包及其子模块（修改任一工具处理器时热重载）
        # tool/ 在项目根目录，路径相对于 server_dir 的上一级
    }
    ...
    def _scan_all_files(self):
        ...
        # tool/ 目录在项目根（server_dir 的上一级），自动发现 backend/ 下所有 .py 文件
        tool_dir = os.path.join(os.path.dirname(self.server_dir), 'tool')
        for sub_dir in ('coding', 'writing', 'minimal'):
            ...
            for f in sorted(os.listdir(backend_dir)):
                if f.endswith('.py') and not f.startswith('_'):
                    fn = 'tool/' + sub_dir + '/backend/' + f
                    fp = os.path.join(backend_dir, f)
                    if os.path.isfile(fp):
                        self._py_mtimes[fn] = os.path.getmtime(fp)
                        mod_name = 'tool.' + sub_dir + '.backend.' + f[:-3]
    ...
    def _check_py_changes(self):
        """检查 Python 文件变更"""
        # 先扫描 tool/ 下各分类的 backend/ 确保新文件被注册
        tool_dir = os.path.join(os.path.dirname(self.server_dir), 'tool')
        for sub_dir in ('coding', 'writing', 'minimal'):
            backend_dir = os.path.join(tool_dir, sub_dir, 'backend')
            if not os.path.isdir(backend_dir):
        ...
        for fn in self.PY_MODULES:
            # tool/ 下的文件相对项目根目录，其余相对 server 目录
            fp = os.path.join(os.path.dirname(self.server_dir), fn) if fn.startswith('tool/') \
                else os.path.join(self.server_dir, fn)
    ...
    def _reload_python(self, changed_files):
        ...
        # tool/ 包下的工具处理器（模块名以 tool. 开头），不在 reload_order 里，单独追加
        for fn in changed_files:
            mod_name = self.PY_MODULES.get(fn)
            if mod_name and mod_name.startswith('tool.'):
                to_reload.append((fn, mod_name))
        ...
        if all_ok:
            try:
                ...
                all_ok = False
                print(f'[HotReload]   ❌ 重建 DBHandler 失败: {e}')
                traceback.print_exc()
