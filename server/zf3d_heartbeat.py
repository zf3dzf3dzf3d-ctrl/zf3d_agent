...
# 默认 API Key（心跳/命令协议认证，不依赖用户登录）
# 优先级：本地数据库 config(zf3d/heartbeat_api_key) > private/heartbeat_key.json
# [安全] 密钥只存 private/heartbeat_key.json（隐私区，不入 git），代码内不硬编码
def _load_default_key():
    try:
        _kf = os.path.join(BASE_DIR, 'private', 'heartbeat_key.json')
        with open(_kf, 'r', encoding='utf-8-sig') as f:
            _k = json.load(f).get('api_key', '')
            if _k:
                return _k
    except Exception:
        pass
    # 未配置时返回空串：服务端会拒绝认证，需在 private/heartbeat_key.json 配置 api_key
    return ''
...
def _get_login_status():
    ...
    try:
        ...
        # 管理员也正常上报心跳（user_group 仅为标记，不再跳过）
...
def _send_heartbeat():
    ...
    # 读取本地模型列表，随心跳上传到网站（手机端 models_list 直接读网站缓存，解决服务器无 F 盘问题）
    models_json = ''
    try:
        mj_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                               os.pardir, 'public', 'config', 'models.json')
        if os.path.exists(mj_path):
            # utf-8-sig：读文本并剥掉可能的 UTF-8 BOM，避免上传到网站的 JSON 带头污染
            with open(mj_path, 'r', encoding='utf-8-sig') as _mjf:
                models_json = _mjf.read()
    except Exception:
        models_json = ''

    # 读取本地项目列表（projects 表）+ 当前活动项目，随心跳上传
    projects_json = ''
    try:
        from config import DB_PATH as _hb_db
        import sqlite3 as _sq
        _c = _sq.connect(_hb_db, timeout=5)
        try:
            _rows = _c.execute(
                'SELECT id, name FROM projects ORDER BY updated_at DESC LIMIT 50').fetchall()
            _active = ''
            try:
                _r = _c.execute(
            ...
            projects_json = json.dumps({
                'projects': [{'id': str(r[0]), 'name': str(r[1])} for r in _rows],
                'active_project_id': _active}, ensure_ascii=False)
        finally:
            _c.close()
    except Exception:
        projects_json = ''
...
def start_heartbeat():
    ...
    # 热更新会重载本模块、把命名空间里的 _heartbeat_thread 重置为 None，
    # 但旧线程仍在跑（控制台出现"心心跳跳已已上上报报"交错输出）。
    # 按线程名查活：只要进程里还有活着的 zf3d-heartbeat 线程就接管它，绝不重复启动。
    for t in threading.enumerate():
        if t is not _heartbeat_thread and t.name == 'zf3d-heartbeat' and t.is_alive():
            _heartbeat_thread = t
            _heartbeat_running = True
            print('[zf3d-heartbeat] 心跳线程已在运行（热更新后接管旧线程）')
            return
    ...
    # 线程循环首行会打印"心跳线程已启动，机器ID: xxx"，这里不再重复打印
