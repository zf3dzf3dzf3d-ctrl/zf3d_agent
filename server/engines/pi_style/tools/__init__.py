...
def pi_write(args, ctx):
    ...
    from path_policy import sanitize_write_path, backup_path
    full, warn = sanitize_write_path(full)
    existed = os.path.isfile(full)
    if existed:
        backup_path(full, tag='pi_write')  # 统一备份到 system/backup/，不再在原文件旁落 .bak
    os.makedirs(os.path.dirname(full) or '.', exist_ok=True)
    content = args.get('content', '')
    with open(full, 'w', encoding='utf-8') as f:
        f.write(content)
    return _ok('wrote path=%s bytes=%d replaced=%s%s' % (args.get('path'), len(content), existed, (' ' + warn) if warn else ''), '')
...
def pi_run(args, ctx):
    import subprocess
    try:
        rc, _o, _e, _note = run_shell(args.get('code', ''), timeout=RUN_TIMEOUT, cwd=ctx.get('project_path') or '.')
    except Exception as _ex:
        return _err('run failed: %s' % _ex)
    out = ((_o or b'') + (_e or b'')).decode('utf-8', 'replace')
    return _ok('exit=%d' % rc, clip_text(out, CLIP, '…[pipeline clip]'))
