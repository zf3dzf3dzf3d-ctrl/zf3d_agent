...
def sandbox_snapshot(full_path, tag='write', source='ai'):
    """沙箱保护：写入前记录可撤销快照（common/sandbox.py）。永不抛错，返回 op_id 或 None。"""
    try:
        from engines.common.sandbox import snapshot_before_write
        return snapshot_before_write(full_path, tag=tag, source=source)
    except Exception:
        return None
...
def core_file_warning(full_path):
    ...
    if is_core_file(full_path):
        return ('\n[安全墙提示] 这是核心文件（%s）。改动前已自动备份到 system/backup/。'
