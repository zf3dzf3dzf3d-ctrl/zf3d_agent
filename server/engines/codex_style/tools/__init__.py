...
from engines.common.shell_exec import run_shell
...
def codex_propose_write(args, ctx):
    ...
    try:
        from path_policy import sanitize_write_path
        full, _warn = sanitize_write_path(full)
    except Exception:
        pass
    old = ''
...
def codex_run_code(args, ctx):
    ...
    try:
        import sys as _sys
        _ti_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(
            os.path.dirname(os.path.abspath(__file__))))), 'tools', 'coding', 'backend')
        if _ti_dir not in _sys.path:
            _sys.path.insert(0, _ti_dir)
        import trash_intercept
        code, trash_notes = trash_intercept.rewrite_command(
            code, ctx.get('project_path') or '.')
    except Exception as _te:
        trash_notes = ['[zf-trash] 拦截器异常(命令原样执行): %s' % _te]
    _audit(ctx, 'codex_run_code', {'cmd': code[:200], 'timeout': timeout})
    try:
        rc, _o, _e, _note = run_shell(code, timeout=timeout, cwd=ctx.get('project_path') or '.')
        out = (_o or b'').decode('utf-8', 'replace')[-6000:]
        err = (_e or b'').decode('utf-8', 'replace')[-3000:]
        result = 'exit=%d\n--- stdout ---\n%s\n--- stderr ---\n%s' % (rc, out or '(empty)', err or '(empty)')
        if trash_notes:
            result = '\n'.join(trash_notes) + '\n' + result
        return True, result
    except subprocess.TimeoutExpired:
        msg = 'command timed out after %ss (killed)' % timeout
        if trash_notes:
            msg = '\n'.join(trash_notes) + '\n' + msg
        return False, msg
...
def _get_mode_file():
    try:
        with open(_APPROVAL_FILE, 'r', encoding='utf-8-sig') as f:
            return f.read().strip()
    except OSError:
        return 'auto_edit'
