...
def _light_guard(messages, single_limit=16000, total_chars=300000):
    """免压缩：不做滑窗。只做两级兜底：
    1) 超长单条工具结果截断（正常不会触发）
    2) 总量守卫：接近 128K 上限时，丢最旧的普通轮次（保留 system/首条），防止 400 轮长对话撑爆上下文报错"""
    for m in messages:
    ...
    if total <= total_chars:
        return messages
    # 从最旧开始丢（跳过 system 和第一条用户消息），每丢一条重算
    keep_head = 0
    for i, m in enumerate(messages):
        if m.get('role') == 'system':
            ...
            break
    i = keep_head + 1  # 保留第一条真实用户消息
    while i < len(messages) - 2 and sum(len(m.get('content') or '') for m in messages) > total_chars:
        m = messages[i]
        if m.get('role') in ('user', 'assistant', 'tool'):
            del messages[i]
        else:
            i += 1
    messages.insert(keep_head + 1, {'role': 'system',
        'content': '（注：对话过长，最早的部分历史已被自动裁剪以保持在上下文上限内）'})
    print('[deepseek_direct] 总量守卫触发，裁剪后总字符 %d' % sum(len(m.get('content') or '') for m in messages))
    return messages
