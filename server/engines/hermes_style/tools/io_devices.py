...
if IS_WIN:
    ...
    try:
        ...
        _mouse = _pm.Controller()
        _kb = _KbCtrl()
        _KEY_ALIASES = {
            "space": _Key.space, "enter": _Key.enter, "return": _Key.enter,
            "esc": _Key.esc, "escape": _Key.esc, "tab": _Key.tab,
            "backspace": _Key.backspace, "delete": _Key.delete, "del": _Key.delete,
            "shift": _Key.shift, "ctrl": _Key.ctrl, "control": _Key.ctrl,
            "alt": _Key.alt, "win": _Key.cmd, "cmd": _Key.cmd, "meta": _Key.cmd,
            ...
            "capslock": _Key.caps_lock, "f1": _Key.f1, "f2": _Key.f2,
            "f3": _Key.f3, "f4": _Key.f4, "f5": _Key.f5, "f6": _Key.f6,
            "f7": _Key.f7, "f8": _Key.f8, "f9": _Key.f9, "f10": _Key.f10,
            "f11": _Key.f11, "f12": _Key.f12,
        }
    except ImportError:
        _mouse = _kb = None
        _KEY_ALIASES = {}
...
def _t_mouse(args, ctx):
    ...
    if not IS_WIN:
        # ---------- mac/linux: pynput 实现 ----------
        if _mouse is None:
            return "ERR pynput not installed (pip3 install pynput)"
        try:
            if action == "get":
                x, y = _mouse.position
                return "OK x=%d y=%d" % (int(x), int(y))
            if action in ("move", "move_rel"):
                if action == "move":
                    x, y = int(args["x"]), int(args["y"])
                else:
                    cx, cy = _mouse.position
                    x, y = int(cx) + int(args.get("dx", 0)), int(cy) + int(args.get("dy", 0))
                _mouse.position = (x, y)
                return "OK moved to x=%d y=%d" % (x, y)
            if action == "click":
                btn = str(args.get("button", "left")).lower()
                times = 2 if btn == "double" else int(args.get("times", 1))
                b = _Btn.right if btn == "right" else _Btn.left
                for _ in range(times):
                    _mouse.click(b)
                    time.sleep(0.02)
                return "OK clicked %s x%d" % (btn, times)
            if action == "scroll":
                _mouse.scroll(0, int(args.get("delta", 0)))
                return "OK scrolled %s" % args.get("delta", 0)
...
def _t_key(args, ctx):
    ...
    if not IS_WIN:
        ...
        try:
            if action == "get":
                return "OK key=%s pressed=unknown(non-windows)" % args.get("key")
            if action == "press":
                ...
                for k in keys:
                    ...
                    if n in _KEY_ALIASES:
                        keyobjs.append(_KEY_ALIASES[n])
                    else:
                        keyobjs.append(n)   # 普通字符
                with _kb.pressed(*keyobjs):
                    pass
            ...
            if action == "type":
                ...
                _kb.type(text)   # pynput 原生支持 unicode/中文
