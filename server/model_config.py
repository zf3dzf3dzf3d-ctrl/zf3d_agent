...
def save_models_config(payload):
    ...
    for m in items:
        ...
        new_key = m.get('key', '') or m.get('apiKey', '') or ''
        if is_masked_key(new_key) or new_key == '':
            # 掩码（原样回传）或空值 -> 均保留服务端已存的真实 key，避免前端整体保存时把未改动的密钥清空
...
def get_default_model():
    """取 isDefault=true 的模型；没有则取第一个。返回真实 key。"""
    cfg = load_models_config(include_key=True)
    items = cfg.get('list', [])
    for m in items:
        if m.get('isDefault'):
