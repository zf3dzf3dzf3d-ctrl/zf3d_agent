...
DEFAULT_API_PORT = 8520
DEFAULT_WS_PORT  = 8521
...
def _load_version():
    ...
    if os.path.exists(LEGACY_APP_CONFIG):
        try:
            with open(LEGACY_APP_CONFIG, 'r', encoding='utf-8-sig') as f:
                data = json.load(f)
            if isinstance(data, dict) and isinstance(data.get('version'), str) and data['version'].strip():
                return data['version'].strip()
        except Exception:
    ...
    if os.path.exists(LEGACY_VERSION_PATH):
        try:
            with open(LEGACY_VERSION_PATH, 'r', encoding='utf-8-sig') as f:
                v = f.read().strip()
                if v:
                    return v
        except Exception:
...
def _load_port():
    ...
    if os.path.exists(LEGACY_PORT_PATH):
        try:
            with open(LEGACY_PORT_PATH, 'r', encoding='utf-8-sig') as f:
