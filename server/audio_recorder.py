...
_锁 = threading.Lock()
_日志路径 = Path(__file__).parent.parent.parent / "隐私区" / "我的日志" / "录音诊断.jsonl"


def _写诊断日志(事件, 数据=None):
    """记录录音生命周期，供接口查询且不依赖前端是否仍保持连接。"""
    记录 = {"时间": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "事件": 事件, "数据": 数据 or {}}
    try:
...
def 查询诊断日志(限制=100):
    try:
        限制 = max(1, min(500, int(限制)))
        if not _日志路径.exists():
            return []
        with open(_日志路径, "r", encoding="utf-8") as f:
            行列表 = f.readlines()[-限制:]
        return [json.loads(行) for 行 in reversed(行列表) if 行.strip()]
    except Exception:
        return []
...
class 录音器:
    ...
    def 停止录制(音量倍数: float = 1.0) -> dict:
        with _锁:
            ...
            # 保留用户选择的增益；此前的峰值归一化会把任意非零倍率拉回同一音量。
            try:
                增益 = max(0.1, min(5.0, float(音量倍数)))
            ...
            # 转MP3。增益已应用到样本，不能再做 loudnorm，否则滑块会被抵消。
            最终路径 = wav路径
