...
(function () {
  ...
  window.UserSettings = {
      ...
      serverLoadPromise = fetch('/api/user-settings', { method: 'GET' })
        ...
        .then(function (data) {
          if (data && data.ok && data.settings && typeof data.settings === 'object') {
            delete data.settings[LS_MIRROR]; // 防镜像套镜像膨胀
            // 服务器为准覆盖缓存（剔除自身镜像键，防止镜像套镜像无限膨胀）
            var s = data.settings;
            delete s[LS_MIRROR];
            cache = s;
            lsWrite(cache);
    ...
    /* 结果验证（第二轮验证）：on=每次任务完成后自动再发送一次验证消息；off=不验证 */
    getChatVerifyMode: function (chatId) {
      var p = preferencesCache || {}, c = p.chatVerifyMode && p.chatVerifyMode[String(chatId)];
      return c || p.defaultVerifyMode || 'off';
    },
    setChatVerifyMode: function (chatId, mode) {
      preferencesCache = preferencesCache || {};
      preferencesCache.defaultVerifyMode = mode;
      preferencesCache.chatVerifyMode = preferencesCache.chatVerifyMode || {};
      if (chatId) preferencesCache.chatVerifyMode[String(chatId)] = mode;
      var changes = { defaultVerifyMode: preferencesCache.defaultVerifyMode, chatVerifyMode: preferencesCache.chatVerifyMode };
  ...
  function flushPreferences() {
    ...
    return fetch('/api/user-preferences', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ preferences: payload }) })
      .then(function (r) {
