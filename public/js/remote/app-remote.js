...
// WebSocket 服务：ws://host:8516（server/remote/ws_server.py，独立线程）
(function () {
    'use strict';
    ...
    try {
        ...
        if (_pj) WSPORT = parseInt(_pj.getAttribute('content'), 10) || 8521;
    } catch (e) {}
    ...
    function connectWS(cb) {
        ...
        ws.onopen = function () {
            ...
            log('已连接信令服务器');
            // 注册
            send({ t: 'register', id: RM.idHash, ver: (window.App && App.VERSION) || '5.1.2' });
            cb && cb();
        };
        ws.onclose = function () {
            RM.wsReady = false;
