...
Object.assign(App, {
    ...
    openProjectPanel: function() {
        ...
        // 修复（5.0.2）：面板初始 translateX(100%) 在视口外，Chromium 合成器可能
    ...
    // 加载记忆中的活动项目（5.0.4 重写：统一走 project-sync.js 的恢复 + 广播）
    _loadActiveProject: function() {
        var self = this;
        if (typeof self.initProjectSync === 'function') {
        ...
        try {
            ...
            var savedName = '';
            try { savedName = localStorage.getItem('active_project_name') || ''; } catch(e2) {}
            if (saved) {
                if (typeof self.setActiveProjectUnified === 'function' && self._projSyncInited) {
                    self.setActiveProjectUnified(saved, savedName);
                } else {
                    self.activeProject = { id: saved, name: savedName };
                    self._activeProjectId = saved;
                }
            }
        } catch(e) {}
        // 2. 再从 DB 加载最新值（如果在线，以 DB 为准；名称从项目列表反查）
        if (typeof DB !== 'undefined') {
            DB.getActiveProject().then(function(res) {
                if (res && res.ok && res.data !== null && res.data !== undefined) {
                    var pid = res.data;
                    try { pid = JSON.parse(pid); } catch(e) {}
                    // 【修复】DB 存的是空串时视为"无活动项目记录"，
                    // 不能用它覆盖 localStorage 里的正确记忆（否则每次打开面板
                    // 活动项目都被清空 → 文件树回落到默认第一个项目）
                    if (pid === '' || pid === '""') return;
                    var pname = self._lookupProjectName ? self._lookupProjectName(pid) : '';
                    // 统一走新系统（若尚未初始化则只设内存值，initProjectSync 会广播）
                    if (typeof self.setActiveProjectUnified === 'function' && self._projSyncInited) {
                        self.setActiveProjectUnified(pid || null, pname);
                    } else {
                        self.activeProject = { id: pid || null, name: pname || '' };
    ...
    _updateAllChatProjectBtns: function() {
        ...
        // 轻量刷新项目数据（不依赖面板 DOM，供 setActiveProject 联动用）
        _refreshProjectsData: function(cb) {
            var self = this;
            var mergeLocal = function(list) {
                list = list || [];
                if (Store.data && Store.data.projects) {
                    Store.data.projects.forEach(function(lp) {
                        if (!list.find(function(p) { return p.id === lp.id; })) list.push(lp);
                    });
                }
                self._projAllProjects = list;
                if (cb) try { cb(); } catch (e) {}
            };
            if (typeof DB !== 'undefined' && DB.online) {
                DB.getProjects().then(function(res) {
                    mergeLocal((res && res.ok && res.data) ? res.data : (self._projAllProjects || []));
                }).catch(function() { mergeLocal(self._projAllProjects || []); });
            } else {
                mergeLocal(Store.data && Store.data.projects ? Store.data.projects.slice() : (self._projAllProjects || []));
            }
        },

        // 设置当前活动项目（单选，永久保存到 DB + localStorage）
    setActiveProject: function(pid, pname) {
        // 5.0.4 重写：统一走 project-sync.js 的唯一入口
        this.setActiveProjectUnified(pid, pname);
    },

    _legacySetActiveProject: function(pid) {
        var self = this;
        ...
        // 同步：当前激活对话的项目归属跟随活动项目 + 立即刷新对话框项目文本
        try {
            var _actChat = null;
            if (this.chatBoxes) {
                for (var _ci = 0; _ci < this.chatBoxes.length; _ci++) {
                    var _cb = this.chatBoxes[_ci];
                    if (_cb && _cb.el && _cb.el.classList.contains('active')) { _actChat = _cb; break; }
                }
            }
            if (_actChat && String(_actChat.projectId || '') !== String(pid || '')) {
                _actChat.projectId = pid || null;
                if (Store.data && Store.data.chatBoxes) {
                    for (var _si = 0; _si < Store.data.chatBoxes.length; _si++) {
                        if (Store.data.chatBoxes[_si].id === _actChat.id) {
                            Store.data.chatBoxes[_si].projectId = pid || null;
                            break;
                        }
                    }
                }
                if (typeof DB !== 'undefined' && DB.online && DB.setNodeProject) {
                    DB.setNodeProject(_actChat.id, pid || '').catch(function() {});
                }
                Store.addLog('info', _actChat.id, 'project-switch', '跟随当前项目: ' + (pid || '无'));
            }
            // 无论归属是否变化，都刷新按钮文本（覆盖标题面板 .proj-name 和配置栏 .proj-label）
            if (typeof this._updateAllChatProjectBtns === 'function') {
                this._updateAllChatProjectBtns();
            } else if (typeof this._updateProjectBtn === 'function') {
                try { this._updateProjectBtn(_actChat); } catch (e) {}
            }
        } catch (e) {}

        // 项目数据可能尚未加载（面板没打开过）：轻量刷新，保证按钮文字实时更新
        try { if (typeof this._refreshProjectsData === 'function') this._refreshProjectsData(function() {
            if (typeof self._updateAllChatProjectBtns === 'function') self._updateAllChatProjectBtns();
            if (self._projPanelOpen) self._renderProjects();
        }); } catch (e) {}

        // 联动文件树：若文件树面板已打开且根项目与活动项目不同，切换到新项目根目录
        try {
            if (this._ftPanelOpen && String(this._ftProjId || '') !== String(pid || '')) {
                this.openFileTreePanel(pid, '');
            }
        } catch (e) {}
    },
    ...
    _bindProjectEvents: function() {
        ...
        body.querySelectorAll('.proj-item-header').forEach(function(header) {
            header.addEventListener('click', function(e) {
                ...
                // 点击项目名称：打开左侧文件树面板（显示该项目文件夹 + 缩略图区域）
                if (e.target.closest('.proj-name') && typeof self.openFileTreePanel === 'function') {
                    var proj = (self._projAllProjects || []).find(function(p) { return String(p.id) === String(pid); });
                    self.openFileTreePanel(pid, proj ? proj.name : '');
                    // 不阻止展开/收起，保持原有行为
                }
            ...
            if (typeof DB !== 'undefined' && DB.online) {
                DB.createProject(folderName).then(function(res) {
                    ...
                    DB.linkFolder(projId, folderPath).then(function() {
                        // 【5.1.0 修复】通知文件树：新项目路径已关联，立即定位
                        try { document.dispatchEvent(new CustomEvent('project-folder-changed', { detail: { projectId: projId, folderPath: folderPath } })); } catch (e) {}
    ...
    deleteProject: async function(pid) {
        ...
        // 如果删除的是当前活动项目，清除选择（统一项目同步系统）
        if (String(self._activeProjectId || '') === String(pid)) {
            self.setActiveProjectUnified(null, '');
        }
    ...
    restoreChatFromProject: function(nid) {
        ...
        for (var j = 0; j < this.chatBoxes.length; j++) {
            if (this.chatBoxes[j].id === nid) {
                // 【5.1.0 修复】激活时同步活动项目到该对话所属项目，📁 按钮才能正确显示
                var apid = this.chatBoxes[j].projectId || null;
                if (apid) {
                    var apname = this._lookupProjectName ? this._lookupProjectName(String(apid)) : '';
                    this.setActiveProjectUnified(String(apid), apname, { skipChatSync: true });
                }
    ...
    _initProjectPanel: function() {
        ...
        // projectBtn 按钮已从顶栏移除（文件树入口统一为对话框上方 📁），此处仅做兼容空判断
        if (projectBtn && !projectBtn.dataset.projectBound) {
            projectBtn.dataset.projectBound = 'true';
            projectBtn.title = '打开文件树 / 缩略图';
            projectBtn.textContent = '🌳';
            projectBtn.addEventListener('click', function() {
                // 顶栏按钮直接打开左侧文件树（不再打开右侧项目面板）
                if (App._ftPanelOpen && App._ftProjId && String(App._ftProjId) === String(fpid || '')) {
                    // 已打开同一项目：保持面板打开（仅右上角 ✕ 关闭），不重复加载
                    return;
                }
                var fpid = App._activeProjectId || null;
                var fproj = null;
                if (fpid && App._projAllProjects) {
                    fproj = App._projAllProjects.find(function(p) { return String(p.id) === String(fpid); });
                }
                App.openFileTreePanel(fpid, fproj ? fproj.name : '');
            });
        }
        // 点击遮罩关闭
        var overlay = document.getElementById('projPanelOverlay');
