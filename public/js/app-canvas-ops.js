...
(function () {
  ...
  function buildExplorePanel() {
    ...
    p.innerHTML =
      ...
      '  <button class="cao-btn cao-btn-sm" id="cao-rollback">↩️ 回滚上一版</button>' +
      '  <button class="cao-btn cao-btn-sm" id="cao-discard">🗑 丢弃沙箱</button>' +
      '</div>' +
      '<div class="cao-env-row">' +
      '  <button class="cao-btn cao-btn-sm" id="cao-autopilot">🤖 AI 托管：关</button>' +
      '</div>';
    host.appendChild(p);
    var chip=document.getElementById('cao-exp-chip');
    if(!chip){chip=document.createElement('div');chip.id='cao-exp-chip';chip.textContent='🧭';chip.title='探索模式 / 沙箱工具（点击展开/收起，Alt+1 显示/隐藏入口）';chip.style.display='none';chip.addEventListener('click',function(){p.style.display=(p.style.display==='none'||!p.style.display)?'flex':'none';});host.appendChild(chip);}
    ...
    document.getElementById('cao-rollback').addEventListener('click', rollbackLast);
    document.getElementById('cao-autopilot').addEventListener('click', toggleAutopilot);
    paintPanel();
  }

  // ================= 🤖 AI 托管（自动驾驶）=================
  // 人零管理：AI 自己进沙箱改，改成功自动应用到正式，改失败自动回滚丢弃。
  var _autopilot = false;
  var _apWatch = null;

  function toggleAutopilot() {
    _autopilot = !_autopilot;
    if (_autopilot) startAutopilot(); else stopAutopilot('已关闭 AI 托管');
    saveKV(KV_MODE, { mode: _mode, autopilot: _autopilot });
  }

  function startAutopilot() {
    if (_env === 'prod') enterSandbox(); // 自动进沙箱
    toast('🤖 AI 托管已开启：AI 自动改、自动应用，失败自动回滚');
    paintAutopilotBtn();
    if (_apWatch) clearInterval(_apWatch);
    _apWatch = setInterval(autopilotTick, 4000);
  }

  function stopAutopilot(msg) {
    _autopilot = false;
    if (_apWatch) { clearInterval(_apWatch); _apWatch = null; }
    paintAutopilotBtn();
    if (msg) toast('🤖 ' + msg);
  }

  function paintAutopilotBtn() {
    var b = document.getElementById('cao-autopilot');
    if (!b) return;
    b.textContent = _autopilot ? '🤖 AI 托管：开' : '🤖 AI 托管：关';
    b.classList.toggle('cao-on', _autopilot);
  }

  // 每次心跳：检查沙箱内节点运行结果，无人干预自动决策
  function autopilotTick() {
    if (!_autopilot || _env !== 'sandbox') return;
    var layers = (window.FlowGlam && FlowGlam._layers) || [];
    var running = 0, failed = 0, done = 0;
    layers.forEach(function (l) {
      var st = api.getState()[l.id] || {};
      Object.keys(st).forEach(function (k) {
        var s = st[k].status;
        if (s === 'running' || s === 'sending') running++;
        else if (s === 'failed' || s === 'error') failed++;
        else if (s === 'done' || s === 'ok' || s) done++;
      });
    });
    if (running > 0) return;              // 还在跑，等
    if (failed > 0) {                     // 有失败 → 自动回滚到改动前
      rollbackLast();
      toast('🤖 检测到 ' + failed + ' 个节点失败，已自动回滚到上一版');
      return;
    }
    // 全部完成且与上次应用版本有差异 → 自动应用到正式
    var cur = takeSnapshot('sandbox');
    var last = _sandboxSnapshots[_sandboxSnapshots.length - 1];
    if (last && JSON.stringify(cur.state) === JSON.stringify(last.state)) return; // 无变化
    applySandbox();
    toast('🤖 沙箱改动已自动应用到正式（历史版本 +1）');
  }

  // 全局快捷键：Alt+1 切换 🧭 探索模式入口；Alt+2 切换 ⚙️ 画布工具入口（默认隐藏）
  document.addEventListener('keydown', function (ev) {
    ...
    if (k === '1') {
      var chip = document.getElementById('cao-exp-chip');
      if (chip) chip.style.display = (chip.style.display === 'none') ? 'flex' : 'none';
    } else {
      var gear = document.getElementById('fgEditGear');
      if (gear) gear.style.display = (gear.style.display === 'none') ? 'flex' : 'none';
    }
  });
  ...
  // 一键回滚：把当前正式状态存入历史，再恢复到上一份历史快照（可反复按，来回切换）
  function rollbackLast() {
    if (!_sandboxSnapshots.length) { toast('还没有历史版本可回滚'); return; }
    var cur = takeSnapshot('prod');
    _sandboxSnapshots.push(cur);
    var target = _sandboxSnapshots[_sandboxSnapshots.length - 2]; // 上一版
    restoreSnapshot(target);
    _prodSnapshot = target;
    _env = 'prod';
    saveKV(KV_SNAP, { prod: _prodSnapshot, history: _sandboxSnapshots.slice(-10) });
    paintPanel(); markSandboxUI(false);
    toast('↩️ 已回滚到上一版（' + new Date(target.ts).toLocaleTimeString() + '），再按可切回来');
  }
...
`;
  ...
  function restoreAll() {
    ...
    loadKV(KV_MODE, null).then(function (m) {
      if (m && m.autopilot) { _autopilot = true; startAutopilot(); }
    });
  }
