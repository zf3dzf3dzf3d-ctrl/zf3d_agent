...
(function() {
    ...
    const HOT_RELOAD = {
        ...
        _skipFiles: [
            ...
            'js/_scan.js',
            // 语音输入模块自带防重复加载守卫（重插 script 会触发"重复加载"告警），
            // 且其状态（录音中）不能被热重载重置，禁止热更新。
            'js/voice-input.js',
        ],
        ...
        _connect() {
            ...
            try {
                ...
                this._sse.onerror = (event) => {
                    ...
                    // readyState=0 表示浏览器正在自动重连，属正常现象（如后端热重载重启时）
                    // 降级为 debug 避免控制台刷屏；只有 CLOSED 才值得警告
                    const logFn = readyState === 0 ? console.debug.bind(console) : console.warn.bind(console);
                    logFn(`[HotReload] SSE 异常 (readyState=${readyState})`, event);
        ...
        /**
         * 动态重载 JS 文件
         * 【2026-09 最终策略】应用户要求：JS 变更不再自动重执行、也不再自动刷新页面。
         *   - 原地重执行会让模块的定时器/observer/监听器叠加（AI 每保存一次叠一层）→ 页面卡死；
         *   - 自动 location.reload() 会打断用户正在进行的操作。
         * 现行为：只弹一次「代码已更新，刷新页面后生效」提示，用户自行选择刷新时机。
         * CSS 仍即时热替换（无执行风险）。
         */
        _reloadJS(filePath) {
            if (this._jsToastTimer) return;   // 8 秒节流：批量保存只提示一次
            this._jsToastTimer = setTimeout(() => { this._jsToastTimer = null; }, 8000);
            this._showToast('代码已更新，刷新页面后生效', 'info', filePath);
        },
