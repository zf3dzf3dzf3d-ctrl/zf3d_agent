...
Object.assign(Tools, {
        ...
        execute: function(name, args, context) {
            ...
            switch (name) {
                ...
                case 'read_file':
                    // 支持（path）单文件与（paths 数组）多文件
                    // 【2026-09-09 重复读拦截】把对话级已读计数透传给服务端 read 工具：
                    // 服务端对比文件 mtime（stat 现取现比，可信），未变更且已读过 → 只回
                    // 「cached 提示」不回全文，杜绝上下文重建后重复读同一文件刷爆上下文
                    //（事故：AI 无视 ⚠️已读提示把 chat_gate.py 读 6 次，read 占 53% 上下文）。
                    // 文件被修改过（mtime 变化）→ 正常返回全文，不受影响。
                    (function () {
                        var _c = self._getToolCache(executionChatId);
                        if (_c && !_c.readFiles) _c.readFiles = {};
                    })();
                    var _rt = {};
                    try { _rt = (self._getToolCache(executionChatId) || {}).readFiles || {}; } catch (e) { _rt = {}; }
                    if (args.paths && args.paths.length) {
                        args.paths.forEach(function (p) { if (p) self._trackReadFile(p, executionChatId); });
                        return callToolApi('read', { paths: args.paths, max_chars: args.max_chars || 8000, _read_count: _rt }, '读取文件');
                    }
                    if (args.path) {
                        self._trackReadFile(args.path, executionChatId);
                        return callToolApi('read', { path: args.path, max_chars: args.max_chars || 8000, _read_count: _rt }, '读取文件');
                    }
                ...
                case 'read_lines':
                    ...
                    // 【2026-09-09 重复读拦截】同 read_file：透传已读计数给服务端判 mtime
                    (function () {
                        var _c = self._getToolCache(executionChatId);
                        if (_c && !_c.readFiles) _c.readFiles = {};
                    })();
                    var _rt2 = {};
                    try { _rt2 = (self._getToolCache(executionChatId) || {}).readFiles || {}; } catch (e) { _rt2 = {}; }
                ...
                case 'table_memory':
                    // 通用表格记忆（SQLite 持久化）：create_table/insert/query/get/update/delete/stats/batch/drop_table/list_tables
                    var tmParams = {
                        action: args.action || 'list',
                        table: args.table || '',
                        data: args.data || '',
                        where: args.where || '',
                        id: args.id != null ? args.id : null,
                        order: args.order || '',
                        limit: args.limit != null ? args.limit : null,
                        field: args.field || '',
                        operations: args.operations || ''
                    };
                    if (tmParams.data && typeof tmParams.data !== 'string') tmParams.data = JSON.stringify(tmParams.data);
                    if (tmParams.where && typeof tmParams.where !== 'string') tmParams.where = JSON.stringify(tmParams.where);
                    if (tmParams.operations && typeof tmParams.operations !== 'string') tmParams.operations = JSON.stringify(tmParams.operations);
                    return callToolApi('table_memory', tmParams, 'table_memory ' + (args.action || 'list') + (tmParams.table ? ' -> ' + tmParams.table : ''));
                case 'table_viewer':
                    // 数据表查看器（纯前端）：调用 window.TableViewer 可视化展示 table_memory 的表
                    var tvAction = args.action || 'open_list';
                    if (typeof window.TableViewer === 'undefined') {
                        return { success: false, message: '数据表查看器未加载（window.TableViewer 不可用）', tool: 'table_viewer' };
                    }
                    if (tvAction === 'open_table' && args.table) {
                        window.TableViewer.showTable(args.table);
                        return { success: true, message: '已打开数据表查看器并定位到表：' + args.table, tool: 'table_viewer', table: args.table };
                    }
                    window.TableViewer.open();
                    return { success: true, message: '已打开数据表查看器（表列表）', tool: 'table_viewer' };
                ...
                case 'browser_control':
                    return callToolApi('browser_control', {
                        ...
                        key: args.key || '',
                        script: args.script || '',
                        expression: args.expression || args.script || '',
                        full: args.full,
                        path: args.path || '',
                        session: args.session || '',
                        headless: args.headless,
                        all: args.all
                    }, '浏览器 ' + (args.action || 'open'));
                case 'switch_port':
                ...
                case 'video_edit':
                    return callToolApi('video_edit', args, '视频剪辑 ' + (args.action || 'info'));
                default:
