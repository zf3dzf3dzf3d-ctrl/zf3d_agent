...
// 【2026-09 性能优化】旧实现每 1s 全页 getElementById + renderModeOptions（含 DOM 写入，91ms）空转。
// 改为：面板存在才启动轮询，面板关闭自停，间隔放宽到 2s。
(function () {
  var _mpTimer = null;
  function _ensurePanelRender() {
    if (document.getElementById('modelPanelMount') && !_mpTimer) {
      _mpTimer = setInterval(function () {
        if (!document.getElementById('modelPanelMount')) {
          clearInterval(_mpTimer); _mpTimer = null; // 面板已关，停轮询
          return;
        }
        ModePlugins.renderModeOptions();
      }, 2000);
    }
  }
  document.addEventListener('click', _ensurePanelRender, true);
  _ensurePanelRender();
})();
