...
(function () {
    ...
    var BOX_SELECTOR = '.chatbox, [data-voice-box], .dog-guard-chat, .mc-agent-window, #qcQuickBar, .brain-panel';
    ...
    function triggerSend(box) {
        ...
        if (btn) {
            ...
            // 【修复】语音命令触发的发送不应走"空消息快捷继续"逻辑（否则再次说"发送"会凭空发出/排队一条"继续"）
            box.__voiceAutoSend = true;
            // 兜底：若点击未消费标记（如按钮被替换），100ms 后自动清除
            setTimeout(function() { box.__voiceSendPending = false; box.__voiceAutoSend = false; }, 100);
            btn.click();
            return;
        }
    ...
    function start(box, btn) {
        ...
        state.active = true;
        window.__VOICE_INPUT_ACTIVE__ = true; // 暴露给语音唤醒模块：听写进行中，暂停待机监听
    ...
    function finish() {
        ...
        clearTimers();
        state.active = false;
        window.__VOICE_INPUT_ACTIVE__ = false; // 通知语音唤醒模块：听写结束，可恢复待机
        state.rec = null;
        setBtnState(false);
    ...
    document.addEventListener('click', function (e) {
        ...
        var box = btn.closest('.chatbox') || btn.closest('[data-voice-box]') || btn.closest('.dog-guard-chat') || btn.closest('[data-mc-agent-input]') || btn.closest('#qcQuickBar') || btn.closest('.brain-panel');
        if (!box) return;
    ...
    document.addEventListener('pointerdown', function (e) {
        ...
        if (e.target.closest && (e.target.closest('.chatbox') === state.box || e.target.closest('[data-voice-box]') === state.box || e.target.closest('.dog-guard-chat') === state.box || e.target.closest('#qcQuickBar') === state.box || e.target.closest('.brain-panel') === state.box)) return;
        stop(true);
    }, true);
