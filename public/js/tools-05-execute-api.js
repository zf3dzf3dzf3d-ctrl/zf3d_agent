...
Object.assign(Tools, {
        // ===== 调用后端 /api/tools/*（异步） =====
        // ===== 改动记账：包装工具 Promise，把成功改动记入 ledger，供"撤销本步"回滚 =====
        _ledgerWrap: function(promise, chatId, collectFn) {
            var self = this;
            if (!self._taskLedgers) self._taskLedgers = {};
            var key = chatId || '_default_';
            var ledger = self._taskLedgers[key] || (self._taskLedgers[key] = []);
            return Promise.resolve(promise).then(function(result) {
                try {
                    if (result && result.success) {
                        collectFn(result.data || {}, ledger);
                        self._ledgerPersist(key, ledger); // 落盘持久化
                    }
                } catch (e) { console.warn('[LedgerWrap] 记账失败:', e); }
                return result;
            });
        },
        _clearTaskLedger: function(chatId) {
            if (this._taskLedgers) delete this._taskLedgers[chatId || '_default_'];
            // 同步清除落盘账本标记（恢复起点），旁路失败静默
            try {
                var k = 'undo_ledger_done_' + (chatId || '_default_');
                try { localStorage.removeItem(k); } catch (e) {}
                fetch('/api/tools/read_lines', { method: 'OPTIONS' }).catch(function() {});
            } catch (e) {}
        },
        // ===== 账本落盘：改动同步写 KV（跨刷新持久化）=====
        _ledgerPersist: function(chatId, ledger) {
            try {
                var key = 'undo_ledger_' + (chatId || '_default_');
                var arr = ledger || [];
                // 【防膨胀护栏】账本最多保留最近 50 条，超出截头（撤销回滚按倒序取，截头不影响）
                if (arr.length > 50) arr = arr.slice(arr.length - 50);
                try { localStorage.setItem(key, JSON.stringify(arr)); } catch (e) {}
            } catch (e) {}
        },
        // ===== 账本恢复：刷新页面后从落盘账本读回未撤销的改动 =====
        _ledgerRestore: function(chatId) {
            var key = 'undo_ledger_' + (chatId || '_default_');
            if (!this._taskLedgers) this._taskLedgers = {};
            var key2 = chatId || '_default_';
            if (this._taskLedgers[key2] && this._taskLedgers[key2].length) return this._taskLedgers[key2];
            try {
                var raw = localStorage.getItem(key);
                if (raw) {
                    var arr = JSON.parse(raw);
                    if (Array.isArray(arr) && arr.length) this._taskLedgers[key2] = arr;
                }
            } catch (e) {}
            return this._taskLedgers[key2] || (this._taskLedgers[key2] = []);
        },
        // ===== 撤销本步任务所有文件改动（写文件→还原/删除；移动→移回） =====
        _undoTaskChanges: function(chatId) {
            var self = this;
            var key = chatId || '_default_';
            var ledger = self._ledgerRestore(chatId);
            if (!ledger.length) {
                return Promise.resolve({ success: true, message: '本步没有可撤销的文件改动。' });
            }
            // 倒序撤销，后发生的改动先回滚
            var changes = ledger.slice().reverse().map(function(op) {
                return { op: op.op || 'write', path: op.path || '', backup_path: op.backup_path || '', src: op.src || '', dst: op.dst || '' };
            }).filter(function(op) {
                return (op.op === 'write' && op.path) || (op.op === 'move' && op.src && op.dst);
            });
            return fetch('/api/tools/undo_changes', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ _chat_id: chatId, changes: changes })
            }).then(function(r) { return r.json(); }).then(function(data) {
                self._clearTaskLedger(chatId);
                self._ledgerPersist(chatId, []); // 撤销后清空落盘账本
                if (data && data.ok) {
                    return { success: true, message: '已撤销 ' + (data.undone !== undefined ? data.undone : changes.length) + '/' + changes.length + ' 项改动。' };
                }
                return { success: false, message: '撤销失败：' + ((data && data.error) || '未知错误') };
            }).catch(function(err) {
                return { success: false, message: '撤销请求失败：' + err.message };
            });
        },
        _callToolApi: function(tool, payload, label) {
            var self = this;
            if (!payload._chat_id) payload._chat_id = self.currentChatId || '';
            // 宽松超时：run_code 20 分钟（可能跑长任务），browser_control 10 分钟（网页加载慢），其余 8 分钟。
            // 用 AbortController 真正断开请求，而不是无限等；超时仅中断本工具，不影响对话主流程。
            var _TOOL_FETCH_TIMEOUTS = { run_code: 20 * 60 * 1000, browser_control: 10 * 60 * 1000 };
            var _timeoutMs = _TOOL_FETCH_TIMEOUTS[tool] || 8 * 60 * 1000;
            var _ctrl = new AbortController();
            var _timer = setTimeout(function() { try { _ctrl.abort(); } catch (e) {} }, _timeoutMs);
            return fetch('/api/tools/' + tool, {
                ...
                if (data && data.ok) {
                    var msg = '';
                    // ===== zf_toolkit：把工具返回的图片路径渲染为 <img>（走 /api/fs/file 通道，实时展示过程图） =====
                    if (tool === 'zf_toolkit') {
                        var imgs = data.images || (data.image ? [data.image] : (data.thumbnail ? [data.thumbnail] : []));
                        var zkHtml = '';
                        for (var zi = 0; zi < imgs.length; zi++) {
                            var zPath = imgs[zi];
                            if (!zPath) continue;
                            var zUrl = '/api/fs/file?path=' + encodeURIComponent(zPath) + '&t=' + Date.now();
                            zkHtml += '<div class="zk-proc-img" style="margin:6px 0;"><img src="' + zUrl + '" style="max-width:100%;max-height:260px;border-radius:8px;border:1px solid rgba(255,255,255,.15);cursor:zoom-in;" onclick="window.open(\'/api/fs/file?path=\' + encodeURIComponent(\'' + zPath.replace(/'/g, "\\'") + '\'))" title="' + zPath + '" onerror="this.parentNode.innerHTML=\'<div style=\\"opacity:.6;font-size:12px\\">图片加载失败: ' + zPath + '</div>\'"></div>';
                        }
                        if (zkHtml) { html = zkHtml + '<div style="opacity:.75;font-size:12px;margin-top:4px;">\U0001F5BC ' + (data.message || data.status || '\u5de5\u5177\u5df2\u6267\u884c') + '</div>'; data.html = html; } + '<div style="opacity:.75;font-size:12px;margin-top:4px;">🖼 ' + (data.message || data.status || '工具已执行') + '</div>';
                    }
                    if (tool === 'read') {
                        if (data.multi && data.files) {
                            ...
                            for (var fi = 0; fi < data.files.length; fi++) {
                                ...
                                if (fr.error) {
                                    ...
                                    // 【重复读拦截】文件未变更且已读过：不回正文，提示模型复用上下文
                                    lines.push('🔁 ' + fr.path + '：' + (fr.note || '【重复读拦截】该文件已读过且未变更，请直接使用上下文中已有内容；确需重读请传 force: true。'));
                                } else {
                            ...
                            msg = '🔁 ' + data.path + '：' + (data.note || '【重复读拦截】该文件已读过且未变更，请直接使用上下文中已有内容；确需重读请传 force: true。');
                        } else {
                        ...
                        if (data.multi && data.files) {
                            ...
                            for (var rli = 0; rli < data.files.length; rli++) {
                                ...
                                if (rf.error) {
                                    ...
                                    // 【重复读拦截】文件未变更且已读过
                                    rlLines.push('🔁 ' + rf.path + '：' + (rf.note || '【重复读拦截】该文件已读过且未变更，请直接使用上下文中已有内容；确需重读请传 force: true。'));
                                } else if (rf.lines) {
                    ...
                    // ===== 内置浏览器：截图回读并内嵌展示（file 动作读 base64 → data URL） =====
                    if (tool === 'browser_control') {
                        var _shot = data.screenshot || '';
                        var _shotErr = data.shot_error || '';
                        var _bUrl = data.url || '';
                        var _bTitle = data.title || '';
                        var _buildShotHtml = function (_b64) {
                            var _h = '';
                            if (_bUrl) _h += '<div class="tool-bc-meta" style="margin-bottom:6px;font-size:12px;opacity:.75;">🌐 ' + self.escapeHtml(_bTitle) + ' — ' + self.escapeHtml(_bUrl) + '</div>';
                            if (_b64) {
                                _h += '<img src="data:image/png;base64,' + _b64 + '" style="max-width:100%;border-radius:6px;border:1px solid rgba(128,128,128,.35);display:block;" alt="页面截图">';
                            } else if (_shotErr) {
                                _h += '<div style="color:#e8a13a;">⚠️ 截图失败：' + self.escapeHtml(_shotErr) + '</div>';
                            }
                            return _h;
                        };
                        if (_shot) {
                            return fetch('/api/tools/browser_control', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ action: 'file', path: _shot, _chat_id: payload._chat_id })
                            }).then(function (r) { return r.json(); }).then(function (_d) {
                                return {
                                    success: true,
                                    message: (_bTitle ? _bTitle + ' — ' : '') + _bUrl,
                                    html: _buildShotHtml((_d && _d.b64) || ''),
                                    tool: tool,
                                    data: data
                                };
                            }).catch(function () {
                                // file 回读失败时降级为纯文本，不让整条结果报错
                                return { success: true, message: (_bTitle ? _bTitle + ' — ' : '') + _bUrl, tool: tool, data: data };
                            });
                        }
                        // 无截图的动作（content/eval/tabs/close 等）
                        var _bcMsg = data.message || '';
                        if (data.text) _bcMsg = (_bTitle ? _bTitle + ' — ' : '') + _bUrl + '\n\n' + data.text;
                        if (data.tabs) {
                            var _tLines = [];
                            for (var _ti = 0; _ti < data.tabs.length; _ti++) _tLines.push((_ti + 1) + '. ' + data.tabs[_ti].title + ' — ' + data.tabs[_ti].url);
                            _bcMsg = '已打开 ' + data.tabs.length + ' 个标签页：\n' + _tLines.join('\n');
                        }
                        if (data.result !== undefined) _bcMsg = String(data.result);
                        msg = _bcMsg || (_bTitle ? _bTitle + ' — ' : '') + _bUrl;
                        // ===== 浏览器面板：tabs / close 等动作以卡片形式展示当前标签状态 =====
                        // ===== 帖子跟踪：track_save / track_note 结果卡片 =====
                        if (data.record) {
                            var _tk = '<div class="tool-bc-panel" style="border:1px solid rgba(128,128,128,.3);border-radius:8px;padding:10px;margin-top:4px;">';
                            _tk += '<div style="font-weight:600;font-size:13px;margin-bottom:6px;">📌 已沉淀长期跟踪记录</div>';
                            _tk += '<div style="font-size:12px;opacity:.85;">记录文件：' + self.escapeHtml(String(data.record).split(/[\\\\/]/).pop()) + '</div>';
                            if (_bUrl) _tk += '<div style="font-size:12px;opacity:.75;">URL：' + self.escapeHtml(_bUrl) + '</div>';
                            if (data.title) _tk += '<div style="font-size:12px;opacity:.75;">标题：' + self.escapeHtml(data.title) + '</div>';
                            _tk += '<div style="font-size:12px;opacity:.6;margin-top:4px;">后续对话可通过 project_record 读取该记录，对比帖子变化。</div></div>';
                            return { success: true, message: msg, html: _tk, tool: tool, data: data };
                        }
                        if (data.tabs) {
                            var _panel = '<div class="tool-bc-panel" style="border:1px solid rgba(128,128,128,.3);border-radius:8px;padding:10px;margin-top:4px;">';
                            _panel += '<div style="font-weight:600;font-size:13px;margin-bottom:6px;">🖥️ 内置浏览器 · ' + data.tabs.length + ' 个标签页</div>';
                            for (var _pj = 0; _pj < data.tabs.length; _pj++) {
                                var _t = data.tabs[_pj];
                                _panel += '<div style="font-size:12px;padding:4px 6px;border-radius:5px;' + (_t.active ? 'background:rgba(80,150,255,.15);font-weight:600;' : 'opacity:.8;') + '">'
                                    + (_t.active ? '📍 ' : '　 ') + (_pj + 1) + '. ' + self.escapeHtml(_t.title || '(无标题)')
                                    + '<br><span style="opacity:.7;margin-left:1.4em;">' + self.escapeHtml(_t.url) + '</span></div>';
                            }
                            _panel += '</div>';
                            return { success: true, message: msg, html: _panel, tool: tool, data: data };
                        }
                    }
                ...
                if (err && err.name === 'AbortError') {
                    return { success: false, message: label + '执行超过 ' + Math.round(_timeoutMs / 60000) + ' 分钟未返回，前端已断开等待（后端可能仍在执行）。建议把大任务拆小分步推进。', tool: tool };
                }
