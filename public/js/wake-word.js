// ===== 语音唤醒模块（Wake Word）v3 =====
// 功能：底部状态栏（风筝/守卫右侧）一个 🎙️ 话筒按钮，默认关闭。可点击开启。
// 听到唤醒词（如"小风你好"）后：
//   1) TTS 回应"我在"；
//   2) 自动弹出 Tab/右键同款快速创建条（QuickCreate）；
//   3) 自动点它的语音按钮，开始正式听写；
//   4) 说出"发送"两个字 → 自动截掉"发送"并提交，弹出新的对话（快速条发送流程本身就是新建对话）。
// 首次需要用户点一次话筒授权麦克风，之后 localStorage 记住，默认关闭。可点击开启。
(function () {
    'use strict';
    ...
    var STORE_KEY = 'wakeWordEnabled'; // 存到 private/用户设置/user_settings.json（经 UserSettings），localStorage 仅缓存
    ...
    var enabled = (function () {
        var v = null;
        try { if (window.UserSettings && UserSettings.get) v = UserSettings.get(STORE_KEY, null); } catch (e) {}
        if (v === null || v === undefined) { try { v = localStorage.getItem(STORE_KEY); } catch (e) {} } // 兼容旧缓存迁移
        return v === '1' || v === 1; // 默认关闭：只有用户显式开启过才启用
    })();
    var rec = null;
    var listening = false;
    var lastWakeAt = 0;
    ...
    function saveEnabled() {
        try {
            if (window.UserSettings && UserSettings.set) {
                UserSettings.set(STORE_KEY, enabled ? '1' : '0'); // 写入 user_settings.json
            } else {
                localStorage.setItem(STORE_KEY, enabled ? '1' : '0'); // 兜底
            }
        } catch (e) {}
    }
    ...
    function updateBtn() {
        ...
        if (btn) {
            ...
            btn.title = enabled
                ? '语音唤醒：已开启。说"你好小峰"（或"小风你好/小峰同学"等）即弹出对话条并自动录音，直接说话即可；说"发送"即可发出。点击关闭。'
                : '语音唤醒：已关闭。点击开启。';
        }
    }
