...
(function () {
    ...
    function msSpeak(text, onDone) {
        try {
            ...
            // 【修复】加 8 秒超时：后端 edge-tts 子进程超时是 30 秒，网络慢时前端会干等
            // 半分钟才 502 回退——表现为"任务完成后很久才说话"。8 秒未响应立即回退。
            var _ctrl = ('AbortController' in window) ? new AbortController() : null;
            var _timer = _ctrl ? setTimeout(function() { _ctrl.abort(); }, 8000) : null;
            fetch('/api/tts', {
                method: 'POST',
                ...
                .catch(function () { if (_timer) clearTimeout(_timer); fallbackSpeak(text, onDone); });
