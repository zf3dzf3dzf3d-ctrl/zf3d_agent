// ========== app-health.js - 健康守护（纯报时器模式） =========
// 规则（2026-09-09 定：与对话系统完全解耦，只是给"人"的报时提醒）：
// 1. 每隔 intervalMinutes（30~60分钟）弹出休息提醒，第一次可以点"再干一会儿"关闭一次
// 2. 关闭后继续工作满 graceMinutes(10分钟) → 弹出强制休息倒计时（无按钮，到点自动结束）
//    ——不再禁用任何输入、不拦截发送、不暂停 Agent 循环；对话照常可用
// 3. 所有计时只在"人在"（有鼠标/键盘活动）时走，人离开计时暂停，回来继续
// 4. 倒计时用 Web Worker 计时：人离开电脑也照样走，到点自动消失
...
var HealthGuard = {
    ...
    _isCountingDown: false,      // 是否处于 3-2-1 预警倒计时阶段
    _lockTotalSeconds: 0,        // 本次锁定总时长（进度条用）
    _warnTimer: null,            // 预警倒计时定时器

    // 人在不在检测（2026-09-11 规则：以鼠标/键盘等输入为准，
    // 5 分钟内没有任何输入 → 判定人离开，工作计时暂停；
    // 离开超 10 分钟 → 工作计时清零）。
    _IDLE_THRESHOLD_SEC: 300,
    ...
    // ===== 人在吗？ =====
    // 规则：以最近一次输入（鼠标/键盘/滚轮/触摸）为准，
    // 5 分钟内没有任何输入 → 判定人离开（工作计时暂停，超 10 分钟清零）。
    ...
    _tick: function() {
        var self = this;
        if (self._isCountingDown) return; // 3-2-1 预警期间不计锁定时间
        if (self._isLocked) {
            // 锁定中：倒计时由 Web Worker/降级定时器（_lockTick）统一每秒递减，这里不重复递减
            return;
        }
    ...
    _showRestModal: function(isForceLock) {
        ...
        // 沉浸休息模式（2026-09-09 用户定规则）：强制休息时画面铺满全屏、
        // 背景暗色模糊，形成"该离开屏幕休息了"的沉浸氛围；
        // 网络与对话不中断（不禁用输入、不拦截发送），无按钮，到点自动结束。
        if (isForceLock) {
            modal.classList.add('force-fullscreen');
        }
        ...
        var buttonsHtml;
        if (isForceLock) {
            // 报时器语义（2026-09-09 用户定规则）：只提醒不强制，人可随时跳过。
            // 不再禁用输入、不影响任何对话——对话系统与此模块完全无关。
            buttonsHtml = ''; // 2026-09-11: 强制休息无按钮，到点自动结束
        } else {
            buttonsHtml =
                (self._snoozeUsed ? '' :
        ...
        modal.innerHTML =
            '<div class="health-rest-box' + (isForceLock ? ' force-lock' : '') + '">' +
                '<div class="health-rest-header">' +
                    (isForceLock
                        ? '<span class="health-rest-title">⏰ 休息时间提醒</span>'
                ...
                '<div class="health-rest-body">' +
                    ...
                    (isForceLock
                        ? '<div class="health-lock-countdown" id="healthLockCountdown">剩余休息时间: ' + self._config.forceLockMinutes + ':00</div>' +
                          '<div class="health-rest-progress"><div class="health-rest-progress-fill" id="healthRestProgress"></div></div>'
        ...
        if (isForceLock) {
            ...
            // 报时器：人可随时跳过休息（立即结束提醒并关弹窗），不强制、不影响对话
            var skipBtn = document.getElementById('healthSkipRestBtn');
            if (skipBtn) {
                skipBtn.addEventListener('click', function() {
                    if (typeof Store !== 'undefined') {
                        Store.addLog('info', '', 'health-skip',
                            '健康守护：用户跳过休息立即继续（报时提醒，不强制）');
                    }
                    self._endForceLock();
    ...
    _showForceLock: function() {
        var self = this;
        // 先播 3-2-1 预警动画，让人有心理预期，倒计时结束再进入休息画面
        self._startPreLockCountdown();
    },

    // ===== 3-2-1 预警倒计时（全屏数字动画，数完进入休息画面） =====
    _startPreLockCountdown: function() {
        var self = this;
        if (document.getElementById('healthCountdownOverlay')) return;

        // 移除可能存在的普通提醒弹窗
        var old = document.getElementById('healthRestModal');
        if (old && old.parentNode) old.parentNode.removeChild(old);

        self._isCountingDown = true;
        var overlay = document.createElement('div');
        overlay.id = 'healthCountdownOverlay';
        overlay.className = 'health-countdown-overlay';
        overlay.innerHTML = '<div class="health-countdown-num" id="healthCountdownNum">3</div>';
        document.body.appendChild(overlay);

        var n = 3;
        var numEl = document.getElementById('healthCountdownNum');
        var finish = function() {
            if (self._warnTimer) { clearInterval(self._warnTimer); self._warnTimer = null; } // 停掉预警循环，防止 finish 被重复触发导致锁定无限重置
            overlay.classList.add('health-countdown-out');
            setTimeout(function() {
                if (overlay.parentNode) overlay.parentNode.removeChild(overlay);
                self._isCountingDown = false;
                self._enterForceLock();
            }, 450);
        };
        var step = function() {
            n--;
            ...
            if (numEl) {
                ...
                void numEl.offsetWidth; // 重置动画
                numEl.classList.add('pop');
            }
        };
        if (numEl) numEl.classList.add('pop');
        self._warnTimer = setInterval(step, 1000);
    },

    // ===== 进入强制休息（全屏沉浸画面） =====
    _enterForceLock: function() {
        var self = this;
        self._isLocked = true;
        self._lockTotalSeconds = (self._config.forceLockMinutes || 10) * 60;
        self._lockRemainSeconds = self._lockTotalSeconds;

        // 显示锁定弹窗（全屏沉浸、置顶）
        self._showRestModal(true);

        // 报时器（2026-09-09 用户定规则）：不禁用任何输入。
        // 网络继续、对话继续、AI 循环照常可用——画面只是提醒人去休息。
        ...
        if (typeof Store !== 'undefined') {
            Store.addLog('info', '', 'health-force-lock',
                '健康守护：休息提醒触发（关闭提醒后连续工作超' + self._config.graceMinutes + '分钟），建议休息' + self._config.forceLockMinutes + '分钟（全屏沉浸提醒，可跳过，不影响对话）');
        }
    },
    ...
    _lockTick: function() {
        ...
        // 休息进度条（随剩余时间推进）
        var bar = document.getElementById('healthRestProgress');
        if (bar && self._lockTotalSeconds > 0) {
            var pct = Math.min(100, ((self._lockTotalSeconds - self._lockRemainSeconds) / self._lockTotalSeconds) * 100);
            bar.style.width = pct + '%';
        }
    ...
    _endForceLock: function() {
        var self = this;
        // 统一收口锁定时钟：无论自然结束还是"跳过休息"，都停掉 Worker/定时器，
        // 防止跳过后残留的 _lockTick 再次触发 _endForceLock（重复日志/重复重置）
        if (self._lockWorker) { self._lockWorker.terminate(); self._lockWorker = null; }
        if (self._countdownTimer) { clearInterval(self._countdownTimer); self._countdownTimer = null; }
    ...
    _closeRestModal: function() {
        ...
        // 清理 3-2-1 预警（防止热更新/跳过时残留全屏遮罩）
        var warnOverlay = document.getElementById('healthCountdownOverlay');
        if (warnOverlay && warnOverlay.parentNode) warnOverlay.parentNode.removeChild(warnOverlay);
        if (self._warnTimer) { clearInterval(self._warnTimer); self._warnTimer = null; }
        self._isCountingDown = false;
