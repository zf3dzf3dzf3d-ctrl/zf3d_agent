...
Object.assign(App, {
        ...
        toggleToolPanel: function(box) {
            ...
            if (tp.classList.contains('open')) {
                ...
                // 修复：关闭面板时保留徽章数字（之前重置为 0 并隐藏，导致数字消失）
                var _badgeClose = btn ? btn.querySelector('.tool-badge') : null;
                var _cntClose = _badgeClose ? _badgeClose.textContent : '0';
                if (btn) btn.innerHTML = '🔧<span class="tool-badge"' + (_cntClose && _cntClose !== '0' ? '' : ' style="display:none"') + '>' + (_cntClose || '0') + '</span>';
            } else {
                ...
                // 修复：打开面板时保留徽章数字（之前重置为 0 并隐藏，导致点开再回来数字消失）
                var _badgeOpen = btn ? btn.querySelector('.tool-badge') : null;
                var _cntOpen = _badgeOpen ? _badgeOpen.textContent : '0';
                if (btn) btn.innerHTML = '💬<span class="tool-badge"' + (_cntOpen && _cntOpen !== '0' ? '' : ' style="display:none"') + '>' + (_cntOpen || '0') + '</span>';
