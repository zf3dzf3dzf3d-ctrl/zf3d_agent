...
Object.assign(App, {
        ...
        addMsg: function(box, text, who, modelId, isFinal, skipSave) {
            ...
            try {
                if (body.childNodes.length > 120) {
                    var _removed = 0;
                    var _node = body.firstChild;
                    while (body.childNodes.length > 80 && _removed < 60 && _node) {
                        var _next = _node.nextSibling;
                        if (_node.classList && (_node.classList.contains('ai-final') || _node.classList.contains('compress-selector'))) {
                            _node = _next; // 保护节点：跳过不删，继续裁后面的
                            continue;
                        }
        ...
        closeChatBox: function(chat) {
            ...
            this.updateStatus();
            this.updateMinimap();
            // 【赛道释放】对话已关闭 → 掐断该对话在池中的未完成 Turn，
            // 避免赛道（lanes_health）上残留"幽灵车"，直到请求自然结束才放行
            try {
                fetch((typeof DB !== 'undefined' ? DB.BASE_URL : '') + '/api/chat-pool/slot/' + encodeURIComponent(chat.id) + '/cancel', {
                    method: 'POST',
