...
Object.assign(App, {
        ...
        addMsg: function(box, text, who, modelId, isFinal, skipSave) {
            ...
            // 消息+工具卡片超过 300 条时，移除最旧的非 AI 最终回复节点（最终答案始终保留）。
            try {
                if (body.childNodes.length > 300) {
                    var _removed = 0;
                    while (body.childNodes.length > 260 && _removed < 50) {
                        var _oldest = body.firstChild;
                        if (!_oldest) break;
                        // 不删 AI 最终回复和压缩选择器，只裁剪普通消息/工具卡片
                        if (_oldest.classList && (_oldest.classList.contains('ai-final') || _oldest.classList.contains('compress-selector'))) {
                            // 跳过保护节点：把它移到列表尾部前的位置不现实，直接停止更安全
                            break;
                        }
                        if (_oldest._typingInterval) clearInterval(_oldest._typingInterval);
                        body.removeChild(_oldest);
                        _removed++;
                    }
                }
            } catch (e) {}
            ...
            if (_dgChat) _dgChat._dogGuardLastActivity = Date.now();
        ...
        restoreLastClosed: function() {
            ...
            // 恢复对话：分类下拉按该对话引擎过滤
            var _catList = Tools.getCategoryList(snapshot.id, snapshot._engine || snapshot.engine || '');
            var _catHtml = '';
