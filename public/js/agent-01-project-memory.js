...
Object.assign(App, {
        ...
        try {
            ...
            (function() {
                ...
                var actions = [
                    ...
                    { id: 'obj-verify', name: '客观验证', tip: '客观验证：新开一个对话，附带前因后果、工具上下文、修改文件的前后对比（git diff）素材包，让新对话快速客观评价结果并修复 bug' },
                ...
                actions.forEach(function(o) {
                    ...
                    btn.onclick = function() {
                        if (o.id === 'undo-step') {
                            ...
                            // 造成"点了客观验证，原对话看似空闲却卡住/状态错乱"的怪象。
                            // 客观验证本身开新对话独立运行，不需要原对话先收尾。
                            try { if (!chat.isSending && self._onSendComplete) self._onSendComplete(box, chat); } catch (e) {}
                            self._objectiveVerify(box, chat);
                        } else if (o.id === 'verify') {
                            ...
                            if (_lastAssistant) {
                                // 【2026 修复】与客观验证一致：仅空闲时才强制收尾，运行中不破坏发送状态
                                try { if (!chat.isSending && self._onSendComplete) self._onSendComplete(box, chat); } catch (e) {}
    ...
    // ===== 客观验证：新开对话 + 注入验证素材包（前因后果/工具上下文/改动前后对比） =====
    _objectiveVerify: function(box, chat) {
        var self = this;
        // 【2026 修复】客观验证基于新对话独立运行，原对话运行中/验证轮中都不再拦截。
        // 原逻辑 chat.isSending || chat._verifyActive 直接 return，导致：
        // 1) 正在跑任务时点客观验证被拒（用户希望"随时随地可验证"）；
        // 2) 上轮验证结束后 _verifyActive 残留为 true 时，明明空闲却误报"正在运行"。
        // 现在仅在验证轮进行中给出提示（避免素材包取到半截上下文），但不阻断。
        if (chat._verifyActive && !chat.isSending) {
            Store.addLog('warn', chat.id, 'obj-verify', '上一轮验证尚未结束，本次客观验证以上下文可能不完整');
            self._stepToast('上一轮验证尚未结束，客观验证素材包可能不完整，已照常发起', false);
        } else if (chat.isSending) {
            Store.addLog('info', chat.id, 'obj-verify', '当前对话运行中发起客观验证（新对话独立运行）');
            self._stepToast('当前对话仍在运行，客观验证已在新对话独立发起', true);
        }
        // 1. 收集原对话上下文：真实用户问题 + 最近 AI 回复 + 关键工具调用摘要（取自运行日志，最可靠）
        ...
        try {
            ...
            for (var _li = _logs.length - 1; _li >= 0 && _toolCalls.length < 15; _li--) {
                if (_logs[_li].type === 'tool_call') _toolCalls.unshift(String(_logs[_li].content || '').substring(0, 120));
            }
        } catch (e) {}
        for (var _i = chat.history.length - 1; _i >= 0; _i--) {
            var _h = chat.history[_i];
            if (!_h) continue;
            if (_h.role === 'assistant' && !_h._meta && !_lastAssistant) _lastAssistant = _h.content || '';
            if (_h.role === 'user' && !_h._guardInject && !_h._verifyRound && !_h._continueRound && !_lastRealUserQ) _lastRealUserQ = _h.content || '';
        }
        ...
        try {
            ...
            xhr.onload = function() {
                ...
                self._objectiveVerifySend(box, chat, {
                    ...
                    toolCalls: _toolCalls, brief: res
                });
            };
            xhr.onerror = function() {
                // 素材包失败也不阻塞：降级为仅上下文验证
                self._objectiveVerifySend(box, chat, { projPath: _projPath, userQ: _lastRealUserQ, aiReply: _lastAssistant, toolCalls: _toolCalls, brief: { ok: false } });
            };
    ...
    _objectiveVerifySend: function(box, chat, ctxData) {
        ...
        var lines = [];
        lines.push('【客观验证任务】你是一个独立的复核者，请基于下面给出的客观素材，对一个已完成的任务做出快速、客观的评价，并指出可能的 bug 与修复方法。');
        lines.push('');
        lines.push('== 前因后果 ==');
        lines.push('项目路径：' + (ctxData.projPath || '（未提供）'));
        lines.push('用户原始问题：' + String(ctxData.userQ || '').replace(/\s+/g, ' ').trim().slice(0, 800));
        lines.push('');
        lines.push('== 执行 AI 的最终答复（摘要） ==');
        lines.push(String(ctxData.aiReply || '').slice(0, 3000));
        ...
        lines.push('3. 给出结论：✅ 通过 / ⚠️ 有问题（列出问题点与修复建议）。只做评价与指出问题，不要大改文件。');
        ...
        if (!selfApp || typeof selfApp.createChatBox !== 'function') {
            Store.addLog('error', chat.id, 'obj-verify', '客观验证失败：无法创建新对话');
            self._stepToast('客观验证失败：无法创建新对话', false);
            return;
        }
        ...
        var newChat = selfApp.createChatBox(cx + 60, cy + 40, chat.modelId || null);
        if (!newChat) {
            self._stepToast('客观验证失败：新对话创建被拦截（窗口数量/频率限制）', false);
            return;
        }
        // 继承项目关联
        if (chat.projectId) {
            newChat.projectId = chat.projectId;
            for (var i = 0; i < Store.data.chatBoxes.length; i++) {
                if (Store.data.chatBoxes[i].id === newChat.id) { Store.data.chatBoxes[i].projectId = chat.projectId; break; }
            }
        }
        Store.addLog('info', chat.id, 'obj-verify', '客观验证已发起：新对话 ' + newChat.id + '，素材包 ' + _prompt.length + ' 字');
        self._stepToast('客观验证已发起：新对话已创建并发送验证素材包', true);
        setTimeout(function() {
            try {
                if (typeof selfApp.addMsg === 'function') selfApp.addMsg(newChat.el, '（客观验证素材包，共 ' + _prompt.length + ' 字）', 'user', newChat.modelId);
            ...
            try {
                ...
                if (_prevTools.length) {
                    ...
                    for (var _bi = _prevTools.length - 1; _bi >= 0 && _briefTotal < 400; _bi--) {
                        var _bc = String(_prevTools[_bi].content || '').replace(/\s+/g, ' ').trim();
