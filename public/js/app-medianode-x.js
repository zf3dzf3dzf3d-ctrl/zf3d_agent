...
(function () {
    ...
    function restoreItems(items) {
        ...
        // 恢复期间暂停 observer 触发的保存，防止「恢复→保存→恢复」请求风暴
        var wasRestoring = window.MRef && window.MRef._setRestoring;
        if (wasRestoring) window.MRef._setRestoring(true);
        try {
            ...
            if (wasRestoring) setTimeout(function () { window.MRef._setRestoring(false); }, 300);
        }
    }
    ...
    function init() {
        ...
        // 任何 media 节点变化后自动保存
        // 【防风暴】只监听 media 节点的直接子树，忽略页面上其他元素（对话框拖动等）的 style 变化；
        // 并且用「哨兵标志」跳过恢复阶段产生的变更，避免 恢复→保存→再恢复 循环请求
        var host = document.getElementById('canvasContent');
        if (host) {
            var _mrefRestoring = false;
            window.MRef = window.MRef || {};
            window.MRef._setRestoring = function (v) { _mrefRestoring = !!v; };
            var mo = new MutationObserver(function (muts) {
                if (_mrefRestoring) return;
                // 过滤：只关心 media 节点相关变更
                    ...
                    if (m.type === 'childList') {
                        if (m.addedNodes && m.addedNodes.length &&
                            Array.prototype.some.call(m.addedNodes, function (n) {
                                return n.classList && n.classList.contains('media-canvas-node');
                            })) { saveSoon(); return; }
                        if (m.removedNodes && m.removedNodes.length &&
                            Array.prototype.some.call(m.removedNodes, function (n) {
                               ...
                               m.target.classList && m.target.classList.contains('media-canvas-node')) {
                        saveSoon(); return;
                    }
                }
            });
    ...
    // 暴露 API（保留 init 中可能已注入的 _setRestoring 等内部标志）
    window.MRef = Object.assign({}, window.MRef, {
        selected: selected,
        clearSel: clearSel,
