...
(function (global) {
  ...
  function remove(id) {
    ...
    if (wasDefault && list.length) {
      // 在同类型中补一个默认；找不到则补 list[0]
      var sub = list.find(function(m) { return defaultTypeOf(m) === t; }) || list[0];
      sub.isDefault = true;
  ...
  function getDefaultFor(type) {
    ...
    var t = _norm(type);
    // 优先 enabled 且有 endpoint 的默认模型；否则退化为该类型任意可用的 visionInput 模型
    var primary = list.find(function(x) { return defaultTypeOf(x) === t && x.isDefault && x.enabled !== false && x.endpoint; });
    if (primary) return primary;
    return list.find(function(x) { return defaultTypeOf(x) === t && x.enabled !== false && x.endpoint; }) || null;
  }
