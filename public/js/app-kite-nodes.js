...
(function () {
  ...
  // removeNode 定义在后加载的 app-kite-links.js 末尾才写回 NS；
  // 不能在加载期捕获（捕获到的永远是 undefined，会导致所有节点关闭/删除按钮点击报错失效），
  // 必须运行时经 NS 取用
  function removeNode(id) { return NS.removeNode ? NS.removeNode(id) : null; }
  ...
  // updateCurvesToNearestChat 在后加载的 app-kite-links.js 中定义，运行时经 NS 取用，不可在加载期捕获
...
// addNode, applyMediaNodeUrl, bindNodeEvents, bindPanelWindow, bindTextOutput, closeActionMenu, createNodeLink, escapeHtml, handleImagePanelResult, node_ports_setup, openImagePanel, refreshNodeLink, refreshNodeLinksOf, showActionMenu, updateImageNode, updateVideoNode
  ...
  // ---------- 自动落点：优先放在来源对话右侧（多个结果依序错开），避免离对话太远/互相叠压 ----------
  // 坐标系：canvasContent 局部坐标（节点 left/top 直接使用）
  function pickAutoPos(w, h, preferChatId) {
    const canvasEl = state.canvas || document.getElementById('canvasContent');
    const hr = (canvasEl || document.body).getBoundingClientRect();
    const vw = window.innerWidth, vh = window.innerHeight;
    // 依序错开：按已有节点数轮转偏移，避免同一落点叠压
    const seq = state.nodes.size;
    const dx = (seq % 4) * 28, dy = (seq % 6) * 36;

    // 优先锚定指定对话；否则取视口中心最近的可见对话（视为"当前"对话）
    let rect = null;
    if (preferChatId) {
      const el = document.getElementById(preferChatId);
      if (el && el.classList && el.classList.contains('chatbox') && el.offsetParent !== null) rect = el.getBoundingClientRect();
    }
    if (!rect) {
      let bestD = Infinity;
      document.querySelectorAll('.chatbox').forEach(c => {
        if (c.offsetParent === null) return;
        const r = c.getBoundingClientRect();
        const d = Math.hypot(r.left + r.width / 2 - vw / 2, r.top + r.height / 2 - vh / 2);
        if (d < bestD) { bestD = d; rect = r; }
      });
    }

    if (!rect) {
      // 无可见对话兜底：视口中央
      return { x: Math.round((vw / 2 - w / 2) - hr.left + dx), y: Math.round(Math.max(8, (vh / 2 - h / 2) - hr.top + dy)) };
    }

    // 首选：对话右侧，顶部对齐
    let x = rect.right - hr.left + 48 + dx;
    let y = rect.top - hr.top + 8 + dy;
    // 右侧放不下 → 对话左侧
    if (hr.left + x + w > vw - 24) {
      x = rect.left - hr.left - 48 - w - dx;
      y = rect.top - hr.top + 8 + dy;
    }
    // 左侧也放不下 → 对话下方居中
    if (hr.left + x < 24) {
      x = rect.left + rect.width / 2 - hr.left - w / 2 + dx;
      y = rect.bottom - hr.top + 24 + dy;
    }
    // 纵向越界贴底
    if (hr.top + y + h > vh - 16) y = (vh - 16 - h) - hr.top;
    if (y < 8) y = 8;
  ...
  function addNode(data) {
    ...
    // 未显式指定坐标时自动落点：来源对话右侧依序排布（修复：原先固定屏幕中心，离对话太远且互相叠压）
    let x = data.x, y = data.y;
    if (x == null || y == null) {
      const pos = pickAutoPos(sizeW, sizeH, data.chatId);
      if (x == null) x = pos.x;
      if (y == null) y = pos.y;
    }
  ...
  function updateVideoNode(id, data) {
    ...
    NS.updateCurvesToNearestChat(node);
    return node;
  }
  ...
  function bindNodeEvents(node) {
    ...
    if (deleteButton) deleteButton.addEventListener('click', (e) => { e.stopPropagation(); if (window.SFX) SFX.play('trash'); removeNode(node.id); });
    ...
    document.addEventListener('mouseup', () => {
      ...
      if (drag.mode === 'move' && (Math.abs(node.x - drag.ox) > 3 || Math.abs(node.y - drag.oy) > 3) && window.SFX) SFX.play('drop');
