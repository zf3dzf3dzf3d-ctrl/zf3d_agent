...
(function () {
    ...
    function saveImageNodes() {
        ...
        })).filter(function (asset) { return asset.imageUrl && asset.imageUrl.indexOf('blob:') !== 0; }); // blob: 临时地址刷新后失效，不持久化
    ...
    function loadImageNodes() {
        ...
        assets.filter(function (asset) { return asset.imageUrl && asset.imageUrl.indexOf('blob:') !== 0; }).forEach(asset => { // 跳过失效的 blob: 旧数据
