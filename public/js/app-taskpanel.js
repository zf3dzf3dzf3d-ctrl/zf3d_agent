...
Object.assign(App, {
    ...
    _initTaskPanel: function() {
        ...
        // 任务/对话按钮不单独绑定：统一由下方 document 委托处理（.zf-dock-tab，含 toggle 开关逻辑），避免双重监听竞态（先开后关）

        // 绑定 Tab 切换（事件委托：底栏/面板内 .tp-tab 统一生效）
        document.addEventListener('click', function(e) {
            var tab = e.target.closest ? e.target.closest('.tp-tab') : null;
            if (tab && !tab.classList.contains('task-panel-close')) {
                // 底栏 dock 按钮由下方 .zf-dock-tab 委托统一处理（含 toggle 判断），
                // 这里只处理面板内部的 tab，避免先 switch 后被误判为 toggle 而直接关闭面板
                if (tab.classList.contains('zf-dock-tab')) return;
                self._switchTab(tab.getAttribute('data-tab'));
            }
        });
        // 底栏全部栏目按钮：统一事件委托 —— 点击打开对应栏目；再次点击当前栏目则关闭面板
        document.addEventListener('click', function(e) {
            var b = e.target.closest ? e.target.closest('.zf-dock-tab') : null;
            if (!b) return;
            var tab = b.getAttribute('data-tab');
            if (!tab) return;
            var panel = document.getElementById('taskPanel');
            var isOpen = panel && panel.classList.contains('open');
            if (isOpen && self._taskActiveTab === tab) {
                self.closeTaskPanel(); // 再次点击当前栏目 = 关闭侧边栏
            } else {
                self._switchTab(tab);
                self.openTaskPanel();
            }
    ...
    _initTaskPanelResize: function() {
        ...
        if (!Number.isFinite(offset)) offset = 290; // 默认宽度 360+290=650px（加宽：容纳长任务/超长计划/历史面板）
    ...
    _switchTab: function(tab) {
        ...
        // 极简底栏：高亮当前栏目 + 头部显示栏目名
        var nameMap = { chat: '对话', task: '任务', longplan: '长任务', goal: '目标', memory: '记忆', history: '历史', model: '模型' };
        var nameEl = document.getElementById('tpActiveTabName');
        if (nameEl) nameEl.textContent = nameMap[tab] || tab;
        document.querySelectorAll('.zf-dock-tab').forEach(function(t) {
            if (t.getAttribute('data-tab') === tab) t.classList.add('zf-dock-active');
            else t.classList.remove('zf-dock-active');
        });
        ...
        var goalBody = document.getElementById('goalPanelBody');
        var memoryBody = document.getElementById('memoryPanelBody');
        var searchBar = document.getElementById('chatSearchBar');
        // 通用：默认隐藏 goal 面板，只有 goal Tab 再打开（避免各分支重复写）
        var showGoal = false;
        if (tab === 'chat') {
            ...
            if (taskBody) taskBody.style.display = 'none';
            var lpHideG = document.getElementById('longPlanPanelBody');
            if (lpHideG) lpHideG.style.display = 'none';
            if (chatBody) chatBody.style.display = 'none';
            if (historyBody) historyBody.style.display = 'none';
            if (modelBody) modelBody.style.display = 'none';
            if (searchBar) searchBar.style.display = 'none';
            if (goalBody) goalBody.style.display = '';
            showGoal = true;
            if (this._loadGoalPanel) {
                this._loadGoalPanel();
            } else {
                console.warn('[TaskPanel] _loadGoalPanel 尚未就绪（app-goal-panel.js 未加载完成）');
            }
        } else if (tab === 'memory') {
            if (taskBody) taskBody.style.display = 'none';
            var lpHideM = document.getElementById('longPlanPanelBody');
            if (lpHideM) lpHideM.style.display = 'none';
            if (chatBody) chatBody.style.display = 'none';
            if (historyBody) historyBody.style.display = 'none';
            if (modelBody) modelBody.style.display = 'none';
            if (searchBar) searchBar.style.display = 'none';
            if (memoryBody) memoryBody.style.display = '';
            if (this._loadMemoryPanel) {
                this._loadMemoryPanel();
            } else {
                console.warn('[TaskPanel] _loadMemoryPanel 尚未就绪（app-memory-panel.js 未加载完成）');
            }
        } else if (tab === 'model') {
            if (taskBody) taskBody.style.display = 'none';
        ...
        // 通用收尾：非 goal/memory Tab 一律隐藏 goal 与 memory 面板
        if (goalBody && !showGoal) goalBody.style.display = 'none';
        if (memoryBody && tab !== 'memory') memoryBody.style.display = 'none';
    },
    ...
    closeTaskPanel: function() {
        ...
        document.querySelectorAll('.zf-dock-tab.zf-dock-active').forEach(function(b){ b.classList.remove('zf-dock-active'); });
        document.querySelectorAll('.tp-tab.active').forEach(function(b){ b.classList.remove('active'); });
    },
    ...
    _updateTaskButton: function() {
        ...
        // 任务按钮不显示任何进度/百分比/完成态，外观与其他底栏按钮一致
        btn.style.opacity = this._taskHasLists && this._taskLists.length > 0 ? '1' : '0.5';
    },
