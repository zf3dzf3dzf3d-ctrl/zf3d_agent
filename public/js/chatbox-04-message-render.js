...
Object.assign(App, {
        ...
        setMsgContent: function(div, text, who) {
            if (who === 'ai' || who === 'ai-final') {
                ...
                vaBtn.addEventListener('click', function(e) {
                    ...
                    if (finalEl) {
                        // 往上多留 90px，确保能看到答案开头
                        // 注意：offsetTop 相对 offsetParent 而非滚动容器，会滚偏；
                        // 用 getBoundingClientRect 换算出相对 .chatbox-body 的真实位置。
                        // 【2026-09 修复】定位前先等一帧并强制目标完成布局（读 offsetHeight
                        // 触发同步 reflow），防止目标在视口外/刚插入时高度为估算值，
                        // 导致定位到最后一个答案时滚动位置偏移。
                        requestAnimationFrame(function() {
                            try { void finalEl.offsetHeight; void body.scrollHeight; } catch (e) {}
                            var bTop = body.getBoundingClientRect().top;
                            var targetTop = finalEl.getBoundingClientRect().top - bTop + body.scrollTop - 90;
        ...
        // 注意：普通 AI 回复只有 'ai' 类，'ai-final' 只加在整个对话最后一条总结上，
        // 所以不能只认 ai-final，否则会滚到最后一条答案去。
        // 正确做法：向下遍历到下一条用户消息为止，返回其中最后一条 AI 回复。
        _findFinalAnswer: function(userDiv) {
            var node = userDiv.nextElementSibling;
            var lastAi = null;
            while (node) {
                if (node.classList && node.classList.contains('msg')) {
                    if (node.classList.contains('user')) break; // 到下一条提问，结束
                    if (node.classList.contains('query-reminder')) break;
                    if (node.classList.contains('ai') || node.classList.contains('ai-final') || node.classList.contains('error')) {
                        lastAi = node; // 记住最后一条 AI 回复（跳过中间的工具/typing消息）
                    }
                }
        ...
        _showQuestionsPanel: function(box, titleEl, pinned) {
            ...
            if (!users.length) {
                // 兜底：消息 DOM 尚未恢复时，从持久化 history 读取用户提问，保证标题悬停面板不空白
                var chatRef = (self.chatBoxes || []).filter(function(c) { return c.el === box || (c.el && c.el.contains(box)); })[0]
                           || (self.chatBoxes || []).filter(function(c) { return box.__chatId && c.id === box.__chatId; })[0];
                var hist = chatRef && chatRef.history || [];
                var qs = hist.filter(function(h) { return h.role === 'user' && String(h.content || '').trim(); });
                if (!qs.length) return;
                users = qs.map(function(h) {
                    var fake = document.createElement('div');
                    fake.className = 'msg user';
                    fake.dataset.originalText = h.content;
                    return fake;
                });
            }
        ...
        _scrollToMsg: function(body, msgEl, extraOffset) {
            ...
            // offsetTop 相对 offsetParent 而非滚动容器，嵌套时会滚偏；
            // 用 getBoundingClientRect 换算出相对 .chatbox-body 的真实位置
            var bTop = body.getBoundingClientRect().top;
            var targetTop = msgEl.getBoundingClientRect().top - bTop + body.scrollTop - 40 - extra;
            if (targetTop < 0) targetTop = 0;
            // 钳制到可滚动范围，保证最后一条答案也能完整聚焦到底
            var maxTop = body.scrollHeight - body.clientHeight;
            if (targetTop > maxTop) targetTop = maxTop;
            if (body.scrollTo) {
                body.scrollTo({ top: targetTop, behavior: 'smooth' });
