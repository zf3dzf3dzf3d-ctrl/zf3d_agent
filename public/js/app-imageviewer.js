...
(function () {
    ...
    let savedGeom = null;
    // ===== 幻灯片模式 =====
    let slideList = [];       // [{url, name}] 当前幻灯片图片列表
    let slideIndex = -1;      // 当前索引
    let slideTimer = null;    // 自动播放定时器
    const SLIDE_KEY = 'zf3d_imageviewer_slide_interval';
    ...
    function build() {
        ...
        panel.innerHTML =
            ...
            '<div class="iv-footer">' +
                ...
                '<span class="iv-slide-nav" style="display:none">' +
                    '<button type="button" class="iv-btn" data-act="prev" title="上一张 (←)">◀</button>' +
                    '<button type="button" class="iv-btn" data-act="play" title="自动播放/暂停">▶</button>' +
                    '<button type="button" class="iv-btn" data-act="next" title="下一张 (→)">▶|</button>' +
                    '<span class="iv-slide-pos" style="font-size:12px;color:var(--text2);min-width:56px;text-align:center;">0/0</span>' +
                '</span>' +
        ...
        // --- 幻灯片工具条 ---
        panel.querySelector('[data-act="prev"]').addEventListener('click', (e) => { e.stopPropagation(); slidePrev(); });
        panel.querySelector('[data-act="next"]').addEventListener('click', (e) => { e.stopPropagation(); slideNext(); });
        panel.querySelector('[data-act="play"]').addEventListener('click', (e) => { e.stopPropagation(); toggleSlidePlay(); });
    ...
    function isVideoUrl(u) {
        ...
        // 先看整体扩展名；若是 /api/fs/file?path=xxx 形式，再取 path 参数里的扩展名
        if (/\.(mp4|webm|ogv|ogg|mov|m4v|avi|mkv)$/.test(clean)) return true;
        const m = clean.match(/[?&]path=([^&]*)/);
        if (m) {
            let p = '';
            try { p = decodeURIComponent(m[1]); } catch (e) { p = m[1]; }
            return /\.(mp4|webm|ogv|ogg|mov|m4v|avi|mkv)$/.test(p.toLowerCase());
        }
        return false;
    }
    ...
    // ===== 幻灯片模式 =====
    function slideNavEl() { return panel ? panel.querySelector('.iv-slide-nav') : null; }

    function updateSlideBar() {
        var nav = slideNavEl();
        if (!nav) return;
        var has = slideList.length > 1;
        nav.style.display = has ? 'inline-flex' : 'none';
        if (!has) return;
        var pos = panel.querySelector('.iv-slide-pos');
        if (pos) pos.textContent = (slideIndex + 1) + '/' + slideList.length;
        var playBtn = panel.querySelector('[data-act="play"]');
        if (playBtn) { playBtn.textContent = slideTimer ? '⏸' : '▶'; playBtn.title = slideTimer ? '暂停自动播放' : '自动播放'; }
    }

    function stopSlidePlay() {
    ...
    function startSlidePlay() {
        ...
        slideTimer = setInterval(function () { slideNext(true); }, slideIntervalMs);
        updateSlideBar();
    }

    function toggleSlidePlay() {
        if (slideTimer) stopSlidePlay(); else startSlidePlay();
    }

    function slideGo(idx) {
        if (!slideList.length) return;
        slideIndex = ((idx % slideList.length) + slideList.length) % slideList.length;
        var item = slideList[slideIndex];
        show(item.url);
        var title = panel.querySelector('.iv-title');
        if (title && item.name) title.textContent = '🖼 ' + item.name + '（' + (slideIndex + 1) + '/' + slideList.length + '）';
        updateSlideBar();
    }

    function slideNext(keepPlay) { if (!keepPlay && slideTimer) { /* 手动翻页不停止播放 */ } slideGo(slideIndex + 1); }
    function slidePrev() { slideGo(slideIndex - 1); }

    // 设置幻灯片列表（由缩略图面板双击图片时传入）
    function setSlideList(list, startIndex) {
        slideList = Array.isArray(list) ? list.slice() : [];
        slideIndex = typeof startIndex === 'number' && slideList.length ? ((startIndex % slideList.length) + slideList.length) % slideList.length : -1;
        updateSlideBar();
    }

    // 方向键翻页（仅查看器打开且有多图时生效）
    window.addEventListener('keydown', (e) => {
        if (!isOpen() || slideList.length < 2) return;
