    ...
    contextMaster: function(srcBox) {
        ...
        try {
            ...
            if (parsed && parsed.messages) {
                ...
                lines.push('');
                // 消息序列（分层紧凑版：assistant 只留 tool_calls 调用名，tool 只留前 150 字）
                lines.push('【消息序列】(紧凑视图: tool 单条截150字, assistant全量保留)');
                msgs.forEach(function(m, i) {
                    ...
                    var cap = 800;
                    if (m.role === 'tool') cap = 150;
                    if (m.tool_calls && m.tool_calls.length) {
                        try { t += '\n[tool_calls] ' + m.tool_calls.map(function(tc){ return (tc.function && tc.function.name) || '?'; }).join(', '); } catch (e) {}
                    }
                    if (t.length > cap) t = t.slice(0, cap) + '…[截断]';
