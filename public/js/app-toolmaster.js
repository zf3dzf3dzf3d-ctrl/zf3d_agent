    ...
    toolMaster: function(srcBox) {
        ...
        try {
            ...
            try {
                ...
                // 【防膨胀护栏】单份报告最大 200KB，超出截断正文
                var lastKey = keys[keys.length - 1];
                if (lastKey && pend[lastKey] && pend[lastKey].length > 200 * 1024) pend[lastKey] = pend[lastKey].slice(0, 200 * 1024) + '\n…(报告过长已截断)';
                // 【防膨胀护栏】总暂存超过 300KB 时清空最旧的全部，只留当前
                try { if (JSON.stringify(pend).length > 300 * 1024) { pend = {}; pend['chat_' + Date.now()] = (''+report).slice(0, 200*1024); } } catch (e2) {}
