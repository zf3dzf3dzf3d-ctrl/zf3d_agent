    ...
    Object.assign(App, {
        ...
        restoreSession: function() {
            ...
            saved.chatBoxes.forEach(function(sb) {
                ...
                // own_tools 引擎：常规分类（极简/编程/写作/流程图等）是朱峰社区底层独有，
                ...
                var rCatList = Tools.getCategoryList(sb.id, sb._engine || sb.engine || '');
                var rCatHtml = '';
                ...
                '<div class="chatbox-header">' +
                    ...
                    '<span class="title">' + ((sb.title && String(sb.title).indexOf('💬 对话') !== 0) ? sb.title : '💬 对话' + self.chatCounter) + '</span>' +
                ...
                '<button class="prev-user-btn" title="定位到上一条用户问题所在的段落"><span>▲</span></button> <button class="scroll-bottom-btn" title="滚动到底部"><span>▼</span></button>' +
                    ...
                    '<div class="eng-picker-wrap">' +
                        '<button class="eng-trigger" title="底层对话引擎（对话处理管线）">' +
                            ...
                            '<span class="eng-name">' + (function(){ var e0=(typeof DB!=='undefined'&&DB.getEngines)?DB.getEngines():[]; var c0=(typeof restoredChat!=='undefined'&&restoredChat&&restoredChat._engine)?restoredChat._engine:(typeof DB!=='undefined'?DB._engine:''); var m0=e0.filter(function(x){return x.id===c0;})[0]; return m0?((m0.icon?m0.icon+' ':'')+m0.name):'默认'; })() + '</span>' +
                            '<span class="eng-arrow">▾</span>' +
                        '</button>' +
                        '<div class="eng-menu" hidden>' + (function(){ var e0=(typeof DB!=='undefined'&&DB.getEngines)?DB.getEngines():[]; var c0=(typeof restoredChat!=='undefined'&&restoredChat&&restoredChat._engine)?restoredChat._engine:(typeof DB!=='undefined'?DB._engine:''); var h0=''; e0.forEach(function(en){ h0+='<div class="eng-item'+(en.id===c0?' active':'')+'" data-eng="'+en.id+'" title="'+(en.description||'').replace(/"/g,'&quot;')+'"><span>'+(en.icon?en.icon+' ':'')+en.name+'</span>'+(en.default?'<span style="opacity:.6;font-size:10px;">默认</span>':'')+'</div>'; }); return h0; })() + '</div>' +
                    '</div>' +
