...
// removeNode, updateCurvesToNearestChat
  ...
  function updateCurvesToNearestChat(node) {
    // 提示词等文本节点默认不自动连线（手动从小圆点拖出才连）；视频节点按要求也不自动连线；仅图片等媒体节点自动连最近对话
    if (node && (node.type === 'text' || node.type === 'video')) return;
  ...
  window.KiteCanvas = {
    ...
    addVideoNode(opts) {
      ...
      // 用户要求：生成视频后不自动连线，不传 chatId
      return addNode({ type: 'video', src: url, prompt: opts.prompt || '' });
    },
    ...
    addImageNode(opts) {
      ...
      // 用户要求：生成图片后不自动连线，不传 chatId
      return addNode({ type: 'image', src: url, prompt: opts.prompt || '' });
    },
    // 显式更新图片节点
    updateImageNode,
    clear() {
