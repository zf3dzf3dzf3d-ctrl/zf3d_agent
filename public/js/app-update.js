...
(function () {
  ...
  function wrapFetch() {
    ...
    var failCount = 0;            // 连续失败计数
    var circuitOpenUntil = 0;     // 熔断截止时间戳
    var CIRCUIT_THRESHOLD = 5;    // 连续失败多少次触发熔断
    var CIRCUIT_PAUSE_MS = 8000;  // 熔断暂停时长（期间请求直接快速失败，不占用连接）
    window.fetch = function () {
      var args = arguments;
      // 熔断中：直接返回失败，不发出真实网络请求，防止雪崩堆积
      if (Date.now() < circuitOpenUntil) {
        return Promise.reject(new TypeError('circuit open (server unreachable)'));
      }
      return rawFetch.apply(window, args).then(
        function (resp) { failCount = 0; activeRequests--; return resp; },
        function (err) {
          activeRequests--;
          // 只对网络层错误计数（服务器无响应），HTTP 4xx/5xx 不算
          if (err instanceof TypeError && !String(err && err.message).includes('circuit open')) {
            failCount++;
            if (failCount >= CIRCUIT_THRESHOLD) {
              circuitOpenUntil = Date.now() + CIRCUIT_PAUSE_MS;
              failCount = 0;
              console.warn('[Guard] 连续请求失败，熔断 ' + (CIRCUIT_PAUSE_MS / 1000) + 's 防止请求雪崩');
            }
          }
          throw err;
        }
