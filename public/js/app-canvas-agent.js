/* 画布特效省电模式：默认开启（隐藏发光连线/脉冲动画）。
   localStorage.setItem('canvasFxLite','0') 后刷新可恢复完整特效 */
(function(){try{if(localStorage.getItem('canvasFxLite')!=='0'){var a=function(){document.body&&document.body.classList.add('fx-lite')};if(document.readyState!=='loading')a();else document.addEventListener('DOMContentLoaded',a);}}catch(e){}})();
(function(){var s=document.createElement('style');s.textContent="body.fx-lite .fg-edge-glow{display:none!important}"+
"body.fx-lite .ca-running,body.fx-lite .ca-blocked{animation:none!important}"+
"body.fx-lite .fg-node{animation:none!important;box-shadow:none!important}";document.head.appendChild(s);})();
/* ============================================================
...
(function () {
  ...
  CA.createNode = function (opts) {
    ...
    // 【性能修复】不在插入后立即读 offsetWidth/offsetHeight（强制重排），
    // 先用布局默认尺寸占位，后续拖拽/重画时按真实尺寸校正
    layer._fgNodes[key] = { el: el, w: 190, h: 60, _caNew: true, _caPrompt: opts.prompt || '' };
    ...
    // 【性能修复】异步批量校正：等浏览器完成布局后，一次性读取所有节点真实尺寸，
    // 再统一更新 w/h 并重画连线端点（读写分离，只触发 1 次强制重排）
    setTimeout(function () {
      try {
        var keys = Object.keys(layer._fgNodes);
        var ws = [], hs = [];
        keys.forEach(function (k) { ws.push(layer._fgNodes[k].el.offsetWidth); hs.push(layer._fgNodes[k].el.offsetHeight); });
        keys.forEach(function (k, i) { layer._fgNodes[k].w = ws[i]; layer._fgNodes[k].h = hs[i]; });
        (layer._fgEdgeRefs || []).forEach(function (r) {
          var a = layer._fgNodes[r.from], b = layer._fgNodes[r.to];
          if (!a || !b) return;
          var x1 = parseFloat(a.el.style.left) + (a.w || 190), y1 = parseFloat(a.el.style.top) + (a.h || 60) / 2;
          var x2 = parseFloat(b.el.style.left), y2 = parseFloat(b.el.style.top) + (b.h || 60) / 2;
          var cx = (x1 + x2) / 2;
          var d = 'M ' + x1 + ' ' + y1 + ' C ' + cx + ' ' + y1 + ', ' + cx + ' ' + y2 + ', ' + x2 + ' ' + y2;
          r.base.setAttribute('d', d);
