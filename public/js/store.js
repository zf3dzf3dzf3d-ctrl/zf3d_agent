...
var Store = {
    ...
    flush: function() {
        ...
        // 【修复：最后一条消息重启后丢失】流式打字动画进行中（未落库）的消息，
        // 在页面关闭前用 sendBeacon 立即补写，避免动画未完成就关页导致消息丢失
        if (this.dbOnline && typeof DB !== 'undefined' && this.data && this.data.chatBoxes && typeof App !== 'undefined' && App.chatBoxes) {
            this.data.chatBoxes.forEach(function(box) {
                var liveChat = App.chatBoxes.find(function(c) { return c.id === box.id; });
                if (liveChat && liveChat._pendingStreamSave && box.id) {
                    var p = liveChat._pendingStreamSave;
                    try {
                        var url = (DB.BASE_URL || '') + DB.API_PREFIX + '/chat/' + encodeURIComponent(box.id);
                        var payload = JSON.stringify({ role: p.role, content: p.content, modelId: p.modelId || '', parentId: null });
                        var sent = false;
                        ...
                        if (!sent) {
                            DB.addChatMessage(box.id, p.role, p.content, p.modelId, null).catch(function() {});
                        }
                        liveChat._pendingStreamSave = null;
                    } catch (e) {}
                }
            });
        }
    ...
    saveChatBox: function(chat, flushNow = false) {
        ...
        var entry = {
            ...
            // 【重启记忆增强】各轮工具结果摘要（跨轮次累积，重启后随 chat 恢复）
            roundToolBriefs: (function(){ try { return JSON.parse(JSON.stringify(chat._roundToolBriefs || (prev && prev.roundToolBriefs) || [])); } catch(e) { return (prev && prev.roundToolBriefs) || []; } })(),
            // 【风筝测速】真实往返间隔数据（刷新/重启后仍可追溯，不再变回「暂无数据」）
            lastGapMs: Number(chat._lastGapMs) || (prev ? (prev.lastGapMs || 0) : 0),
            lastApiAt: Number(chat._lastApiAt) || (prev ? (prev.lastApiAt || 0) : 0),
            gapLog: (function(){ try { return JSON.parse(JSON.stringify(chat._gapLog || (prev && prev.gapLog) || [])); } catch(e) { return (prev && prev.gapLog) || []; } })()
        };
        // 更新内存
        var found = false;
        for (var i = 0; i < this.data.chatBoxes.length; i++) {
    ...
    addLog: function(level, boxId, action, detail) {
        ...
        // SQLite — 1s 批处理：一次请求发送整批日志（后端支持 logs 数组）
        if (this.dbOnline && typeof DB !== 'undefined') {
            ...
            this._debounce('logs', function() {
                ...
                this._logBuffer = [];
                if (typeof DB.addLogs === 'function') {
                    DB.addLogs(batch).catch(function() {});
                } else {
    ...
    clearModelKey: function(modelId) {
        ...
        // 同步清空服务端 /api/models/config（private/api_keys.json）中的 key，
        // 否则 Models.save() 后服务端残留旧 key 会被回填，出现“清空后提交又回来”的问题
        try {
            fetch('/api/models/config').then(function(res){ return res.json(); }).then(function(cfgData){
                var srvList = (cfgData && cfgData.ok && cfgData.config && Array.isArray(cfgData.config.list)) ? cfgData.config.list : null;
                if (!srvList || !srvList.length) return;
                var changed = false;
                for (var i = 0; i < srvList.length; i++) {
                    var mm = srvList[i];
                    if (!mm) continue;
                    if (mm.id === modelId || mm.modelId === modelId) {
                        if (mm.key) { mm.key = ''; changed = true; }
                    }
                }
                if (!changed) return;
                return fetch('/api/models/config', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ list: srvList })
                });
            }).then(function(r) {
                if (r && !r.ok) console.warn('[Store] clearModelKey sync api_keys.json failed');
            }).catch(function(e) {
                console.warn('[Store] clearModelKey sync error:', e);
            });
        } catch (e) {
            console.warn('[Store] clearModelKey sync error:', e);
        }
