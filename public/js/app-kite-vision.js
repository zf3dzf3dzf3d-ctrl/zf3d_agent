...
// openDualPanels, openImageEditPanel, openVisionPanel, setVisionStatus
  ...
  function openDualPanels(clientX, clientY) {
    ...
    if (panel) {
      panel.classList.add('kite-dual-panel');
      // z-index 不写死：取当前所有节点最大 z-index +1，保证永远压在普通对话节点之上
      var _maxZ = 0;
      host.querySelectorAll('.kite-node,.node,.chatbox,[class*="node"]').forEach(function (el) {
        var z = parseInt(el.style.zIndex, 10);
        if (!isNaN(z) && z > _maxZ) _maxZ = z;
      });
      var _pz = Math.max(_maxZ + 1, 10001);
      panel.style.zIndex = _pz;
      // 同步抬高普通对话窗口的 zCounter，防止后续点击对话窗口时 ++zCounter 反超面板
      try {
        if (window.App) {
          if (typeof App.zCounter !== 'number' || App.zCounter < _pz + 1) App.zCounter = _pz + 1;
          if (App.store && typeof App.store === 'object' && 'zCounter' in App.store) App.store.zCounter = App.zCounter;
        }
      } catch (e) {}
    }
    return { toolbar: null };
