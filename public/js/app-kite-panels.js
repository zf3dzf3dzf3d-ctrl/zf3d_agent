  ...
  const dualPanelBuilders = new Map([
    ['image', function (host, anchor, origin) {
      ...
      panel.querySelector('.kite-panel-generate').addEventListener('click', () => {
        ...
        // v5.1.2 视频生成确认：视频成本较高，开始前必须用户点击确定
        if (mediaType === 'video' && !window.confirm('🎬 确认生成视频？\n\n视频生成消耗较多算力/费用，可能需要较长时间。\n\n点击「确定」开始生成，「取消」放弃。')) { status.textContent = '已取消视频生成'; return; }
    ...
    ['chat', function (host, anchor, origin) {
      ...
      panel.innerHTML = `<div class="kite-panel-header"><h3>✨ 创建对话框</h3><button type="button" class="kite-panel-close" title="关闭">✕</button></div>
        <div class="kite-create-cols">
          <div class="kite-create-col kite-create-col-text">
            <div class="kite-create-col-title" style="display:flex;align-items:center;justify-content:space-between;"><span></span></div>
      ...
      function fillModelList(sortByLatency) {
        var models = (window.Models && Models.list ? Models.list : []).filter(function (m) {
          if (!isTextModel(m)) return false;
          return m.visible !== false;
        });
        if (sortByLatency) {
          // ⚡ 测速排序：有延迟数据的排前面（快→慢），没数据的排后面（保持原序）
          models = models.slice().sort(function (a, b) {
            var ra = Models.getLatency ? Models.getLatency(a.id) : null;
            var rb = Models.getLatency ? Models.getLatency(b.id) : null;
            if (ra && rb) return ra.avg - rb.avg;
            if (ra) return -1;
            if (rb) return 1;
            return 0;
          });
        ...
        models.forEach(m => {
          ...
          item.dataset.mid = m.id; // 标记模型ID，供后台测速完成后单独刷新延迟标注
          var lat = (Models.latencyText ? Models.latencyText(m.id) : '');
          item.innerHTML = '<span class="ctx-icon">💬</span> ' + (m.name || m.id) +
            (lat ? ' <span class="kite-chat-latency" style="opacity:.65;font-size:.85em;">⚡' + lat + '</span>' : '');
          item.title = (m.modelId || m.id || '') + (lat ? ' · 平均响应 ' + lat : '');
        ...
        // ===== 列表最下面：随手标题入口（在双击的画布位置创建便签标记）=====
        const noteBtn = document.createElement('button');
        noteBtn.type = 'button';
        noteBtn.className = 'kite-chat-item kite-chat-quick-note';
        noteBtn.innerHTML = '<span class="ctx-icon">📍</span> 随手标题';
        noteBtn.title = '在画布此位置创建一个随手标题（回车确认，可拖动/双击编辑/✕删除）';
        noteBtn.addEventListener('click', () => {
          // origin 是双击时的画布内坐标（openDualPanels 传入）
          if (window.QuickNote) QuickNote.create(origin.x, origin.y, '');
          panel.remove();
        });
        list.appendChild(noteBtn);
      ...
      function runAutoLatencyTest() {
        if (!window.Models || typeof Models.test !== 'function') return;
        var autoTargets = (Models.list || []).filter(function (m) {
          return isTextModel(m) && m.visible !== false;
        });
        // 后台测速：每个模型测完立刻单独刷新它自己的延迟标注，不阻塞列表、不互相等待
        autoTargets.forEach(function (m) {
          Promise.resolve(Models.test(m)).catch(function () { return null; }).then(function () {
            var item = list.querySelector('.kite-chat-item[data-mid="' + (m.id || '').replace(/"/g, '\\"') + '"]');
            if (!item) return; // 面板已重建或该模型已隐藏，跳过
            var lat = (Models.latencyText ? Models.latencyText(m.id) : '');
            var latHtml = lat ? ' <span class="kite-chat-latency" style="opacity:.65;font-size:.85em;">⚡' + lat + '</span>' : '';
            var iconHtml = '<span class="ctx-icon">💬</span> ';
            var nameHtml = (m.name || m.id);
            item.innerHTML = iconHtml + nameHtml + latHtml;
            item.title = (m.modelId || m.id || '') + (lat ? ' · 平均响应 ' + lat : '');
          });
        });
      }
      fillModelList(); // 先立即显示模型列表（不等测速）
      if (window.Models && !Models._loaded && typeof Models.load === 'function') {
        Models.load().then(function() {
          fillModelList(); // 模型列表异步加载完成后重建列表（带最新延迟数据）
          runAutoLatencyTest(); // 再启动后台测速，防止首次打开时列表为空导致漏测
        }).catch(function() {});
      }
      runAutoLatencyTest(); // 后台测速（不阻塞列表显示，测完一个刷新一个）
      return panel;
    }],
  ]);
