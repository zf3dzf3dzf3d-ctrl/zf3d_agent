...
(function () {
  ...
  function ensurePanelButton() {
    ...
    if(!gear){gear=document.createElement('div');gear.id='fgEditGear';gear.textContent='⚙️';gear.title='画布工具（编辑模式）（Alt+2 显示/隐藏入口）';gear.style.cssText='position:absolute;right:14px;bottom:60px;z-index:60;width:36px;height:36px;border-radius:10px;display:none;align-items:center;justify-content:center;cursor:pointer;background:rgba(10,20,35,.85);border:1px solid rgba(0,229,255,.4);font-size:16px;pointer-events:auto;box-shadow:0 0 12px rgba(0,229,255,.25);';gear.addEventListener('click',function(){bar.style.display=(bar.style.display==='none')?'flex':'none';});host.appendChild(gear);}
    ...
    btn.style.cssText = 'padding:8px 14px;border-radius:10px;border:1px solid rgba(0,229,255,.5);' +
      'background:rgba(10,20,35,.85);color:#9fe8ff;cursor:pointer;font-size:13px;' +
