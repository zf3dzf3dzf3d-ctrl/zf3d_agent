...
Object.assign(App, {
        ...
        createChatBox: function(clientX, clientY, modelId) {
            ...
            // 【用户习惯继承】未显式指定模型时，使用上次选择的大模型/模型ID/思考强度
            ...
            if (!modelId) {
                try {
                    ...
                    if (_habit && _habit.modelId && Models.get(_habit.modelId)) {
                        modelId = _habit.modelId;
                    }
                } catch (e) { _habit = null; }
            }
            ...
            // 习惯中保存的模型ID/思考强度覆盖初始快照（仅新对话）
            if (_habit) {
                if (_habit.modelIdOverride) initialModelIdOverride = String(_habit.modelIdOverride);
                if (_habit.reasoningEffort) initialReasoningEffort = String(_habit.reasoningEffort);
            }
            ...
            var catList = Tools.getCategoryList(box.id, _newEngId);
            var catHtml = '';
            ...
            // 底层对话引擎选择器数据（server/engines/，DB.loadEngines 已缓存）
            var _engList = (typeof DB !== 'undefined' && DB.getEngines) ? DB.getEngines() : [];
            var _engHtml = '';
            _engList.forEach(function(en) {
                _engHtml += '<div class="eng-item" data-eng="' + en.id + '" title="' + (en.description || '').replace(/"/g, '&quot;') + '">' +
                    '<span>' + (en.icon ? en.icon + ' ' : '') + en.name + '</span>' +
                    (en.default ? '<span style="opacity:.6;font-size:10px;">默认</span>' : '') +
                    '</div>';
            });
            ...
            box.innerHTML =
                '<div class="chatbox-header" title="拖拽移动对话；Shift+左键拖拽：按下即在鼠标处复制一个一模一样的对话并跟随拖动">' +
                    ...
                    '<button class="hd-btn min-btn" data-act="minimize" title="最小化">—</button>' +
                    '<button class="hd-btn close" data-act="close" title="关闭">✕</button>' +
                    '</div>' +
                '</div>' +
                '<div class="chatbox-body">' +
                    '<div class="msg ai">' + (() => {
                        // 欢迎语包含：模型名 + 底层引擎 + 工具模式 + 当前项目
                        var _w = '您好！我是' + (model ? model.name : '当前模型');
                        // 模型名不带"模型"两字时自动补"大模型"
                        if (_w.indexOf('模型') === -1) _w += '大模型';
                        // 模型ID与思考强度与模型名同一行（先拼接，最后统一加句号换行）
                        try {
                            var _effMid = initialModelIdOverride || (model ? model.modelId : '') || '';
                            if (_effMid) _w += '，模型ID：' + _effMid;
                            var _effRe = initialReasoningEffort || '';
                            if (_effRe) {
                        ...
                        _w += '。<br>';
                        // 底层对话引擎（默认引擎或第一个）
                        var _catName = Tools.chatCategories[box.id] || _newCatName || '极简';
                        ...
                        _w += '我使用的是' + (_eng ? (_eng.icon ? _eng.icon + ' ' : '') + _eng.name : '朱峰底层') + '对话引擎，' +
                              ((_cat && _cat.icon ? _cat.icon + ' ' : '📄 ') + _catName) + '模式。<br>';
                        // 当前项目上下文
                        try {
                            if (typeof App.getProjectContext === 'function') {
                                var _pc = App.getProjectContext();
                                if (_pc && (_pc.root || _pc.project_id)) {
                                    _w += '当前项目：' + (_pc.project_name || '未命名') + '。<br>';
                                } else {
                                    _w += '当前项目：未指定。<br>';
                                }
                            } else {
                        ...
                        } catch (e) { _w += '当前项目：未指定。<br>'; }
                        _w += '请告诉我您的目标和任务。';
                        if (!model) _w += '<br>（未选择模型，请在底部下拉菜单选择）';
                        return _w;
                    })() + '</div>' +
                '</div>' +
                ...
                '<div class="chatbox-configrow">' +
                    '<div class="eng-picker-wrap">' +
                        ...
                        '</button>' +
                        '<div class="eng-menu" hidden>' + _engHtml + '</div>' +
                    '</div>' +
