        ...
        // 全局浮动弹窗（挂 document.body，fixed 居中，无遮罩可点别处），返回 Promise
            ...
            return new Promise(function(resolve) {
                                ...
                                askTimer = setTimeout(function() {
                                    // 【超时语义修正】超时≠用户取消：cancelled=true 会让循环停止对话；
                                    // 超时应把"未回答"如实回传给模型，由模型决定继续推进还是收尾。
                                    askResolve({ success: false, _timeout: true, question: question, answer: '', message: '等待用户输入超时（5 分钟无回答）。用户可能不在电脑前。请基于已有信息继续推进任务，或用 task_complete 收尾并说明在等用户哪项输入。', tool: 'ask_user' });
                                    try { if (modal && modal.parentNode) modal.parentNode.removeChild(modal); } catch(e){}
                                    try { Store.addLog('warn', (chat && chat.id) || '', 'ask-user-timeout', 'ask_user 等待用户回答超时(300s)，已回传超时结果继续循环: ' + String(question).slice(0, 80)); } catch(e2) {}
