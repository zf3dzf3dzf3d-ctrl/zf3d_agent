...
.fg-edge-glow {
  ...
  opacity:.22; animation: fg-fade-in 1.2s ease forwards;
}
.fg-edge-label {
  position:absolute;
  padding:2px 9px;
...
`;
  ...
  FG.deploy = function (text, opts) {
    ...
    nodeIds.forEach(function (id, i) {
      ...
      // 【性能修复】不在插入循环里读 offsetWidth/offsetHeight（每次读取都会强制重排，
      // N 个节点 = N 次重排，启动恢复多层图时造成 Forced reflow 卡顿）。
      // 先用布局默认尺寸占位，650ms 后的精确重画会按真实尺寸校正连线。
      var NODE_W = (window.FG_NODE_W || 190);
      nodeEls[id] = { el: el, w: NODE_W, h: 60 };
    });
    ...
    _buildStyleBar();
    setTimeout(function () {
      // 【性能修复】先把所有节点的真实尺寸一次性读完，再统一写回，
      // 避免读一个写一个造成的读写交错强制重排
      var ws = [], hs = [];
      nodeIds.forEach(function (id) { ws.push(nodeEls[id].el.offsetWidth); hs.push(nodeEls[id].el.offsetHeight); });
      nodeIds.forEach(function (id, i) { nodeEls[id].w = ws[i]; nodeEls[id].h = hs[i]; });
      ...
      // 真实尺寸测出后，校正图层关闭按钮位置（原占位尺寸可能有偏差）
      try { positionCloseBtn(); } catch (e) {}
      FG._persist();
    }, 650);
    ...
    // 【性能修复】抽成函数：插入时用占位尺寸先定位，650ms 真实尺寸测出后再校正一次
    var positionCloseBtn = function () {
      try {
  ...
  _closeCss.textContent = [
    ...
    'background:rgba(20,20,28,.55);border:1px solid rgba(255,255,255,.18);',
