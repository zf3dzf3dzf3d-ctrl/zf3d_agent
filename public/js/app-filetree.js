...
Object.assign(App, {
    ...
    // ===== 【5.1.2 选中持久化】缩略图选中文件跨重启记忆 =====
    // 存储走 UserSettings（private/用户设置/user_settings.json + localStorage 镜像），
    // key = ft_last_selected_files（绝对路径数组）。重启浏览器/服务端后由
    // restoreFtMemory() → _restoreSel() 恢复，getProjectContext 注入的 selected 不再丢失。
    _persistSel: function() {
        var self = this;
        try {
            var arr = [];
            for (var k in self._ftSelected) if (self._ftSelected[k]) arr.push(k);
            if (arr.length > 2000) arr.length = 2000; // 防全选大目录撑爆设置文件
            if (typeof UserSettings !== 'undefined' && UserSettings.set) UserSettings.set('ft_last_selected_files', arr);
        } catch (e) {}
    },
    _restoreSel: function() {
        var self = this;
        try {
            // 本次会话已有选中（用户刚操作过）则不覆盖
            for (var k in self._ftSelected) { if (self._ftSelected[k]) return; }
            if (typeof UserSettings === 'undefined' || !UserSettings.get) return;
            var arr = UserSettings.get('ft_last_selected_files', null);
            if (!arr || !arr.length) return;
            var sel = {};
            for (var i = 0; i < arr.length; i++) { if (arr[i]) sel[arr[i]] = true; }
            self._ftSelected = sel;
            // 面板已打开则立即刷新选中态
            if (self._ftPanelOpen) {
                if (self._renderThumbs) self._renderThumbs();
                if (self._syncSelBar) self._syncSelBar();
                if (self._notifySelChange) self._notifySelChange();
            }
        } catch (e) {}
    },
    _ftHist: [],
    ...
    closeFileTreePanel: function() {
        ...
        this._ftMinimized = false;
        var dock = document.getElementById('ftDock');
        if (dock) dock.remove();
    },

    // ===== 最小化到底部 dock（类 macOS 缩放动画）=====
    minimizeFileTreePanel: function() {
        var self = this;
        var panel = document.getElementById('ftPanel');
        if (!panel) return;
        var label = '';
        try { label = (App.activeProject && App.activeProject.name) || localStorage.getItem('active_project_name') || ''; } catch (e) {}
        if (!label) { var t = panel.querySelector('.ft-title'); if (t) label = t.textContent || ''; }
        label = (label || '文件夹').trim();
        var short = label.slice(0, 4);
        var dock = document.getElementById('ftDock');
        if (!dock) {
        ...
        tag.className = 'ft-dock-tag';
        tag.textContent = short;
        tag.title = label + '（点击恢复）';
        tag.onclick = function() { self.restoreFileTreePanel(); };
        dock.appendChild(tag);
        // 飞向底部 dock（ genie 缩放）
        var r = panel.getBoundingClientRect();
        var dx = 12 - r.left;
        var dy = (window.innerHeight - 10) - r.top;
        panel.style.transformOrigin = '50% 100%';
        panel.style.transition = 'transform .45s cubic-bezier(.3,0,.2,1), opacity .4s ease';
        panel.style.transform = 'translate(' + dx + 'px,' + dy + 'px) scale(.02)';
        panel.style.opacity = '0';
        var overlay = document.getElementById('ftPanelOverlay');
        if (overlay) overlay.classList.remove('open');
        self._ftPanelOpen = false;
        self._ftMinimized = true;
        setTimeout(function() {
    ...
    restoreFileTreePanel: function() {
        var self = this;
        var dock = document.getElementById('ftDock');
        if (dock) dock.classList.remove('show');
        var panel = document.getElementById('ftPanel');
        if (!panel) return;
        self._ftMinimized = false;
        panel.classList.add('open');
        // 从 dock 位置放大飞回原位
        var r = panel.getBoundingClientRect();
        var dx = 12 - r.left;
    ...
    toggleFileTreePanel: function() {
        if (this._ftMinimized) { this.restoreFileTreePanel(); return; }
    ...
    _renderShell: function() {
        ...
        if (header) {
            ...
            header.innerHTML =
                ...
                '<button class="ft-nav-btn" id="ftMinBtn" title="最小化到底部">─</button>' +
        ...
        var minBtnEl = document.getElementById('ftMinBtn');
        if (minBtnEl) minBtnEl.onclick = function() { self.minimizeFileTreePanel(); };
    ...
    _renderTree: function() {
        ...
        var build = function() {
            var html = self._sidebarHtml();
            // 钉住项目节点也要渲染子级（此前只渲染盘符子级，导致展开项目节点一片空白）
            var pins = self._getPins() || [];
            pins.forEach(function(p) {
                var pk = self._norm(p);
                html += self._renderTreeLevel(pk, 1);
            });
            html += '<div class="ft-home-title">💾 磁盘</div>';
    ...
    // ===== 缩略图并发加载队列【5.1.3 优化】：同一时刻最多 4 张原图，
    // 防止大目录几百个原图请求挤爆浏览器连接池（同域仅 6 连接），把 /api/fs/browse 等接口一起堵死 =====
    _ftImgQueue: [],
    _ftImgActive: 0,
    // 缩略图地址：带 v=mtime 版本参数 → 文件更新后 URL 变化，配合后端图片长缓存，重开面板不重拉
    _thumbSrc: function(f) {
        return '/api/fs/file?path=' + encodeURIComponent(f.path) + '&v=' + (f.mtime || 0);
    },
    _ftQueueImg: function(img, src) {
        if (!img || !src) return;
        img.removeAttribute('data-src');
        this._ftImgQueue.push({ img: img, src: src });
        this._ftPumpImgs();
    },
    _ftPumpImgs: function() {
        var self = this;
        var run = function(job) {
            var img = job.img;
            if (!img || !img.isConnected) return;   // 已被重渲染替换掉，跳过
            self._ftImgActive++;
            img.onload = function() { self._ftImgActive--; self._ftPumpImgs(); };
            img.onerror = function() {
                self._ftImgActive--;
                try { img.parentNode.innerHTML = '🖼'; } catch (e) {}
                self._ftPumpImgs();
            };
            img.src = job.src;
        };
        while (self._ftImgActive < 4 && self._ftImgQueue.length) {
            run(self._ftImgQueue.shift());
        }
    },
    ...
    _renderThumbs: function() {
        ...
        if (self._ftViewMode === 'list') {
            ...
            sorted.forEach(function(f) {
                ...
                // 【5.1.3 优化】图片懒加载：先占位不请求，滚动接近视口才经并发队列拉取（见 _ftThumbObserver）
                html += '<div class="ft-thumb" data-path="' + self._esq(f.path) + '" title="' + self._esq(f.name) + '">' +
                    '<div class="ft-thumb-img">' +
                        (isImg
                            ? '<img data-src="' + self._esq(self._thumbSrc(f)) + '" decoding="async" alt="" />'
                            : (isVid
                                ? '<div class="ft-video-thumb" title="双击用查看器播放">🎬</div>'
        ...
        // 【5.1.3 优化】缩略图懒加载：进入视口附近的占位图才经并发队列加载原图
        if (self._ftThumbObserver) { try { self._ftThumbObserver.disconnect(); } catch (e) {} }
        self._ftThumbObserver = new IntersectionObserver(function(entries) {
            entries.forEach(function(en) {
                if (!en.isIntersecting) return;
                var img = en.target;
                self._ftThumbObserver.unobserve(img);
                self._ftQueueImg(img, img.getAttribute('data-src'));
            });
        }, { root: box, rootMargin: '400px' });
        box.querySelectorAll('img[data-src]').forEach(function(img) { self._ftThumbObserver.observe(img); });
    ...
    _renameSelected: function() {
        ...
        var src = paths[0];
        var oldName = src.replace(/[\\/]+/, '/').split('/').pop();
        var v = window.prompt('重命名：', oldName);
        if (!v || v.trim() === oldName) return;
        fetch('/api/fs/ops', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'rename', paths: [src], target: v.trim() })
        }).then(function(r) { return r.json(); }).then(function(d) {
            if (d && d.ok) {
                if (typeof ToastStack !== 'undefined' && ToastStack.show) ToastStack.show('已重命名为 ' + (d.done || []).join(','), 'ok');
                self._ftSelected = {};
                self._persistSel();
                delete self._ftDirCache[self._norm(self._ftCwd)];
                self._renderThumbs();
                ...
                if (typeof ToastStack !== 'undefined' && ToastStack.show) ToastStack.show('重命名失败: ' + ((d && d.error) || ''), 'error');
            }
        }).catch(function(err) {
            if (typeof ToastStack !== 'undefined' && ToastStack.show) ToastStack.show('网络错误: ' + err, 'error');
        });
    },
    // 【新增】在当前目录新建文件夹
    _mkdir: function() {
        var self = this;
        var v = window.prompt('新建文件夹，名称：', '新建文件夹');
        if (!v) return;
        fetch('/api/fs/ops', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'mkdir', paths: [self._ftCwd || ''], target: v.trim() })
        }).then(function(r) { return r.json(); }).then(function(d) {
            if (d && d.ok) {
                if (typeof ToastStack !== 'undefined' && ToastStack.show) ToastStack.show('已创建文件夹 ' + (d.done || []).join(','), 'ok');
                delete self._ftDirCache[self._norm(self._ftCwd)];
                self._renderThumbs();
                self._renderTree();
            } else {
                if (typeof ToastStack !== 'undefined' && ToastStack.show) ToastStack.show('创建失败: ' + ((d && d.error) || ''), 'error');
            }
        }).catch(function(err) {
            if (typeof ToastStack !== 'undefined' && ToastStack.show) ToastStack.show('网络错误: ' + err, 'error');
        });
    ...
    _bindFsHotkeys: function() {
        ...
        // 【新增】文件区右键菜单：新建文件夹 / 重命名 / 删除
        if (self.__fsCtxBound) return; self.__fsCtxBound = true;
        document.addEventListener('contextmenu', function(e) {
            if (!self._ftPanelOpen) return;
            var zone = e.target.closest && (e.target.closest('#ftThumbs') || e.target.closest('#ftTree'));
            if (!zone) return;
            e.preventDefault();
            var old = document.getElementById('ftCtxMenu');
            if (old) old.remove();
            var menu = document.createElement('div');
            menu.id = 'ftCtxMenu';
            menu.style.cssText = 'position:fixed;z-index:99999;background:#1e1e1e;border:1px solid #444;border-radius:6px;padding:4px 0;min-width:150px;box-shadow:0 4px 16px rgba(0,0,0,.5);font-size:13px;color:#ddd;';
            var items = [
                ['📁 新建文件夹', function() { self._mkdir(); }],
                ['✏️ 重命名', function() { self._renameSelected(); }],
                ['🗑 删除', function() { self._deleteSelected(); }]
            ];
            items.forEach(function(it) {
                var el = document.createElement('div');
                el.textContent = it[0];
                el.style.cssText = 'padding:6px 14px;cursor:pointer;white-space:nowrap;';
                el.onmouseenter = function() { el.style.background = '#2d2d2d'; };
                el.onmouseleave = function() { el.style.background = ''; };
                el.onclick = function() { menu.remove(); it[1](); };
                menu.appendChild(el);
            });
