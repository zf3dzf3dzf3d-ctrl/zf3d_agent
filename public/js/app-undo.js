...
(function () {
    ...
    var CanvasMenu = {
        ...
        renderMenu: function () {
            ...
            // 一键重启后台（原右下角悬浮按钮，收进文件菜单）
            // 走 /api/revive 安全重启：重启前自动等待 AI 半成品代码恢复可加载，
            // 绝不带病启动；旧进程精确结束，不误杀训练任务等其他 Python 程序。
            // 【体验重做】不再用全屏黑暗遮罩（用户反馈像"整个页面关掉又弹新节目"），
            // 改为右下角轻量 toast：界面全程可见可操作，当前对话原样保留。
            menu.appendChild(this._item('🔄 一键重启后台', '界面保持不动，几秒完成，带进度提示', function () {
                if (window.__reviving) return;
                if (!confirm('重启后台服务器？当前界面和对话保持不动，约 3~8 秒完成。')) return;
                window.__reviving = true;
                // 【体验改进】右上角轻量进度条：全程可见阶段反馈，不再"点了没反应"。
                // 页面不刷新、对话不打断；完成后自动消失。
                var bar = null, barText = null, barSub = null;
                function ensureBar() {
                    if (bar) return;
                    bar = document.createElement('div');
                    bar.style.cssText = 'position:fixed;top:14px;right:14px;z-index:99999;' +
                        'background:var(--bg1,#1e1e1e);border:1px solid var(--border,#3a3a3a);border-radius:10px;' +
                        'padding:12px 16px;min-width:230px;box-shadow:0 6px 24px rgba(0,0,0,.45);font-size:13px;';
                    var spin = document.createElement('span');
                    spin.textContent = '🔄 ';
                    spin.style.cssText = 'display:inline-block;animation:__revspin 1s linear infinite;';
                    if (!document.getElementById('__revspin_style')) {
                        var st = document.createElement('style'); st.id = '__revspin_style';
                        st.textContent = '@keyframes __revspin{from{transform:rotate(0)}to{transform:rotate(360deg)}}';
                        document.head.appendChild(st);
                    }
                    barText = document.createElement('div');
                    barText.style.cssText = 'color:var(--text,#eee);display:inline;';
                    barSub = document.createElement('div');
                    barSub.style.cssText = 'color:var(--text2,#999);font-size:11px;margin-top:4px;';
                    bar.appendChild(spin); bar.appendChild(barText); bar.appendChild(barSub);
                    document.body.appendChild(bar);
                }
                function setBar(text, sub, done, ok) {
                    if (!bar) return;
                    barText.textContent = text;
                    barSub.textContent = sub || '';
                    if (done) {
                        bar.style.borderColor = ok ? 'var(--green,#4caf50)' : 'var(--red,#ef5350)';
                        bar.querySelector('span').textContent = ok ? '✅ ' : '⚠️ ';
                        spin = null;
                        setTimeout(function () { if (bar && bar.parentNode) bar.parentNode.removeChild(bar); bar = null; }, 4000);
                    }
                }
                ensureBar();
                setBar('正在通知后台重启…', '界面可正常使用，对话不打断');
                // 【2026-09 修复】真正触发重启：此前本按钮只轮询 /api/health、从不发重启请求，
                // 点了等于没点。现恢复调用 POST /api/revive——服务端拉起 auto_revive --force
                // （等代码可加载后由 restart_server.py 精确重启后台，与外部守护同一套带锁重启逻辑）。
                try {
                    fetch('/api/revive', { method: 'POST' }).then(function (r) { return r.json(); }).then(function (j) {
                        // 请求已被服务端受理 → 立即给第二阶段反馈，消除"没反应"感
                        setBar('旧进程正在结束…', '几秒后自动完成，无需等待');
                    }).catch(function () {
                        // 服务器已死时 fetch 必然失败：交由下方轮询判断；彻底死亡由外部守护自动复活
                        setBar('后台短暂离线…', '正在等待新进程上线');
                    });
                } catch (e) {}
                ...
                var poll = function () {
                    ...
                    fetch('/api/health', { cache: 'no-store', signal: ctl.signal })
                        .then(function (r) { return r.json(); })
                        .then(function (h) {
                        if (h && h.ok) {
                            // 【按用户要求】只重启后台，不刷新前台页面——当前对话原样保留
                            var waited = Math.round((Date.now() - started) / 1000);
                            setBar('后台重启完成（' + waited + ' 秒）', '对话继续，无需任何操作', true, true);
                            window.__reviving = false;
                            return;
                        }
                        // 轮询中：阶段文案随时间推进，让用户知道进展到哪一步
                        var s = Math.round((Date.now() - started) / 1000);
                        if (s < 3) setBar('旧进程正在结束…', '已等待 ' + s + ' 秒');
                        else if (s < 10) setBar('新进程启动中…', '已等待 ' + s + ' 秒，正常范围');
                        else setBar('还在启动…', s + ' 秒（首次加载模型/引擎会稍慢）');
                        retry();
                        })
                        .catch(function () {
                            var s = Math.round((Date.now() - started) / 1000);
                            setBar('后台短暂离线…', '正在等待新进程上线（' + s + ' 秒）');
                            retry();
                        });
                };
                var retry = function () {
                    if (Date.now() - started > 180000) {
                        setBar('3 分钟未恢复', '外部守护会自动拉起后台；也可双击 .启动朱峰社区智能体无限.bat', true, false);
                        window.__reviving = false;
                        return;
                    }
                    setTimeout(poll, 1000);
                };
                setTimeout(poll, 1200);   // 服务端 --delay 1s 自杀，1.2 秒起轮询（1 秒一次）
            }, false));
        ...
        // ===== 重启服务（已废弃：统一走「🔄 一键重启后台」App._doRestart，避免两套重启逻辑冲突） =====
        ...
        quitApp: function () {
            ...
            if (App._confirmDialog) {
                App._confirmDialog({ title: '退出系统', icon: '⏻', confirmText: '退出', html: '<b style="color:var(--red,#ef5350)">确定退出朱峰社区智能体系统？</b><br><span style="font-size:12px;color:var(--text2)">退出后页面将不可用，需手动重新启动程序。</span>' }).then(function (ok) { if (ok) doQuit(); });
            } else if (window.confirm('确定退出系统？')) doQuit();
        },
