...
Object.assign(App, {
        ...
        renderModelList: function() {
            ...
            Models.list.forEach(function(m) {
                ...
                html += '<div class="model-item" data-model-id="' + esc(m.id) + '" draggable="true">' +
                    ...
                    '</div>' +
                    // 第3.5行：协议格式（v5.1.3：自动识别 / OpenAI / Anthropic）
                    '<div class="mi-settings-row">' +
                        '<div class="mi-field"><span class="mi-field-label">协议格式</span>' +
                        '<select data-apiformat-select="' + esc(m.id) + '">' +
                        '<option value=""' + (!m.apiFormat ? ' selected' : '') + '>自动识别</option>' +
                        '<option value="openai"' + (m.apiFormat === 'openai' ? ' selected' : '') + '>OpenAI 格式</option>' +
                        '<option value="anthropic"' + (m.apiFormat === 'anthropic' ? ' selected' : '') + '>Anthropic 格式</option>' +
                        '</select></div>' +
                    '</div>' +
        ...
        saveModelSettings: async function(id) {
            ...
            // 【v5.1.3】协议格式（自动识别 / openai / anthropic）
            var _afSel = container.querySelector('[data-apiformat-select="' + id + '"]');
            m.apiFormat = _afSel ? _afSel.value : '';
            var self = this;
            try {
                await Models.save();
            } catch (e) {
        ...
        saveModel: function() {
            ...
            var apiFormat = (document.getElementById('cfg-apiformat') || {}).value || '';
            ...
            var newModel = { name: name, endpoint: endpoint, key: key, modelId: modelId, apiFormat: apiFormat, reasoningEffort: reasoningEffort, visible: true, enabled: true };
