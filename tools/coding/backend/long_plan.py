...
计划文件存于「public/项目记录/超长计划/」目录，单一事实来源，
支持多个对话框接力执行同一个计划。
...
def _plans_dir(ctx):
    d = os.path.join(ctx.base_dir, 'private', '记忆', '超长计划')
...
def handle(body, ctx):
    ...
    if action == 'stats':
        ...
        blocked = [s['no'] for s in steps if s['status'] == 'blocked']
        # 日志最后 10 条
        ...
        step_notes = {}
        for s in steps:
            mm2 = re.search(r'^###\s*\[[ xX~✓√\-!]\]\s*步骤\s*%d\s*[:：].*?\n(.*?)(?=\n###|\n## |\Z)' % s['no'], content, re.S | re.M)
            if mm2:
                nm = re.search(r'- 产出与卡点:\s*(.*)', mm2.group(1))
                if nm:
                    step_notes[s['no']] = nm.group(1).strip()
        ctx.send_json({
            ...
            'blocked_count': len(blocked), 'blocked_steps': blocked,
            ...
            'message': ('计划已全部完成 ✅' if (steps and not pending)
                        else ('进度 %d/%d，挂起 %d 步（%s），接下来从步骤 %s 开始。'
                              % (len(done), len(steps), len(blocked),
                                 ','.join(map(str, blocked)) or '-', pending[0] if pending else '-')))
        })
        return
