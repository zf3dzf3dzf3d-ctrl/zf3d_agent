...
_CATEGORIES = ('minimal', 'coding', 'writing', 'vision')
        ...
        for category in _CATEGORIES:
            ...
            for module_path in sorted(backend_dir.glob('*.py')):
                ...
                module = importlib.import_module(
                    f'tools.{category}.backend.{module_name}'
                )
...
def get_handler(tool_name):
    ...
    若注册表中没有该工具（典型情况：服务启动后新增的工具文件），
    重置扫描标记强制重扫一次，让新工具无需重启服务即可生效。
    """
    _scan()
    mod = _REGISTRY.get(tool_name)
    if mod is None:
        global _SCANNED
        _SCANNED = False
        _scan()
        mod = _REGISTRY.get(tool_name)
    # 别名：前端统一调用 /api/tools/find（kg-extract.js / kg-code.js），对应 find_files 后端
    if mod is None and tool_name == 'find':
        mod = _REGISTRY.get('find_files')
    return mod
