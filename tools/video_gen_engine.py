...
def _video_models():
    ...
    for name in ('models.json',):
        for base in ('private', '.'):
            ...
            try:
                # utf-8-sig：兼容带 BOM 的 models.json，否则 json.load 失败静默回退默认渠道
    ...
    ak = _key('火山方舟API', '火山方舟', '火山方舟视觉')
    if ak and not any(v['provider'] == 'ark' for v in out):
        # 注意：Coding Plan 专用 key 只支持 /api/plan/v3/ 通道（标准 /api/v3/ 会 401）
        out.append({'provider': 'ark', 'endpoint': 'https://ark.cn-beijing.volces.com/api/plan/v3',
                    'key': ak, 'model': 'doubao-seedance-2.0'})
    # 默认优先走火山方舟 seedance-2.0，智谱/硅基作为后备
...
def _zhipu_generate(cfg, prompt, duration, size, quality='quality',
                    ...
                    image_url=''):
    """智谱 CogVideoX：POST videos/generations 提交 -> 轮询 async-result/<id>；带 image_url 时为图生视频"""
...
def _sf_generate(cfg, prompt, duration, size, wait=True, timeout=600, fps=30, image_url=''):
    ep = cfg['endpoint'].rstrip('/')
    headers = {'Authorization': 'Bearer ' + cfg['key'], 'Content-Type': 'application/json'}
    ...
    if image_url:
        # 图生视频：切换到 I2V 模型
        model = model.replace('T2V', 'I2V') if 'T2V' in model else 'Wan2.1-I2V-14B-720P'
    payload = {'model': model, 'prompt': prompt, 'size': '%sx%s' % (w, h)}
    if image_url:
        payload['image_url'] = image_url
    if fps:
...
def _ark_generate(cfg, prompt, duration, size, wait=True, timeout=600, fps=30, image_url=''):
    ep = cfg['endpoint'].rstrip('/')
    headers = {'Authorization': 'Bearer ' + cfg['key'], 'Content-Type': 'application/json'}
    text = prompt + ' --resolution 480p --duration %d' % max(5, int(duration or 5))
    if fps:
    ...
    payload = {'model': cfg['model'], 'content': content}
    try:
        ...
        hint = '（注意：方舟"计划/Coding Plan"类 key 需走 /api/plan/v3/ 通道，标准 key 走 /api/v3/）' \
            if e.code == 401 else ''
...
def generate_video(prompt, duration=5, size='832x480', model='', key='', fps=30,
                   negative_prompt='', seed=None, wait=True, timeout=330, image_url=''):
    """主入口：按渠道优先级尝试，任一成功即返回；带 image_url 时为图生视频"""
    ...
    for cfg in channels:
        ...
        kwargs = {'wait': wait, 'timeout': timeout, 'fps': fps, 'image_url': image_url or ''}
        if cfg['provider'] == 'zhipu':
            kwargs['negative_prompt'] = negative_prompt
            kwargs['seed'] = seed
        r = eng(cfg, prompt, duration, size, **kwargs)
        if r.get('ok'):
