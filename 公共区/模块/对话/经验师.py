...
class 经验师类:
    ...
    def 沉淀经验(self, 用户消息: str, 推理结果: dict) -> dict:
        ...
        # 4. 冗余检测
        相似经验 = self._搜索相似经验(经验内容.get("name", ""), 经验内容.get("tags", []))
        if 相似经验:
            # 合并更新已有经验
            结果 = self._合并更新经验(相似经验[0], 经验内容)
            print(f"  📝 经验已合并更新: {相似经验[0]}")
            return {"操作": "合并", "名称": 相似经验[0]}
        else:
            # 新建经验
            名称 = self._保存新经验(经验内容)
            if 名称:
    ...
    def _搜索相似经验(self, 名称: str, 标签: list) -> list:
        ...
        for 条目 in self.索引["经验列表"]:
            ...
            # 得分≥3认为相似（至少一个标签匹配或两个关键词匹配）
            if 得分 >= 3:
                得分表[已有名] = 得分

        if 得分表:
            return sorted(得分表.keys(), key=lambda x: 得分表[x], reverse=True)
        return []
    ...
    def 召回经验(self, 用户消息: str, limit: int = 3) -> str:
        ...
        # 取得分最高的N条
        排序结果 = sorted(得分表.keys(), key=lambda x: 得分表[x], reverse=True)[:limit]
