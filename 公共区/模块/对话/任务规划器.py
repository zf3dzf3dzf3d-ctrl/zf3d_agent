...
class 任务规划器类:
    ...
    def 生成计划(self, 用户消息: str, 对话历史: list, 模型直连器,
        ...
        规划提示 = (
            ...
            f"可用操作:\n{操作说明}\n\n"
            f"请评估任务复杂度，决定需要几步完成。\n"
            f"简单任务（如改一行文字、查找一个文件）直接1-2步即可，不需要强行拆分。\n"
            f"复杂任务最多{self.最大步骤数}步。"
        ...
        系统提示 = (
            ...
            "要求：\n"
            "1. 根据任务复杂度决定步骤数：简单任务1-2步，复杂任务3-10步\n"
            "2. 只规划能通过工具操作执行的步骤，不要规划'运行GUI程序测试'等无法通过工具自动完成的步骤\n"
            "3. 每个步骤标注预计使用的操作名称\n"
            "4. 标注步骤间的依赖关系（依赖前面哪些步骤）\n"
            "5. 定义明确的成功标准\n"
            "6. 标注是否支持中途重规划\n\n"
            "输出JSON格式：\n"
            '```json\n'
