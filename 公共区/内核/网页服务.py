...
class 网页请求处理器(BaseHTTPRequestHandler):
    ...
    def do_HEAD(self):
        """HEAD请求：返回200表示端点存在（用于dev-reload探测）"""
        解析结果 = urlparse(self.path)
        路径 = unquote(解析结果.path)
        if 路径 == "/api/dev-reload-sse":
            # 检查开发模式是否开启
            _开发模式 = False
            try:
    ...
    def _处理API_GET(self, 路径: str, 解析结果):
        ...
        if 路径 == "/api/config":
            ...
            try:
                from 操作.高级 import _后台任务管理器
                管理器 = _后台任务管理器()
                self._返回JSON({"任务列表": 管理器.列表()})
            except Exception as e:
                self._返回JSON({"任务列表": [], "错误": str(e)})
        elif 路径 == "/api/files":
    ...
    def _处理API_POST(self, 路径: str, 数据: dict):
        ...
        if 路径 == "/api/agent-login":
            ...
            描述 = 数据.get("描述", "")
            原始代码 = 数据.get("原始代码", "")
            if not 描述 and not 原始代码:
                self._返回JSON({"成功": False, "错误": "请输入动画描述"})
                return
            if not self.模型直连器:
            ...
            if 原始代码:
                if 格式 == "svg":
                    系统提示 = (
                        f"你是一个SVG动画设计师。请对以下现有SVG动画代码进行修改。\n"
                        f"只输出修改后的完整SVG代码（带<svg>标签），不要任何解释说明。\n"
                        f"规则：\n"
                        f"1. 以现有代码为基础，仅根据用户要求做修改，未提及的部分保持原样\n"
                        f"2. 输出完整的可独立运行的SVG代码，不能省略未修改的部分\n"
                        f"3. 动画时长{时长}秒，{('循环播放' if 循环 else '播放一次')}\n"
                        f"4. 保持viewBox不变，用<animate>或CSS动画实现动效\n"
                        f"5. 不要添加注释、不要包裹```代码块标记\n"
                        f"用户修改要求：{描述}\n\n"
                        f"现有SVG代码：\n{原始代码}"
                    )
                else:
                    系统提示 = (
                        f"你是一个CSS动画设计师。请对以下现有HTML动画代码进行修改。\n"
                        f"只输出修改后的完整HTML代码，不要任何解释说明。\n"
                        f"规则：\n"
                        f"1. 以现有代码为基础，仅根据用户要求做修改，未提及的部分保持原样\n"
                        f"2. 输出完整的可独立运行的HTML文件，不能省略未修改的部分\n"
                        f"3. 使用CSS animation和@keyframes实现动画，时长{时长}秒，{('infinite循环' if 循环 else '播放'+str(时长)+'秒')}\n"
                        f"4. 纯HTML+CSS+JS单文件，不引用外部资源\n"
                        f"5. 不要添加注释、不要包裹```代码块标记\n"
                        f"用户修改要求：{描述}\n\n"
                        f"现有HTML代码：\n{原始代码}"
                    )
                ...
                if 格式 == "svg":
                    系统提示 = (
                        f"根据用户描述生成一个SVG动画。只输出SVG代码（带<svg>标签），不要解释。\n"
                        f"要求：\n"
                        f"1. 纯SVG代码，用<animate>或CSS动画实现动效\n"
                        f"2. viewBox='0 0 {宽} {高}'，动画时长{时长}秒，{('循环播放' if 循环 else '播放一次')}\n"
                        f"3. 用SVG形状：circle/rect/path/polygon/ellipse\n"
                        f"4. 深色背景(#1a1a2e)，图形居中\n"
                        f"5. 可用作网站favicon\n"
                        f"用户描述：{描述}"
                    )
                else:
                    系统提示 = (
                        f"根据用户描述生成一个CSS动画HTML页面。只输出HTML代码，不要解释。\n"
                        f"要求：\n"
                        f"1. 纯HTML+CSS+JS单文件，不引用外部资源\n"
                        f"2. 使用CSS animation和@keyframes实现动画，时长{时长}秒，{('infinite循环' if 循环 else '播放'+str(时长)+'秒')} \n"
                        f"3. 用div+CSS绘制图形（圆、方、三角等）\n"
                        f"4. 画布尺寸{宽}x{高}，居中显示，深色背景(#1a1a2e)\n"
                        f"5. 动画丝滑流畅，使用ease-in-out或cubic-bezier缓动\n"
                        f"用户描述：{描述}"
                    )
            ...
            try:
                ...
                if 结果.get("成功"):
                    ...
                    # 提取代码块或清理markdown标记
                    import re as _re
                    代码块 = _re.search(r'```(?:html|svg)?\s*\n(.*?)```', 回复, _re.DOTALL)
                    if 代码块:
                        回复 = 代码块.group(1).strip()
                    # 如果是SVG格式但用户要求HTML，包裹一下
                    if 格式 != "svg" and 回复.startswith("<svg"):
                        回复 = f'<!DOCTYPE html><html><head><meta charset="utf-8"><style>body{{margin:0;display:flex;align-items:center;justify-content:center;min-height:100vh;background:#1a1a2e;}}</style></head><body>{回复}</body></html>'
                    self._返回JSON({"成功": True, "数据": 回复})
                else:
            ...
            try:
                结果 = self.模型直连器.发送消息(
                    [{"role": "user", "content": 系统提示}],
                    "你是Lottie动画生成器，只输出JSON。",
                    跳过缓存=True
                )
                if not 结果:
                    self._返回JSON({"成功": False, "错误": "LLM返回为空，请检查模型配置"})
                    return
                if 结果.get("成功"):
                    回复 = 结果.get("回复内容", "").strip()
                    import re as _re_lottie
                    回复 = _re_lottie.sub(r'^```html\s*', '', 回复)
                    回复 = _re_lottie.sub(r'^```\s*', '', 回复)
                    回复 = _re_lottie.sub(r'\s*```$', '', 回复)
                    回复 = 回复.strip()
                    if '<html' in 回复.lower() or '<div' in 回复.lower() or '<style' in 回复.lower():
                        self._返回JSON({"成功": True, "数据": 回复})
                    else:
                        self._返回JSON({"成功": False, "错误": "LLM未输出有效HTML"})
                else:
                    self._返回JSON({"成功": False, "错误": 结果.get("错误", "LLM调用失败")})
            except Exception as e:
                self._返回JSON({"成功": False, "错误": str(e)})
            return
        ...
        elif 路径 == "/api/save-animation":
            """保存动画文件到指定文件夹"""
            文件名 = 数据.get("文件名", "animation_" + str(int(time.time())) + ".html")
            内容 = 数据.get("内容", "")
            目录 = 数据.get("目录", "")
            if not 内容:
                self._返回JSON({"成功": False, "错误": "缺少动画内容"})
                return
            if not 目录:
                self._返回JSON({"成功": False, "错误": "缺少保存目录"})
                return
            # 路径安全校验
            if ".." in 目录 or "\x00" in 目录:
                self._返回JSON({"成功": False, "错误": "非法路径"})
                return
            try:
                if not os.path.exists(目录):
                    os.makedirs(目录, exist_ok=True)
                保存路径 = os.path.join(目录, 文件名)
                ...
                if 操作 in ("article", "post", "work") and 参数.get("cover", ""):
                    ...
                    if not 封面值.startswith("http"):
                        ...
                        if _os_zf.path.exists(封面路径):
                            ...
                            try:
                                with urllib.request.urlopen(_上传请求, timeout=120) as _上传响应:
        ...
        if 路径 == "/api/restart":
            ...
            try:
                ...
                图数据 = 数据.get("图", {})
                用到的员工名 = set()
                for node in 图数据.get("nodes", []):
                    if node.get("type") in ("employee", "员工"):
                        emp_name = node.get("config", {}).get("员工名", "")
                        if emp_name:
                            用到的员工名.add(emp_name)
                # 从员工配置.json中提取这些员工的完整配置（含人设追加等全部字段）
                员工列表 = []
                配置路径 = self.配置加载器.项目根目录 / "公共区" / "配置" / "员工配置.json"
                if 配置路径.exists():
                    try:
                        全部员工 = json.loads(配置路径.read_text(encoding="utf-8")).get("员工列表", [])
                        for emp in 全部员工:
                            if emp.get("姓名") in 用到的员工名:
                                员工列表.append(emp)
                    except Exception:
                        pass
                # 读取当前员工分组
                ...
                if 分组路径.exists():
                    try:
                        分组列表 = json.loads(分组路径.read_text(encoding="utf-8"))
                    except Exception:
                        pass
                # 组合保存：图 + 员工 + 分组
                保存数据 = {
                ...
                路径文件.write_text(json.dumps(保存数据, ensure_ascii=False, indent=2), encoding="utf-8")
                self._返回JSON({"成功": True, "路径": str(路径文件)})
            except Exception as e:
                self._返回JSON({"成功": False, "错误": str(e)})
        elif 路径 == "/api/wf-load":
            """从文件载入节点图（含员工+分组，只创建不存在的员工）"""
            import os as _os
            目录 = self.配置加载器.项目根目录 / "节点图"
            if not 目录.exists():
                目录.mkdir(parents=True, exist_ok=True)
            ...
            if 文件名:
                路径文件 = 目录 / (文件名 + ".json")
                try:
                    保存数据 = json.loads(路径文件.read_text(encoding="utf-8"))
                    # 兼容旧格式：如果只有nodes/conns没有员工信息
                    图 = {"nodes": 保存数据.get("nodes", []), "conns": 保存数据.get("conns", [])}
                    # 如果有员工信息，只创建不存在的
                    员工列表 = 保存数据.get("员工", [])
                    新增数 = 0
                    if 员工列表 and "员工管理" in self.模块注册:
                        for emp in 员工列表:
                            emp_name = emp.get("姓名", "")
                            if emp_name:
                                # 检查是否已存在
                                try:
                                    运行时 = self.模块注册["员工管理"].获取运行时配置(emp_name)
                                    if not 运行时.get("成功"):
                                        # 不存在才创建（仅当工作流中包含完整人设追加时才创建，避免垃圾提示词）
                                        人设 = emp.get("人设追加", "")
                                        if 人设:
                                            self.模块注册["员工管理"].创建员工({
                                                "姓名": emp_name,
                                                "头像": emp.get("头像", "🤖"),
                                                "角色": emp.get("角色", emp_name),
                                                "目标": emp.get("目标", emp.get("角色", emp_name)),
                                                "人设追加": 人设,
                                                "独立记忆": emp.get("独立记忆", True),
                                                "状态": "在岗"
                                            })
                                            新增数 += 1
                                            print(f"  [WF-Load] 创建缺失员工(含人设): {emp_name}")
                                        else:
                                            print(f"  [WF-Load] 跳过创建员工「{emp_name}」：工作流中缺少人设追加，需手动创建")
                                except Exception as _e:
                                    print(f"  [WF-Load] 创建员工失败: {emp_name}: {_e}")
                    # 如果有分组信息，合并到现有分组（不覆盖已有的）
                    分组列表 = 保存数据.get("员工分组", [])
                    if 分组列表:
                        分组路径 = self.配置加载器.项目根目录 / "公共区" / "配置" / "员工分组.json"
                        现有分组 = []
                        if 分组路径.exists():
                            try:
                                现有分组 = json.loads(分组路径.read_text(encoding="utf-8"))
                            except Exception:
                                pass
                        ...
                        现有分组名 = {g.get("name"): g for g in 现有分组}
                        for g in 分组列表:
                            gname = g.get("name")
                            if gname in 现有分组名:
                                # 同名分组：合并employeeNames，去重
                                已有名 = set(现有分组名[gname].get("employeeNames", []))
                                for 名 in g.get("employeeNames", []):
                                    if 名 not in 已有名:
                                        现有分组名[gname].setdefault("employeeNames", []).append(名)
                                        已有名.add(名)
                            else:
                                现有分组.append(g)
                                现有分组名[gname] = g
                        ...
                        if "员工管理" in self.模块注册:
                            try:
                                ...
                                pass
                    self._返回JSON({"成功": True, "图": 图, "新增员工数": 新增数})
                except Exception as e:
                    self._返回JSON({"成功": False, "错误": str(e)})
            else:
            ...
            """轮询ComfyUI异步生成的图片"""
            # 工作流执行期间不响应前端轮询（防止清空队列导致图片节点取不到图）
            if hasattr(网页请求处理器, '_wf执行中') and 网页请求处理器._wf执行中:
                self._返回JSON({"成功": True, "图片列表": []})
                return
...
"""
                            ...
                            if not 运行时.get("成功"):
                                ...
                                if 人设:
                                    self.模块注册["员工管理"].创建员工({
                                        "姓名": emp_name,
                                        "头像": e.get("头像", "🤖"),
                                        "角色": e.get("角色", emp_name),
                                        "目标": e.get("目标", e.get("角色", emp_name)),
                                        "人设追加": 人设,
                                        "独立记忆": e.get("独立记忆", True),
                                        "状态": "在岗"
                                    })
                                    新增数 += 1
                                    print(f"  [Template-Load] 创建缺失员工: {emp_name}")
                                else:
                                    print(f"  [Template-Load] 跳过创建员工「{emp_name}」：缺少人设追加")
                if 模板分组:
                    分组路径 = self.配置加载器.项目根目录 / "公共区" / "配置" / "员工分组.json"
                    现有分组 = json.loads(分组路径.read_text(encoding="utf-8")) if 分组路径.exists() else []
                    现有分组映射 = {g.get("name"): g for g in 现有分组}
                    for g in 模板分组:
                        gname = g.get("name")
                        if gname in 现有分组映射:
                            # 同名分组：合并employeeNames，去重
                            已有名 = set(现有分组映射[gname].get("employeeNames", []))
                            for 名 in g.get("employeeNames", []):
                                if 名 not in 已有名:
                                    现有分组映射[gname].setdefault("employeeNames", []).append(名)
                                    已有名.add(名)
                        else:
                            现有分组.append(g)
                            现有分组映射[gname] = g
    ...
    def _处理员工工作流(self, 数据: dict):
        ...
        try:
            ...
            # 清空ComfyUI图片队列（防止上次运行的旧图片被本轮取到）
            if hasattr(网页请求处理器, '_comfyui图片队列'):
                _旧数量 = len(网页请求处理器._comfyui图片队列)
                if _旧数量 > 0:
                    print(f"  [WF] 清空ComfyUI图片队列: {_旧数量}张旧图片")
                    网页请求处理器._comfyui图片队列.clear()
            # 并发保护：仅定时任务触发的执行受_lock限制，手动执行不受限
            _是定时任务 = 数据.get("source") == "timer"
            if _是定时任务 and getattr(网页请求处理器, '_wf执行中', False):
                self._返回JSON({"成功": False, "错误": "工作流正在执行中，跳过本次定时触发"})
                return
            # 设置工作流执行中标志（防止前端轮询清空队列）
            网页请求处理器._wf执行中 = True
                ...
                try:
                    if ntype == "target" or ntype == "目标":
                        ...
                        if not 运行时:
                            ...
                            _SSE写入({"类型": "节点完成", "id": nid, "name": nname, "输入": "", "输出": f"员工「{员工名}」不存在，请先手动创建", "成功": False})
                        ...
                        if 启用工具 and self.操作注册中心:
                            ...
                            for _尝试 in range(1, 4):
                                ...
                                if 结果.get("成功"):
                                    ...
                                    _可重试 = any(kw in 错误 for kw in _可重试错误)
                                    if _可重试 and _尝试 < 3:
                                        _重试延迟 = _尝试 * 2
                                        print(f"  [WF] {nname} 第{_尝试}次失败({错误[:40]})，{_重试延迟}秒后重试")
                                        _SSE写入({"类型": "节点进度", "id": nid, "name": nname, "消息": f"⚠️ 第{_尝试}次失败，{_重试延迟}秒后重试..."})
                                        _time2.sleep(_重试延迟)
                                    else:
                                        回复 = None
                                        break
                        ...
                        # 输入/图片节点：优先从ComfyUI队列拿新图片
                        config = node.get("config", {})
                        输出 = ""
                        import time as _time_img
                        _已消费的图片 = set()
                        for _ in range(24):  # 24×5秒=120秒
                            _time_img.sleep(5)
                            # 每轮打印调试信息到后端+SSE
                            _队列长度 = len(网页请求处理器._comfyui图片队列) if hasattr(网页请求处理器, '_comfyui图片队列') else 0
                            _队列内容 = [f"{item.get('文件名','?')}({item.get('状态','?')})" for item in (网页请求处理器._comfyui图片队列 if hasattr(网页请求处理器, '_comfyui图片队列') else [])]
                            _调试msg = f"⏳ 等待图片... ({_+1}/24) 队列{_队列长度}项: {_队列内容}"
                            print(f"  [图片节点] {_调试msg}")
                            _SSE写入({"类型": "节点进度", "id": nid, "name": nname, "消息": _调试msg})
                            # 检查ComfyUI图片队列
                            if not hasattr(网页请求处理器, '_comfyui图片队列'):
                                网页请求处理器._comfyui图片队列 = []
                            for item in 网页请求处理器._comfyui图片队列:
                                if item.get("图片路径") and item.get("状态") == "完成":
                                    _路径 = item["图片路径"]
                                    if _路径 in _已消费的图片:
                                        continue
                                    输出 = _路径
                                    _已消费的图片.add(_路径)
                                    print(f"  [图片节点] ✅ 取到新图片: {输出}")
                                    _SSE写入({"类型": "节点进度", "id": nid, "name": nname, "消息": f"✅ 取到新图片: {输出}"})
                                    break
                            if 输出:
                                break
                        # fallback
                        if not 输出:
                            输出 = config.get("内容", "") or config.get("路径", "")
                            print(f"  [图片节点] ❌ 队列无新图，用config旧路径: {输出}")
                            _SSE写入({"类型": "节点进度", "id": nid, "name": nname, "消息": f"⚠️ 用旧路径: {输出}"})
                        # 更新config为新路径，让前端显示最新图片
                        if 输出:
                            config["路径"] = 输出
                            config["内容"] = 输出
                        节点输出[nid] = 输出
                        _SSE写入({"类型": "节点完成", "id": nid, "name": nname, "输入": "", "输出": 输出[:200] if 输出 else "(空)", "成功": True, "图片": 输出})
                    ...
                    elif ntype == "loop" or ntype == "循环":
                        # 循环节点：输出循环计数和上游输入，供下游节点使用
                        # config: {次数: 3, 间隔秒: 10}
                        循环次数 = int(config.get("次数", 3))
                        间隔秒 = int(config.get("间隔秒", 5))
                        上游输入 = _收集上游输入(nid)
                        结果列表 = []
                        for 循环i in range(循环次数):
                            _轮次输出 = f"循环第{循环i+1}/{循环次数}轮\n{上游输入}"
                            结果列表.append(_轮次输出)
                            _SSE写入({"类型": "节点进度", "id": nid, "name": nname, "当前": 循环i+1, "总数": 循环次数, "输出": _轮次输出[:200]})
                            if 循环i < 循环次数 - 1 and 间隔秒 > 0:
                                import time as _loop_time
                                _loop_time.sleep(间隔秒)
                        节点输出[nid] = "\n---\n".join(结果列表)
                        _SSE写入({"类型": "节点完成", "id": nid, "name": nname, "输入": 上游输入[:200], "输出": f"✅ 循环完成（{循环次数}次，间隔{间隔秒}秒）", "成功": True})
                        _wf写日志(nid, nname, ntype, "成功", 上游输入[:200], f"循环{循环次数}次", "", int((_time2.time()-_t0)*1000))
                        ...
                        if not api密钥:
                            ...
                            try:
                                if ntype == "zf3d_post":
                                    ...
                                    def _上传封面到网站(图片路径, 网站地址, api密钥):
                                        """用base64+urlencode上传封面，缩放+压缩为JPEG再上传"""
                                        import io as _io, base64 as _b64
                                        with open(图片路径, "rb") as _f:
                                            _原始数据 = _f.read()
                                        _上传数据 = _原始数据
                                        _文件名 = os.path.basename(图片路径)
                                        try:
                                            from PIL import Image as _PIL
                                            _img = _PIL.open(_io.BytesIO(_原始数据))
                                            if _img.mode in ('RGBA', 'P'):
                                                _img = _img.convert('RGB')
                                            _buf = _io.BytesIO()
                                            _img.save(_buf, format='JPEG', quality=85)
                                            _上传数据 = _buf.getvalue()
                                            _文件名 = _文件名.rsplit('.', 1)[0] + '.jpg' if '.' in _文件名 else _文件名 + '.jpg'
                                        except Exception:
                                            pass
                                        _b64数据 = _b64.b64encode(_上传数据).decode("ascii")
                                        print(f"  [AIGC上传] base64大小={len(_b64数据)} 字节, 原始={len(_原始数据)} 字节, JPEG={len(_上传数据)} 字节")
                                        _表单 = urlencode({"img": _b64数据, "filename": _文件名, "type": "cover"}).encode("utf-8")
                                        print(f"  [AIGC上传] 表单总大小={len(_表单)} 字节")
                                        ...
                                        try:
                                            with urllib.request.urlopen(_请求, timeout=60) as _响应:
                                                return json.loads(_响应.read().decode("utf-8"))
                                            ...
                                            try:
                                                ...
                                                pass
                                            print(f"  [AIGC上传] HTTP {_he.code} 错误响应: {_err_body}")
                                            raise
                                    # 收集所有上游输入，按拓扑顺序排列
                                    ...
                                    if not 封面原始 and len(_上游输出) >= 3:
                                        ...
                                        封面原始 = _上游输出[-1]
                                    # 过滤错误文本：不把上游错误信息当标题/内容
                                    _错误关键词 = ["连接错误", "调用失败", "超时", "Error:", "Traceback", "Exception", "不存在", "员工不存在", "未知错误", "失败", "Idle"]
                                    _标题是错误 = 标题 and any(kw in 标题 for kw in _错误关键词)
                                    if _标题是错误:
                                        print(f"  [AIGC] 标题疑似错误文本，跳过发布: {标题[:60]}")
                                        标题 = ""
                                        config["标题"] = ""
                                    if 内容 and any(kw in 内容 for kw in _错误关键词):
                                        print(f"  [AIGC] 内容疑似错误文本，清空: {内容[:60]}")
                                        内容 = ""
                                        config["内容"] = ""
                                    # 纯文本转HTML：如果没有HTML标签，按段落包装
                                    if 内容 and "<" not in 内容:
                                        _段落列表 = [p.strip() for p in 内容.split("\n") if p.strip()]
                                        内容 = "".join(f"<p>{p}</p>" for p in _段落列表)
                                    # 规范化路径：去掉多余的反斜杠（ComfyUI输出路径常带双反斜杠）
                                    if 封面原始 and isinstance(封面原始, str):
                                        封面原始 = 封面原始.replace("\\\\", "\\").strip()
                                    print(f"  [AIGC调试] 封面原始='{封面原始}' 标题='{(标题 or '')[:30]}' 内容长度={len(内容 or '')} 自动发布={config.get('自动发布', False)}")
                                    _SSE写入({"类型": "节点进度", "id": nid, "name": nname, "消息": f"🔍 封面原始='{封面原始[:60]}'"})
                                    ...
                                    if _url匹配:
                                        封面 = _url匹配.group()
                                        print(f"  [AIGC调试] 封面是URL: {封面}")
                                    elif 封面原始 and _os_aigc.path.exists(封面原始.strip()):
                                        print(f"  [AIGC调试] 封面文件存在: {封面原始}")
                                        # 本地图片文件，上传到网站
                                        _SSE写入({"类型": "节点进度", "id": nid, "name": nname, "消息": "📤 正在上传封面图片..."})
                                        try:
                                            _上传结果 = _上传封面到网站(封面原始.strip(), 网站地址, api密钥)
                                            if _上传结果.get("success"):
                                                封面 = f"{网站地址}{_上传结果.get('data', {}).get('url', '')}"
                                                _SSE写入({"类型": "节点进度", "id": nid, "name": nname, "消息": f"✅ 封面上传成功: {封面[:60]}"})
                                            else:
                                                封面 = ""
                                                _封面上传失败原因 = _上传结果.get("message", "上传失败")
                                        except Exception as _upload_err:
                                            封面 = ""
                                            _封面上传失败原因 = str(_upload_err)
                                            _SSE写入({"类型": "节点进度", "id": nid, "name": nname, "消息": f"❌ 封面上传失败: {_封面上传失败原因[:80]}"})
                                    else:
                                        封面 = 封面原始
                                        print(f"  [AIGC调试] 封面既不是URL也不是本地文件: '{封面原始}'")
                                        _SSE写入({"类型": "节点进度", "id": nid, "name": nname, "消息": f"❌ 封面路径无效: '{封面原始[:60]}'"})
                                    分类 = config.get("分类", "100")
                                    自动发布 = config.get("自动发布", False)
                                    # 存预览数据到节点输出
                                    _预览数据 = {"标题": 标题, "内容": 内容, "封面": 封面原始 or _上游URL, "分类": 分类, "网站地址": 网站地址, "api密钥": api密钥}
                                    # 写回node.config（仅在内容非空时才覆盖，防止错误文本持久化）
                                    if 标题:
                                        config["标题"] = 标题
                                    if 内容:
                                        config["内容"] = 内容
                                    ...
                                    # 自动发布模式：AIGC节点自己每5秒扫描ComfyUI图片队列
                                    # 不依赖图片节点端口传递，直接从队列拿图片
                                    if 自动发布 and len(_纯文本) >= 5 and len(_内容纯文本) >= 20 and not 封面:
                                        _SSE写入({"类型": "节点进度", "id": nid, "name": nname, "消息": "⏳ AIGC开始扫描图片队列..."})
                                        import time as _wait_img
                                        _上传失败 = False
                                        for _尝试 in range(24):
                                            _wait_img.sleep(5)
                                            if not hasattr(网页请求处理器, '_comfyui图片队列'):
                                                网页请求处理器._comfyui图片队列 = []
                                            _队列长度 = len(网页请求处理器._comfyui图片队列)
                                            _队列内容 = [f"{item.get('文件名','?')}({item.get('状态','?')})" for item in 网页请求处理器._comfyui图片队列]
                                            _调试msg = f"⏳ AIGC扫描 ({_尝试+1}/24) 队列{_队列长度}项: {_队列内容}"
                                            print(f"  [AIGC] {_调试msg}")
                                            _SSE写入({"类型": "节点进度", "id": nid, "name": nname, "消息": _调试msg})
                                            for _item in 网页请求处理器._comfyui图片队列:
                                                if _item.get("状态") == "完成" and _item.get("图片路径"):
                                                    封面原始 = _item["图片路径"].replace("\\\\", "\\").strip()
                                                    print(f"  [AIGC] 找到图片: {封面原始}")
                                                    _SSE写入({"类型": "节点进度", "id": nid, "name": nname, "消息": f"📁 找到图片: {封面原始[:60]}"})
                                                    if _os_aigc.path.exists(封面原始):
                                                        _SSE写入({"类型": "节点进度", "id": nid, "name": nname, "消息": "📤 正在上传封面..."})
                                                        try:
                                                            _上传结果 = _上传封面到网站(封面原始, 网站地址, api密钥)
                                                            if _上传结果.get("success"):
                                                                封面 = f"{网站地址}{_上传结果.get('data', {}).get('url', '')}"
                                                                config["封面"] = 封面原始
                                                                print(f"  [AIGC] ✅ 上传成功: {封面}")
                                                                _SSE写入({"类型": "节点进度", "id": nid, "name": nname, "消息": f"✅ 封面上传成功: {封面[:60]}"})
                                                            else:
                                                                print(f"  [AIGC] ❌ 上传失败: {_上传结果.get('message','')}")
                                                                _SSE写入({"类型": "节点进度", "id": nid, "name": nname, "消息": f"❌ 上传失败: {_上传结果.get('message','')[:60]}"})
                                                                _上传失败 = True
                                                        except Exception as _img_err:
                                                            print(f"  [AIGC] ❌ 异常: {_img_err}")
                                                            _SSE写入({"类型": "节点进度", "id": nid, "name": nname, "消息": f"❌ 异常: {str(_img_err)[:60]}"})
                                                            _上传失败 = True
                                                    else:
                                                        print(f"  [AIGC] ❌ 文件不存在: {封面原始}")
                                                        _SSE写入({"类型": "节点进度", "id": nid, "name": nname, "消息": f"❌ 文件不存在: {封面原始[:60]}"})
                                                        _上传失败 = True
                                                    break
                                            if 封面:
                                                print(f"  [AIGC] 上传成功，等待5秒后继续发布...")
                                                _wait_img.sleep(5)
                                                break
                                            if _上传失败:
                                                print(f"  [AIGC] 上传失败，停止重试")
                                                break
                                        _状态[2] = "✅封面" if 封面 else "❌封面"

                                    if 自动发布 and 封面 and len(_纯文本) >= 5 and len(_内容纯文本) >= 20:
                                        # === 重复内容检测：与上次发布的标题/内容比较 ===
                                        _上次记录 = getattr(网页请求处理器, '_aigc上次发布', {})
                                        _上次标题 = _上次记录.get("标题", "")
                                        _上次内容 = _上次记录.get("内容", "")
                                        _上次封面 = _上次记录.get("封面", "")
                                        # 标题完全相同，或内容前100字相同，或封面URL相同 → 跳过
                                        _标题重复 = _纯文本 == _上次标题
                                        _内容重复 = _内容纯文本[:100] == _上次内容[:100] if _内容纯文本 and _上次内容 else False
                                        _封面重复 = 封面 and 封面 == _上次封面
                                        if _标题重复 or _内容重复 or _封面重复:
                                            _重复原因 = []
                                            if _标题重复: _重复原因.append("标题与上次相同")
                                            if _内容重复: _重复原因.append("内容与上次相同")
                                            if _封面重复: _重复原因.append("封面与上次相同")
                                            输出 = f"⏭️ 跳过发布（{'，'.join(_重复原因)}）\n标题: {标题[:50]}"
                                            节点输出[nid] = 输出
                                            _SSE写入({"类型": "节点完成", "id": nid, "name": nname, "输入": 上游输入[:200], "输出": 输出, "成功": True})
                                            _wf写日志(nid, nname, ntype, "跳过", 上游输入[:200], 输出[:500], "重复内容", int((_time2.time()-_t0)*1000))
                                        else:
                                            # === 自动发布模式：直接调用API发布 ===
                                            try:
                                                _表单 = urlencode({
                                                    "title": 标题, "content": 内容,
                                                    "category_id": 分类, "source": "AI生成",
                                                    "cover": 封面, "user_id": str((网页请求处理器._网站会话 or {}).get("用户", {}).get("user_id", ""))
                                                }).encode("utf-8")
                                                _接口 = f"{网站地址}/api/agent_api.asp?a=article&key={quote(api密钥)}"
                                                _请求 = urllib.request.Request(_接口, data=_表单, method="POST")
                                                _请求.add_header("Content-Type", "application/x-www-form-urlencoded")
                                                with urllib.request.urlopen(_请求, timeout=120) as _响应:
                                                    _发布结果 = json.loads(_响应.read().decode("utf-8"))
                                                if _发布结果.get("success"):
                                                    _d = _发布结果.get("data", {})
                                                    _文章ID = _d.get("id", "?")
                                                    _文章URL = f"{网站地址}{_d.get('url', '')}"
                                                    # 记录本次发布的标题/内容/封面，供下次比较
                                                    网页请求处理器._aigc上次发布 = {"标题": _纯文本, "内容": _内容纯文本, "封面": 封面}
                                                    输出 = f"✅ 自动发布成功\nID: {_文章ID}\nURL: {_文章URL}\n标题: {标题[:50]}"
                                                    节点输出[nid] = 输出 + "\n__AIGC_PUBLISHED__" + _json_aigc.dumps({"id": _文章ID, "url": _文章URL, "标题": 标题}, ensure_ascii=False)
                                                    _SSE写入({"类型": "节点完成", "id": nid, "name": nname, "输入": 上游输入[:200], "输出": 节点输出[nid], "成功": True})
                                                    _wf写日志(nid, nname, ntype, "成功", 上游输入[:200], 输出[:500], "", int((_time2.time()-_t0)*1000))
                                                else:
                                                    _err = _发布结果.get("message", "发布失败")
                                                    输出 = f"❌ 自动发布失败: {_err}"
                                                    节点输出[nid] = 输出
                                                    _SSE写入({"类型": "节点完成", "id": nid, "name": nname, "输入": 上游输入[:200], "输出": 输出, "成功": False})
                                                    _wf写日志(nid, nname, ntype, "失败", 上游输入[:200], 输出[:500], _err, int((_time2.time()-_t0)*1000))
                                            except Exception as _pub_ex:
                                                输出 = f"❌ 自动发布异常: {_pub_ex}"
                                                节点输出[nid] = 输出
                                                _SSE写入({"类型": "节点完成", "id": nid, "name": nname, "输入": 上游输入[:200], "输出": 输出, "成功": False})
                                                _wf写日志(nid, nname, ntype, "失败", 上游输入[:200], 输出[:500], str(_pub_ex), int((_time2.time()-_t0)*1000))
                                    else:
                                        # === 预览模式：不发布，等用户双击确认 ===
                        ...
                        # 用锁保护检查，防止并发时多个节点同时通过检查
                        with 网页请求处理器._comfyui全局锁:
                            if 网页请求处理器._comfyui执行中:
                                ...
                                网页请求处理器._comfyui执行中[nid] = "pending"
                                _comfyui跳过 = False
                        if not _comfyui跳过:
                            上游输入 = _收集上游输入(nid)
            ...
            # 并发延迟包装：同层节点逐个延迟1帧启动，避免同时请求API导致连接错误
            _并发延迟秒 = 0.05
            def _延迟执行节点(nid, idx):
                if idx > 0:
                    _time2.sleep(idx * _并发延迟秒)
                _执行节点(nid)
                ...
                for lv in 非循环层:
                    ...
                    if len(层) == 1:
                        ...
                        with _cf.ThreadPoolExecutor(max_workers=len(层)) as executor:
                            futures = {executor.submit(_延迟执行节点, nid, idx): nid for idx, nid in enumerate(层)}
