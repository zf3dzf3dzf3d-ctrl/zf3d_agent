...
class 进化引擎类:
    ...
    def __init__(self, 模型直连器, 项目根目录, 配置):
        ...
        self._对话测试模式 = False
        self._沙箱目录 = None
        self._导师介入阈值 = 3   # 连续失败3次→导师介入
        self._排错员阈值 = 6    # 连续失败6次→排错员兜底
    ...
    def _尝试升级介入(self, 消息, 文件, 代码, 问题列表):
        """根据连续失败次数决定是否触发导师或排错员"""
        失败次数 = self._读取连续失败()

        # 第1级：导师介入（失败3次）— 重新分析问题给新思路
        if 失败次数 == self._导师介入阈值 and not self._导师已介入:
            self._导师已介入 = True
            self._日志记录("导师", f"介入：连续失败{失败次数}次，重新分析问题")
            导师建议 = self._导师分析(文件, 代码, 问题列表)
            if 导师建议:
        ...
        # 第2级：排错员兜底（失败6次）— 直接给出修复方案
        if 失败次数 >= self._排错员阈值 and not self._排错员已介入:
            self._排错员已介入 = True
            self._日志记录("排错员", f"介入：连续失败{失败次数}次，接管修复")
            ...
            if 排错方案 and 排错方案.get("修改列表"):
                ...
                if 修改详情:
                    ...
                    return
            self._日志记录("排错员", "修复失败，放弃此文件")
            # 重置标志，下一轮可以再次介入
            self._导师已介入 = False
            self._排错员已介入 = False
            self._重置连续失败()

    def _导师分析(self, 文件, 代码, 问题列表):
        """导师：分析失败原因，给出新修复思路"""
        错误摘要 = "\n".join([f"- {e['原因']}" for e in self._错误历史[-5:]])
        提示 = (
            f"开发者连续修复失败{self._读取连续失败()}次。请分析为什么失败，给出新的修复思路。\n\n"
            f"文件: {文件}\n"
            f"问题列表: {json.dumps(问题列表, ensure_ascii=False)[:1000]}\n"
            f"失败历史:\n{错误摘要}\n\n"
            f"代码:\n{self._截断代码(代码)}\n\n"
            f"请分析：1.之前修复为什么失败 2.有没有不同的修复思路 3.是否应该跳过这个问题\n"
            f"输出JSON: {{\"分析\": \"...\", \"新思路\": \"...\", \"建议跳过\": false}}"
        )
        回复 = self._调用LLM(
            "你是资深开发导师。开发者修复代码时连续失败，你需要分析失败原因并给出新的修复思路。只输出JSON。",
            提示
        )
        结果 = self._提取JSON(回复)
        if 结果:
