        ...
        try:
            with self._规则锁:
                ...
                for 规则 in 规则列表["规则列表"]:
                    ...
                    # 模块匹配
                    if 规则.get("目标模块"):
                        if 规则["目标模块"] not in 记录.get("来源", ""):
                            命中 = False
                    # 函数匹配
                    if 规则.get("目标函数") and 命中:
                        if 规则["目标函数"] not in 记录.get("来源", ""):
                            命中 = False
                    # 异常类型匹配
                    if 规则.get("异常类型") and 命中:
                        if 规则["异常类型"] != 记录.get("异常类型", ""):
                            命中 = False
                    # 关键字匹配
