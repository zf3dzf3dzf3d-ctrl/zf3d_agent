...
class 多线程下载(操作基类):
    ...
    def _执行下载(self, 下载地址, 保存路径, 线程数, 重试次数, 下载ID=0, 文件名=""):
        ...
        try:
            # 优先使用 aria2c（可通过实例属性禁用）
            _aria2c路径 = self._aria2c路径 if hasattr(self, '_aria2c路径') else _检测aria2c()
            if _aria2c路径:
    ...
    def _多线程下载(self, url, 保存路径, 文件大小, 线程数, 重试次数, 临时目录, 元数据路径, 下载ID=0, 文件名=""):
        ...
        while any(t.is_alive() for t in 线程列表):
            ...
            耗时 = time.time() - 开始时间
            本次新增 = 已完成字节数 - 上次已下载  # 只算本次实际下载的字节
            速度 = 本次新增 / (1024*1024) / 耗时 if 耗时 > 0 else 0
            百分比 = 已完成字节数 * 100 // 文件大小 if 文件大小 > 0 else 0
