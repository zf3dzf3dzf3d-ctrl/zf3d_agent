...
class 图片分析(操作基类):
    名称 = "图片分析"
    描述 = "读取本地图片文件并用大模型分析（支持png/jpg/gif/webp/bmp）。【重要】此操作需要模型支持vision，如果当前模型不支持vision则不要调用此操作，也不要先截图再分析——直接用文字描述结果即可"
    参数结构 = {
    ...
    def 执行(self, 参数: dict) -> 操作结果:
        ...
        try:
            ...
            if not getattr(self.模型直连器, "支持vision", False):
                模型名 = getattr(self.模型直连器, "当前模型名", "当前模型")
                return 操作结果.失败(
                    f"当前模型「{模型名}」不支持图片分析(vision)。"
                    f"请在设置中切换到支持vision的模型（如通义千问、智谱GLM等），再重试。"
                )
