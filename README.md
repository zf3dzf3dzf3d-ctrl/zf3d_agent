# ZF3D Community Agent Infinity 5.2.0 · Introduction
...
### 🏊 Chat Pool System (chat_pool) — the headline upgrade
Previously a chat round was "carried by your browser": a Wi-Fi blip killed the upstream request, losing the reply and triggering a retry storm that resent the whole round. 5.2.0 moves request ownership into a **server-side memory pool**:

| Layer | What | Benefit |
|---|---|---|
| **A Buffer** | Request ownership moves from browser to a server slot; upstream replies land in a slot event buffer | No lost replies on Wi-Fi blips/page refresh, no retry storms |
| **B Connection Pool** | Domain-grouped HTTPS keep-alive reuse | Saves 50~200ms of TCP+TLS handshake per request |
| **C Prefix-Cache Observability** | Server parses cached_tokens from upstream usage | Cache hit rate / savings become visible numbers |
| **D Session Pool** | Persistent dialog→slot mapping + usage stats | Refresh the page and resume instantly |

### 🚦 chat_gate v4 Traffic Control · 🐕 Dog Guard v5.2 · 🔥 Hot Reload
One gate schedules all LLM requests (per-domain lanes + AIMD adaptation); the guard only flags truly stalled tools (30 min default, deduped, quiet while busy); server modules hot-reload without restarting; kite rendering optimized; fireworks/notify and minimize-button fixes.
...
| | | |
|---|---|---|
| ![整体界面](https://zf3d-img.oss-cn-guangzhou.aliyuncs.com/agent/images/5.2.0/ui-overview.jpg) | ![对话列表](https://zf3d-img.oss-cn-guangzhou.aliyuncs.com/agent/images/5.2.0/nav-panel.jpg) | ![多种分类编程模式](https://zf3d-img.oss-cn-guangzhou.aliyuncs.com/agent/images/5.2.0/model-key.jpg) |
| *Main UI · Infinite Canvas* | *400+ Parallel Dialogs* | *Coding Modes* |
| ![集成多个底部引擎](https://zf3d-img.oss-cn-guangzhou.aliyuncs.com/agent/images/5.2.0/engine.jpg) | ![模型配置管家](https://zf3d-img.oss-cn-guangzhou.aliyuncs.com/agent/images/5.2.0/model-key-config.jpg) | ![任务列表](https://zf3d-img.oss-cn-guangzhou.aliyuncs.com/agent/images/5.2.0/long-task.jpg) |
| *Six Engine Backends* | *Model Config Manager* | *Task List & Long Plans* |
| ![风筝和小狗守卫](https://zf3d-img.oss-cn-guangzhou.aliyuncs.com/agent/images/5.2.0/kite-system.jpg) | ![文件树和缩略图](https://zf3d-img.oss-cn-guangzhou.aliyuncs.com/agent/images/5.2.0/file-system-undo.jpg) | ![日志和上下文](https://zf3d-img.oss-cn-guangzhou.aliyuncs.com/agent/images/5.2.0/calendar-weather.jpg) |
...
# ZF3D Community Agent Infinity 5.2.0 - Complete Help (Full Documentation with Screenshots)

> **New in 5.2.0**: Chat Pool (chat_pool) keeps replies safe through Wi-Fi drops and page refreshes (no more retry storms), connection pooling saves 50~200ms per request, prefix-cache observability shows cache hit rates and savings, chat_gate v4 schedules all LLM traffic, Dog Guard v5.2 detects truly stalled tools (30 min default), server modules hot-reload without restarts, and kite rendering is optimized. See the "What's New in 5.2.0" section above for details.

> This document covers all current features of the software, written after a feature-by-feature code review (~150 frontend modules in public/js, server/ route Mixins and six engines, the three toolkits in tools/, extensions/, plugin modes in modes/, and archived notes in 项目记录/).
...
Tool definitions are split across files (tools-defs-*.js frontend + tools/ server implementations), switched by category (tools-00-category-state.js, tools-02-category-switch.js):
...
- **Video generation engine** (tools/video_gen_engine.py, tools/): server-side video generation.
...
- **Video generation**: server-side video engine (tools/video_gen_engine.py, tools/), played in media nodes.
...
- **Audio/video processing** (tools/ pipeline & media nodes): server-side conversion and playback.
...
- **Update check** (app-update.js, app-version.js, tools/check_release.py): version check and updates.
...
├── tools/       # Three toolkits (minimal / coding / writing) + video generation engine
├── modes/       # Plugin modes
├── extensions/  # MCP, skills, declarative UI
├── docs/        # Docs (help & intro for 5.2.0)
├── private/     # Private configs (API keys, not committed)
├── system/      # System core files
├── 项目记录/     # Change logs, file change index, dev archive
├── Linux和Mac帮助包/ # Linux & Mac help package
├── index.html   # Entry page
├── wake-word.js # Voice wake word
└── zf3d.ico     # Program icon
```
...
| **B 连接池** | 按 API 域名分组的 HTTPS 长连接复用 | 每请求省 TCP+TLS 握手 50~200ms |
| **C 前缀缓存观测** | 服务端解析上游 usage 里的 cached_tokens | 缓存命中率/省钱额变成可见数字 |
| **D 会话池** | 对话→槽位持久映射 + 用量统计 | 多对话集中管理、刷新页面秒恢复 |

### 🚦 对话交通系统 chat_gate v4
所有大模型请求经唯一闸门调度：按接口域名分车道组 + AIMD 自适应调度 + 车道数自适应伸缩。**闸门=交警（谁先走），池=车队+仓库（用什么走、货在哪）**。

### 🐕 小狗守卫 v5.2：工具停滞检测
只有「真正在运行但工具超过阈值（默认 30 分钟）无动静」才触发提醒，自动去重、忙时免打扰；崩溃报告不丢失，日志自动轮转。

### 🔥 热更新引擎 / 🪁 风筝优化 / 🎆 交互修复
服务端模块热重载即改即生效；风筝节点渲染与位置管理专项优化；烟花通知必弹、最小化按钮补回等稳定性修复。
...
---
# 朱峰社区智能体无限（Infinity）5.2.0 · 完整帮助（带截图全文档）
...
| 六 | [画布流水线与流程图（5.2.0）](#六画布流水线与流程图520) | 进阶用户 |

### 👥 第三部分 · 多智能体协作（让多个 AI 分工干活）
| 章 | 内容 | 适合谁 |
|---|---|---|
| 七 | [派小弟协议：多智能体派工（5.2.0）](#七派小弟协议多智能体派工520) | 进阶用户 |
| 八 | [风筝系统](#八风筝系统) | 进阶用户 |
| 九 | [发散收敛多视角对话](#九发散收敛多视角对话) | 进阶用户 |
| 十 | [跨主机远程控制（5.2.0）](#十跨主机远程控制520) | 有多台电脑的用户 |
...
| 十五 | [知识图谱系统（5.2.0）](#十五知识图谱系统520) | 进阶用户 |
...
> 各版本的主要变化一览，方便老用户快速了解升级内容。当前版本：**v5.2.0**。

### v5.2.0（2026-09）—— 对话池与后勤线重造时代
- **对话池系统（chat_pool）**：请求所有权移交服务器槽位，断网/刷新不掉字、断点续读，杜绝重试风暴
- **连接池**：按域名分组 HTTPS 长连接复用，每请求省 50~200ms 握手
- **前缀缓存观测**：cached_tokens 解析，缓存命中率与省钱额可见
- **chat_gate v4**：唯一闸门调度，域名分车道 + AIMD 自适应 + 车道伸缩
- **小狗守卫 v5.2**：工具停滞检测（默认 30 分钟）、忙时免打扰、日志轮转
- **热更新引擎**：服务端模块热重载、前端变更自动通知刷新
- **风筝系统性能优化** + 烟花通知兜底、最小化按钮补回等交互修复
...
![整体界面](https://zf3d-img.oss-cn-guangzhou.aliyuncs.com/agent/images/5.2.0/ui-overview.jpg)
...
![对话列表](https://zf3d-img.oss-cn-guangzhou.aliyuncs.com/agent/images/5.2.0/nav-panel.jpg)
...
![文件树和缩略图](https://zf3d-img.oss-cn-guangzhou.aliyuncs.com/agent/images/5.2.0/file-system-undo.jpg)
...
## 七、派小弟协议：多智能体派工（5.2.0）

![风筝和小狗守卫](https://zf3d-img.oss-cn-guangzhou.aliyuncs.com/agent/images/5.2.0/kite-system.jpg)
...
![多种分类编程模式](https://zf3d-img.oss-cn-guangzhou.aliyuncs.com/agent/images/5.2.0/model-key.jpg)
...
![集成多个底部引擎](https://zf3d-img.oss-cn-guangzhou.aliyuncs.com/agent/images/5.2.0/engine.jpg)

Claude Code / Codex / DeepSeek 直连 / Hermes / OpenClaw / Pi 风格，各自带专属工具集，即插即用可扩展。引擎日志完善，问题定位快。

## 十三、多模型配置与模型配置管家

![模型配置管家](https://zf3d-img.oss-cn-guangzhou.aliyuncs.com/agent/images/5.2.0/model-key-config.jpg)
...
![任务列表](https://zf3d-img.oss-cn-guangzhou.aliyuncs.com/agent/images/5.2.0/long-task.jpg)
...
- **跨对话搜索**：搜索所有历史对话内容
- **文件变更溯源（5.2.0）**：所有文件修改自动生成 .bak 备份并记账，task_complete 结算时写入 `项目记录/变更日志.md` 与 `文件变更索引.md`，每次任务改了什么可追溯
- **SQLite 持久化**：每 5 秒自动保存，后台离线回退本地存储

## 十九、守护系统：小狗守卫 + 健康守护

![日志和上下文](https://zf3d-img.oss-cn-guangzhou.aliyuncs.com/agent/images/5.2.0/calendar-weather.jpg)

| 守护 | 职责 |
|---|---|
| 🐕 小狗守卫 | 任务彻底失败自动救、停滞超过阈值（默认 30 分钟）主动干预、空闲日常巡逻；所有动作记进「📒 账本」可查 |
...
![导航区域](https://zf3d-img.oss-cn-guangzhou.aliyuncs.com/agent/images/5.2.0/nav-area.jpg)
![历史菜单](https://zf3d-img.oss-cn-guangzhou.aliyuncs.com/agent/images/5.2.0/history-menu.jpg)
...
├── index.html         # 入口页面
├── server/            # 服务端（模块化 Python、六大引擎、热更新、对话池 chat_pool）
├── public/            # 前端（app-*.js 模块、无限画布）
├── tools/             # 三大工具集（coding/writing/minimal）+ 视频生成引擎
├── modes/             # 对话模式（插件模式系统）
├── extensions/        # MCP 等扩展
├── docs/              # 文档（5.2.0 使用帮助与介绍）
├── private/           # 隐私区（API Key、版本号）
├── system/            # 系统核心文件
├── 项目记录/           # 变更日志、文件变更索引、开发档案
├── Linux和Mac帮助包/   # Linux/Mac 启动脚本与说明
...
**Q：任务失败怎么办？**
A：失败自适应重启对话，配合离岗模式可无人值守；小狗守卫会在彻底失败时自动救、停滞超过阈值（默认 30 分钟）主动干预。

**Q：怎么让多个 AI 一起干活？**
A：5.2.0 用派小弟协议——主脑对话把任务派给小弟节点，监控→回收→三关验收全自动；也可以用风筝系统分派，或复制会话做 A/B 对照。
...
**Q：怎么记录文件改动历史？**
A：5.2.0 自动记录——所有文件修改自动生成 .bak 与变更索引，task_complete 结算时写入 `项目记录/变更日志.md` 与 `文件变更索引.md`。
