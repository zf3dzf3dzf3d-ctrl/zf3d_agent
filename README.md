...
**全能桌面AI助手。系统做调度执行，大模型做思考决策。节点工作流编排多AI协作，日历计划任务按时间自动调度，276+操作即开即用。零依赖、零安装、复制即运行。**

---

## 📋 v3.2.1 更新摘要

- **日历计划任务** — 月历视图创建定时任务，支持仅一次/每日/每周/每月/间隔模式，前端离线时Windows桌面弹窗通知
- **21个操作节点** — 文件操作(4)/网络操作(3)/系统操作(2)/图片处理(4)/代码工具(2)/媒体(4) 全部零token
- **员工列表整合** — 数字员工固定在工作流面板右侧，拖拽到画布即创建节点
- **开发热重载** — 前端文件保存自动刷新浏览器，后端重启按钮一键重载
- **云端出图** — 免ComfyUI直接调用API出图，支持AgnesAI(免费)/Seedream/Nano Banana/Grok/GPT Image
- **版本号统一+隐私加固** — 清除硬编码个人路径，MCP配置改为相对路径
...
- **拖拽编排** — 从员工列表拖入画布创建节点，连线即定义执行流向，一图看清整个协作流程
- **8方向缩放** — 面板支持左/右/上/下/四角8方向自由拖拽调整大小
- **循环控制** — 连线支持单次执行、固定循环（N轮累积）、条件循环（AI判断跳出）
- **AI一键设计** — 大白话描述需求，AI自动创建节点、分配员工、连线布局
- **文件拖入** — 图片/文本拖入画布即创建输入节点，数据在节点间自动流转
- **执行全程透明** — 节点状态变色（待执行→执行中→完成/失败）、连线小球动画、双击查看输入输出、日志逐条记录
- **剪刀模式** — 按住Y键画线切断连线，快速调整流程
- **分组框** — Ctrl+G创建分组，可收缩/展开/改颜色/拖动
...
- **月历视图** — 点击日期选时间，直观创建定时任务
- **5种重复模式** — 仅一次 / 每日 / 每周 / 每月 / 间隔（N分钟循环）
- **双通道通知** — 前端在线时浏览器内弹窗+提示音；前端离线时Windows MessageBox置顶弹窗
- **任务管理** — ▶立即执行 / ✏️修改 / ⏸️暂停 / 🗑️删除
- **子目录搜索** — 工作流文件支持按文件夹分类管理（如 `节点图/日常/xxx.json`）

## 👥 数字员工系统（v3.0+）

每个员工都是独立人格，各司其职：

- **独立人设** — 每个员工有专属提示词、角色、目标，AI自动生成人设
- **独立记忆** — 每个员工有独立记忆库，互不干扰
- **工具调用** — 高级员工可启用文件操作、代码执行、图片生成等实操能力
- **层级管理** — 拖拽分配上下级关系，树形结构展示
- **分类管理** — 自定义颜色分类，折叠/排序/拖入
- **独立对话** — 每个员工有独立聊天窗口（SSE流式），@提及切换
- **定时提醒** — 员工可主动弹窗通知用户
